# TODO: Translation updated at 2023-05-27 13:27

# game/code/e06/e06s04.rpy:20
translate russian e06s04_882706cd:

    # lc "It's all set."
    lc "Всё готово."

# game/code/e06/e06s04.rpy:26
translate russian e06s04_925f9977:

    # lc "I'm going to go freshen up."
    lc "Пойду освежусь."

# game/code/e06/e06s04.rpy:29
translate russian e06s04_fa102d42:

    # mc "Alright lover."
    mc "Хорошо, любимая."

# game/code/e06/e06s04.rpy:38
translate russian e06s04_6be74b0a:

    # mc "Welcome to la Casa de Cox."
    mc "Добро пожаловать в \"la Casa de Cox\"."

# game/code/e06/e06s04.rpy:41
translate russian e06s04_032676d2:

    # arj "Where should we put our luggage?"
    arj "Где нам поставить наши вещи?"

# game/code/e06/e06s04.rpy:44
translate russian e06s04_e478ac19:

    # mc "Don't worry about them. We're having a pool day. Min is already out there."
    mc "Не беспокойся о них. У нас будет день бассейна. Мин уже там."

# game/code/e06/e06s04.rpy:47
translate russian e06s04_c172bd12:

    # sy "Yes."
    sy "Да."

# game/code/e06/e06s04.rpy:50
translate russian e06s04_1c5f6920:

    # arj "I'd rather get things organized."
    arj "Я бы предпочла все организовать."

# game/code/e06/e06s04.rpy:53
translate russian e06s04_19b3d85c:

    # mc "It's okay just to cut loose today, sweetheart."
    mc "Детка, сегодня можно просто расслабиться."

# game/code/e06/e06s04.rpy:57
translate russian e06s04_4ed3dbd6:

    # mc "We've got lunch coming in an hour. And I'll help move everything where it needs to go."
    mc "Через час будет обед. Я помогу отнести все туда, куда нужно."

# game/code/e06/e06s04.rpy:58
translate russian e06s04_5629437d:

    # mc "My first priority is everyone just gets to relax and have fun."
    mc "Мой главный приоритет - чтобы все просто расслабились и получили удовольствие."

# game/code/e06/e06s04.rpy:61
translate russian e06s04_0102589e:

    # arj "Okay. I can relax. Totally."
    arj "Ладно. Я могу расслабиться. Полностью."

# game/code/e06/e06s04.rpy:74
translate russian e06s04_056ca269:

    # sy "Come on red, let's check out our quarters in the master's manse."
    sy "Пойдем, рыжая, проверим наши покои в хозяйском особняке."

# game/code/e06/e06s04.rpy:89
translate russian e06s04_ce84d147:

    # mes "Hey handsome."
    mes "Привет, красавчик."

# game/code/e06/e06s04.rpy:90
translate russian e06s04_b916dbc0:

    # mes "What kept you?"
    mes "Что тебя задержало?"

# game/code/e06/e06s04.rpy:94
translate russian e06s04_454a3661:

    # mc "Final guests just arrived."
    mc "Последние гости только что прибыли."

# game/code/e06/e06s04.rpy:97
translate russian e06s04_0f606fb8:

    # mes "Mmm. Think you have time to reapply my sunscreen before you have your hands full?"
    mes "Ммм. Думаешь, успеешь намазать меня солнцезащитным кремом до того, как у тебя будут заняты руки?"

# game/code/e06/e06s04.rpy:103
translate russian e06s04_1bf60051:

    # mc "Of course."
    mc "Конечно."

# game/code/e06/e06s04.rpy:120
translate russian e06s04_fefca8ce:

    # mc "All done."
    mc "Готово."

# game/code/e06/e06s04.rpy:123
translate russian e06s04_8042f4f4:

    # mes "Umm. I think you missed a few spots?"
    mes "Мне кажется, ты пропустил пару мест?"

# game/code/e06/e06s04.rpy:126
translate russian e06s04_5a034afa:

    # mc "Haha. If I start on that, I'll never get to work on my own tan."
    mc "Хаха. Если я начну заниматься этим, то никогда не смогу заняться своим загаром."

# game/code/e06/e06s04.rpy:134
translate russian e06s04_b5b37a2e:

    # mes "Spoilsport. But I'm sure I'll get some time with my favorite teacher tonight."
    mes "Испорченный. Но я уверена, что сегодня у меня будет немного времени с моим любимым учителем."

# game/code/e06/e06s04.rpy:136
translate russian e06s04_8d5661a3:

    # mes "Mmm. I guess I can be patient. For now. Not like I have to wait as long as Lydia is."
    mes "Ммм. Думаю, я потерплю. Пока. Вряд ли мне придется ждать так долго, как Лидии."

# game/code/e06/e06s04.rpy:156
translate russian e06s04_df5835f9:

    # mes "Hey you two."
    mes "Привет вам обеим."

# game/code/e06/e06s04.rpy:159
translate russian e06s04_1cefeaf9:

    # arj "Hello Min."
    arj "Привет, Мин."

# game/code/e06/e06s04.rpy:162
translate russian e06s04_180fde8f:

    # sy "Cannonball!"
    sy "Пушечное ядро!"

# game/code/e06/e06s04.rpy:171
translate russian e06s04_ece89939:

    # mes "Well, we certainly know who the wild card of the bunch is."
    mes "Ну, мы, конечно, знаем, кто является \"дикой картой\" в этой группе."

# game/code/e06/e06s04.rpy:174
translate russian e06s04_0a979c32:

    # arj "I thought it might be you, Min."
    arj "Я думала, это можешь быть ты, Мин."

# game/code/e06/e06s04.rpy:177
translate russian e06s04_18356222:

    # mes "Me? Never. If anything, I'm Chaotic Lawful."
    mes "Я? Ни в коем случае. Если уж на то пошло, я - хаотичный законник."

# game/code/e06/e06s04.rpy:180
translate russian e06s04_473111d5:

    # mes "Stacy, on the other hand, is completely Chaotic Neutral."
    mes "Стейси, с другой стороны, полностью хаотично-нейтральна."

# game/code/e06/e06s04.rpy:183
translate russian e06s04_5305554f:

    # arj "Absolutely."
    arj "Абсолютно точно."

# game/code/e06/e06s04.rpy:186
translate russian e06s04_63313904:

    # mc "One of these days, you're going to hurt yourself pulling something like that."
    mc "В один прекрасный день ты покалечишься, вытворяя что-то подобное."

# game/code/e06/e06s04.rpy:195
translate russian e06s04_c7217a3c:

    # sy "Okay \"dad\"."
    sy "Хорошо \"пап\"."

# game/code/e06/e06s04.rpy:197
translate russian e06s04_41821a70:

    # sy "You know I'm not a little girl anymore, [mcname]."
    sy "Ты знаешь, что я уже не маленькая девочка, [mcname]."

# game/code/e06/e06s04.rpy:198
translate russian e06s04_cac31bb5:

    # sy "I can take care of myself."
    sy "Я могу позаботиться о себе."

# game/code/e06/e06s04.rpy:201
translate russian e06s04_7e44b287:

    # mc "I think that's exactly what you said before you wiped out your first time on a skateboard."
    mc "Думаю, именно это ты сказала, когда впервые встала на скейтборд."

# game/code/e06/e06s04.rpy:204
translate russian e06s04_d7ecf23b:

    # sy "Yup. By 'take care of myself', I mean I know I'll always have someone to help me when I screw up."
    sy "Да. Под \"позаботиться о себе\" я имею в виду, что я знаю, что у меня всегда есть кто-то, кто поможет мне, если я облажаюсь."

# game/code/e06/e06s04.rpy:213
translate russian e06s04_d74cf3ea:

    # arj "So, Stacy and I will share a room. And then you and Lydia have your own rooms."
    arj "Итак, мы со Стейси будем жить в одной комнате. А у вас с Лидией будут отдельные комнаты."

# game/code/e06/e06s04.rpy:216
translate russian e06s04_03850b18:

    # arj "How did you swing a room? Did [mcname] decide that?"
    arj "Как вы распределяли комнаты? Это решал [mcname]?"

# game/code/e06/e06s04.rpy:219
translate russian e06s04_36586c76:

    # mes "Nope. I called dibs when I moved in."
    mes "Нет. Я заявила о своем выборе, когда переехала."

# game/code/e06/e06s04.rpy:222
translate russian e06s04_b02fe471:

    # arj "Well, that's not really fair, is it?"
    arj "Ну, это не совсем справедливо, не так ли?"

# game/code/e06/e06s04.rpy:225
translate russian e06s04_4a8a9e5a:

    # mes "Fairest thing in the world. Everyone knows international dibs rules."
    mes "Самая справедливая вещь в мире. Все знают правила международного бабла."

# game/code/e06/e06s04.rpy:228
translate russian e06s04_93e79b50:

    # mes "I guess in this case, it was more a perk of being Lydia's best friend. She wanted to come to me first after she got out of jail."
    mes "Думаю, в данном случае это было скорее преимущество того, что я лучшая подруга Лидии. Она хотела прийти ко мне первой после выхода из тюрьмы."

# game/code/e06/e06s04.rpy:231
translate russian e06s04_c95a234f:

    # arj "I see."
    arj "Понимаю."

# game/code/e06/e06s04.rpy:234
translate russian e06s04_1ae8762e:

    # mes "But it's not all bad, I'm sure most nights we will all fall asleep curled up around [mcname] in whatever room the fun starts in."
    mes "Но всё не так уж плохо, я уверена, что большинство ночей мы все будем засыпать, свернувшись калачиком вокруг [mcname] в любой комнате, где начинается веселье."

# game/code/e06/e06s04.rpy:237
translate russian e06s04_2af47fcc:

    # arj "Mmm. That doesn't sound bad."
    arj "Ммм. Звучит неплохо."

# game/code/e06/e06s04.rpy:245
translate russian e06s04_742f908e:

    # mes "You know... I brought all my bondage gear with me."
    mes "Знаешь... Я привезла с собой все свое снаряжение для бондажа."

# game/code/e06/e06s04.rpy:248
translate russian e06s04_f66cf2a6:

    # mes "If Stacy ever gives you trouble, we can tie her up in the basement."
    mes "Если Стейси когда-нибудь доставит тебе неприятности, мы можем связать её в подвале."

# game/code/e06/e06s04.rpy:251
translate russian e06s04_3556e90e:

    # arj "Hehehe."
    arj "Хе-хе."

# game/code/e06/e06s04.rpy:261
translate russian e06s04_drinks_4fad0f69:

    # lc "Now the welcoming party is in full swing."
    lc "Приветственная вечеринка в самом разгаре."

# game/code/e06/e06s04.rpy:269
translate russian e06s04_drinks_9570dfc5:

    # lc "Hello Mr. Temptation."
    lc "Привет, мистер Искуситель."

# game/code/e06/e06s04.rpy:271
translate russian e06s04_drinks_58863c60:

    # lc "Honey... it is getting harder and harder for me to wait till we're married."
    lc "Дорогой... Мне все сложнее и сложнее ждать, пока мы поженимся."

# game/code/e06/e06s04.rpy:274
translate russian e06s04_drinks_da8738d4:

    # mc "I know, sweetie. Don't worry, with everyone fully moved in, I'm sure I'll be kept busy."
    mc "Я знаю, милая. Не волнуйся, когда все переедут, я уверен, что буду занят."

# game/code/e06/e06s04.rpy:277
translate russian e06s04_drinks_9d182b73:

    # lc "Mmhmm. Thank god I'll at least be able to have some fun of my own watching you and the others until the big day."
    lc "Мммм... Слава богу, я хотя бы смогу немного развлечься, наблюдая за тобой и остальными до самого знаменательного дня."

# game/code/e06/e06s04.rpy:284
translate russian e06s04_drinks_176560fe:

    # arj "Thank you, Lydia."
    arj "Спасибо, Лидия."

# game/code/e06/e06s04.rpy:287
translate russian e06s04_drinks_871bd10a:

    # sy "Wow. That's actually not bad at all, Lydia. You could give Iona a run for her money."
    sy "Ух ты. На самом деле это совсем не плохо, Лидия. Ты можешь дать Ионе фору."

# game/code/e06/e06s04.rpy:290
translate russian e06s04_drinks_e79a2d54:

    # lc "Growing up, I was my parents' darling little helper."
    lc "В детстве я была любимой маленькой помощницей своих родителей."

# game/code/e06/e06s04.rpy:292
translate russian e06s04_drinks_49d7f679:

    # lc "But now I get to help those I chose."
    lc "Но теперь я могу помочь тем, кого сама выбрала."

# game/code/e06/e06s04.rpy:295
translate russian e06s04_drinks_ea4ff881:

    # mc "Cheers to that."
    mc "Выпьем за это."

# game/code/e06/e06s04.rpy:320
translate russian e06s04_later_908a5de0:

    # mes "We will have to remember this day as the official start of your harem, [mcname]."
    mes "Нам придётся запомнить этот день как официальное открытие твоего гарема, [mcname]."

# game/code/e06/e06s04.rpy:323
translate russian e06s04_later_facdc3eb:

    # mc "You're right. Shit. I should have prepared something to honor the day."
    mc "Ты права. Черт. Я должен был приготовить что-нибудь в честь этого дня."

# game/code/e06/e06s04.rpy:326
translate russian e06s04_later_b940ce0c:

    # arj "It's alright, [mcname]. You've been busy with the wedding."
    arj "Все в порядке, [mcname]. Ты был занят свадьбой."

# game/code/e06/e06s04.rpy:329
translate russian e06s04_later_0c168524:

    # sy "I gotta ask you something, Lydia."
    sy "Лидия, я хочу спросить тебя кое о чем."

# game/code/e06/e06s04.rpy:332
translate russian e06s04_later_ad6d5f24:

    # lc "Of course, Stacy."
    lc "Конечно, Стейси."

# game/code/e06/e06s04.rpy:335
translate russian e06s04_later_13c84637:

    # sy "You still have no jitters about all this? Having us all live here, and each of us getting our brains fucked to oblivion by the lucky stallion here."
    sy "Ты все еще не переживаешь из-за всего этого? Мы все будем жить здесь, и каждая из нас будет трахаться до беспамятства с этим счастливым жеребцом."

# game/code/e06/e06s04.rpy:339
translate russian e06s04_later_3f45d272:

    # arj "I was also a little curious. Once upon a time, I had to get to the point of accepting I would have to share him."
    arj "Мне также было немного любопытно. Однажды мне пришлось смириться с тем, что мне придётся его делить."

# game/code/e06/e06s04.rpy:341
translate russian e06s04_later_6dce681b:

    # arj "Meeting Stacy helped. But you and I are very different."
    arj "Встреча со Стейси помогла. Но мы с тобой очень разные."

# game/code/e06/e06s04.rpy:345
translate russian e06s04_later_80b52b85:

    # lc "Of course, I'm alright with it. I love [mcname] with all my heart, and nothing gets me more excited than seeing him fuck other girls."
    lc "Конечно, я не против. Я люблю [mcname] всем сердцем, и ничто не возбуждает меня больше, чем видеть, как он трахает других девушек."

# game/code/e06/e06s04.rpy:348
translate russian e06s04_later_da4a14a6:

    # sy "But you still want him to fuck you one day, right?"
    sy "Но ты всё равно хочешь, чтобы однажды он трахнул тебя, верно?"

# game/code/e06/e06s04.rpy:351
translate russian e06s04_later_2c1fea64:

    # lc "One day, I want him to do everything he's been wanting to do to me since we met."
    lc "Однажды я захочу, чтобы он сделал со мной всё, что хотел сделать с момента нашего знакомства."

# game/code/e06/e06s04.rpy:352
translate russian e06s04_later_7f6f836d:

    # lc "And of course, I'll probably want him to do everything he's done to all of you as well."
    lc "И, конечно, я, вероятно, захочу, чтобы он сделал всё то же самое, что он делал со всеми вами."

# game/code/e06/e06s04.rpy:356
translate russian e06s04_later_161ab4ba:

    # mc "Yoza!"
    mc "Yoza!"

# game/code/e06/e06s04.rpy:359
translate russian e06s04_later_851a97b8:

    # mes "With that mindset, I wonder which one of us will have the first bun in the oven."
    mes "С таким настроем мне интересно, кто из нас будет первой булочкой в духовке."

# game/code/e06/e06s04.rpy:362
translate russian e06s04_later_e03aa1aa:

    # sy "I want to be first. That is definitely at the top on my bucket list."
    sy "Я хочу быть первой. Это определённо занимает первое место в моём списке желаний."

# game/code/e06/e06s04.rpy:365
translate russian e06s04_later_f3fb6e46:

    # lc "Hehehe."
    lc "Хе-хе."

# game/code/e06/e06s04.rpy:368
translate russian e06s04_later_833c996a:

    # arj "Hahaha."
    arj "Хахаха."

# game/code/e06/e06s04.rpy:372
translate russian e06s04_later_4fa467ad:

    # arj "All done."
    arj "Все готово."

# game/code/e06/e06s04.rpy:375
translate russian e06s04_later_5baec176:

    # lc "It will happen at the right moment."
    lc "Это произойдет в нужный момент."

# game/code/e06/e06s04.rpy:377
translate russian e06s04_later_9de78716:

    # lc "But each day before the wedding is going to feel like torture."
    lc "Но каждый день до свадьбы будет казаться пыткой."

# game/code/e06/e06s04.rpy:380
translate russian e06s04_later_5de891c1:

    # lc "I cannot wait to give you my virginity, [mcname]."
    lc "Мне не терпится отдать тебе свою девственность, [mcname]."

# game/code/e06/e06s04.rpy:383
translate russian e06s04_later_2b94a2bb:

    # mc "It's going to be an amazing moment, my love."
    mc "Это будет восхитительный момент, любовь моя."

# game/code/e06/e06s04.rpy:384
translate russian e06s04_later_ad97ad96:

    # mc "I'll take you for the first time, and then shower you with pleasure while I fuck everyone else here."
    mc "Я возьму тебя в первый раз, а потом буду доставлять тебе удовольствие, трахая всех остальных здесь."

# game/code/e06/e06s04.rpy:387
translate russian e06s04_later_df62864c:

    # arj "I'd love to help out with the wedding."
    arj "Я бы с удовольствием помогла со свадьбой."

# game/code/e06/e06s04.rpy:390
translate russian e06s04_later_54d5500a:

    # lc "Really?"
    lc "Правда?"

# game/code/e06/e06s04.rpy:393
translate russian e06s04_later_afd6f833:

    # arj "Of course. It will be like helping out my sister, I'd love to, Lydia."
    arj "Конечно. Это будет как помощь моей сестре, с удовольствием, Лидия."

# game/code/e06/e06s04.rpy:396
translate russian e06s04_later_0389a1ef:

    # mc "There are a few things to work out."
    mc "Есть несколько моментов, которые необходимо решить."

# game/code/e06/e06s04.rpy:399
translate russian e06s04_later_7069d532:

    # sy "Yeah. Who is running this shebang anyway?"
    sy "Да. Кто управляет этим балаганом?"

# game/code/e06/e06s04.rpy:402
translate russian e06s04_later_e756f4b3:

    # lc "I think the most important missing piece is an officiant. It has to be someone we trust."
    lc "Думаю, самый важный недостающий элемент - это священник. Это должен быть человек, которому мы доверяем."

# game/code/e06/e06s04.rpy:405
translate russian e06s04_later_ed049c8e:

    # mes "And someone who won't freak out knowing that it's a wedding that includes an existing polyamorous harem."
    mes "И кто-то, кто не сойдет с ума, узнав, что это свадьба, которая включает в себя существующий полиамурный гарем."

# game/code/e06/e06s04.rpy:408
translate russian e06s04_later_28b7b2fc:

    # lc "Exactly. There are some libertine people around town, but not many are church folks."
    lc "Именно. В городе есть несколько распутников, но мало кто из них служит в церкви."

# game/code/e06/e06s04.rpy:411
translate russian e06s04_later_e526a903:

    # mc "Hmmm."
    mc "Хмммм."

# game/code/e06/e06s04.rpy:414
translate russian e06s04_later_3b1793de:

    # sy "What about Lyssa?"
    sy "А как же Лисса?"

# game/code/e06/e06s04.rpy:417
translate russian e06s04_later_4af7a871:

    # mc "There is a thought."
    mc "Есть такая мысль."

# game/code/e06/e06s04.rpy:420
translate russian e06s04_later_31e6984c:

    # sy "She could totally do it. She's a lawyer she knows the legal side of things, and she told me once that she's a certified minister."
    sy "Она точно может это сделать. Она юрист, она знает юридическую сторону вещей, и она сказала мне однажды, что она дипломированный священник."

# game/code/e06/e06s04.rpy:421
translate russian e06s04_later_95c98222:

    # sy "I think she even officiated a wedding for some of her friends."
    sy "Думаю, она даже проводила свадьбу для некоторых своих друзей."

# game/code/e06/e06s04.rpy:424
translate russian e06s04_later_8e7a88ff:

    # mc "Is that alright with you, my darling?"
    mc "Ты не против, дорогая?"

# game/code/e06/e06s04.rpy:427
translate russian e06s04_later_2061a105:

    # lc "As long as we get to say our own vows, Lyssa sounds perfect for the job."
    lc "Пока мы можем произносить свои собственные клятвы, Лисса идеально подходит для этой работы."

# game/code/e06/e06s04.rpy:430
translate russian e06s04_later_18e9117a:

    # mc "Great, I'll call her later today."
    mc "Отлично, я позвоню ей сегодня."

# game/code/e06/e06s04.rpy:433
translate russian e06s04_later_a4b09457:

    # mes "I can lend a hand as well. I know a delightful place close by that makes amazing cakes."
    mes "Я тоже могу помочь. Я знаю одно восхитительное место неподалеку, где пекут потрясающие торты."

# game/code/e06/e06s04.rpy:435
translate russian e06s04_later_58019f45:

    # mes "And I think I remember some of the details you always wanted for your big day, Lydia."
    mes "И мне кажется, я помню некоторые детали, которые ты всегда хотела для своего знаменательного дня, Лидия."

# game/code/e06/e06s04.rpy:440
translate russian e06s04_later_d6fd6991:

    # lc "You're the best!"
    lc "Ты лучшая!"

# game/code/e06/e06s04.rpy:443
translate russian e06s04_later_03cf1a1f:

    # arj "Well, if the cake is taken, I'm sure I can handle decorations. Where are you two planning to have the event?"
    arj "Ну, если торт уже занят, я уверена, что справлюсь с украшениями. Где вы двое планируете провести мероприятие?"

# game/code/e06/e06s04.rpy:446
translate russian e06s04_later_a851a215:

    # mc "Oh shoot. I didn't find a venue yet."
    mc "О, черт. Я еще не нашел место проведения."

# game/code/e06/e06s04.rpy:449
translate russian e06s04_later_14f64169:

    # lc "We don't need one. We can have it right here."
    lc "Не нужно. Мы можем провести его прямо здесь."

# game/code/e06/e06s04.rpy:452
translate russian e06s04_later_b917e772:

    # mc "You're serious, aren't you?"
    mc "Ты серьезно, не так ли?"

# game/code/e06/e06s04.rpy:455
translate russian e06s04_later_3c351097:

    # lc "We don't need a big spot or some fancy place with a fancy name."
    lc "Нам не нужно большое место или какое-то место с модным названием."

# game/code/e06/e06s04.rpy:458
translate russian e06s04_later_318460bb:

    # lc "We just need a place where the whole harem can witness our wonderful day."
    lc "Нам просто необходимо место, где весь гарем сможет стать свидетелем нашего чудесного дня."

# game/code/e06/e06s04.rpy:459
translate russian e06s04_later_b8c03505:

    # lc "Right?"
    lc "Правильно?"

# game/code/e06/e06s04.rpy:462
translate russian e06s04_later_3c0bbbee:

    # mc "Absolutely!"
    mc "Абсолютно!"

# game/code/e06/e06s04.rpy:470
translate russian e06s04_later_f3fd8ec8:

    # mes "Great, looks like we've got things all settled. I'll create a binder so we don't lose track of anything."
    mes "Отлично, похоже, мы все уладили. Я создам список, чтобы мы ничего не забыли."

# game/code/e06/e06s04.rpy:473
translate russian e06s04_later_6c5cdb32:

    # mc "If you change your mind about having the wedding here, or you want anything else, just tell me."
    mc "Если передумаешь устраивать свадьбу здесь или захочешь чего-то ещё, просто скажи мне."

# game/code/e06/e06s04.rpy:476
translate russian e06s04_later_9963463c:

    # lc "All I need is you, [mcname]. Everything else will fall into place, I know it will. I love you, darling."
    lc "Всё, что мне нужно, это ты, [mcname]. Остальное всё встанет на свои места, я знаю. Я люблю тебя, дорогой."

# game/code/e06/e06s04.rpy:480
translate russian e06s04_later_31152f7e:

    # mc "I love you, sweetheart."
    mc "Я люблю тебя, милая."

# game/code/e06/e06s04.rpy:484
translate russian e06s04_later_a910dc2d:

    # sy "I can't believe it, the wedding is coming up very shortly and we haven't planned out a bachelor party for [mcname]!"
    sy "Не могу поверить, свадьба уже совсем скоро, а мы до сих пор не спланировали мальчишник для [mcname]!"

# game/code/e06/e06s04.rpy:487
translate russian e06s04_later_1be08199:

    # mes "Isn't every day of his life a bachelor party?"
    mes "Разве не каждый день его жизни - мальчишник?"

# game/code/e06/e06s04.rpy:490
translate russian e06s04_later_cf0a62d7:

    # sy "Traditions exist for a reason. Just because [mcname] has all of us doesn't mean we should skip an important moment like this."
    sy "Традиции существуют не просто так. То, что у [mcname] есть все мы, не означает, что мы должны пропустить такой важный момент, как этот."

# game/code/e06/e06s04.rpy:493
translate russian e06s04_later_6bea0edd:

    # arj "I'm sure he wouldn't say no to a special surprise that doesn't include a cock cage."
    arj "Уверена, что он не откажется от особого сюрприза, который не включает в себя клетку для члена."

# game/code/e06/e06s04.rpy:496
translate russian e06s04_later_8257c83d:

    # mes "Sounds like you're volunteering to take over this part of the planning Stacy."
    mes "Похоже, ты решила взять эту часть на себя, Стейси."

# game/code/e06/e06s04.rpy:499
translate russian e06s04_later_e00e4fee:

    # sy "Totally. I can put something together. No problem."
    sy "Полностью. Я могу что-нибудь придумать. Без проблем."

# game/code/e06/e06s04.rpy:502
translate russian e06s04_later_28c66834:

    # arj "You don't have to do it alone. In fact, I know a girl that I think would be perfect for the main event. She's always the best and makes a party really shine."
    arj "Тебе не обязательно делать это одной. На самом деле, я знаю девушку, которая, как мне кажется, идеально подойдет для главного события. Она всегда самая лучшая и делает вечеринку по-настоящему яркой."

# game/code/e06/e06s04.rpy:505
translate russian e06s04_later_35abc2db:

    # sy "Thanks Amrose."
    sy "Спасибо, АмРоуз."

