# TODO: Translation updated at 2023-05-27 13:27

# game/code/e06/e06s01.rpy:95
translate russian e06s01_0143d3aa:

    # lc "What's wrong?"
    lc "Что-то не так?"

# game/code/e06/e06s01.rpy:98
translate russian e06s01_6e0cbc41:

    # mc "Nothing?"
    mc "Ничего?"

# game/code/e06/e06s01.rpy:102
translate russian e06s01_9afdaa4c:

    # lc "Come on. You can tell me."
    lc "Да ладно. Ты же можешь мне сказать."

# game/code/e06/e06s01.rpy:105
translate russian e06s01_9f7bb4fa:

    # mc "Just a little jittery, I guess."
    mc "Думаю, просто немного нервничаю."

# game/code/e06/e06s01.rpy:108
translate russian e06s01_2ad5c857:

    # mc "I'm worried this is a dream."
    mc "Я боюсь, что это сон."

# game/code/e06/e06s01.rpy:109
translate russian e06s01_055c1548:

    # mc "Or worse, it's about to turn into a nightmare, and not the fun Tim Burton kind..."
    mc "Или, что еще хуже, это превратится в кошмар, и не такой веселый, как у Тима Бертона..."

# game/code/e06/e06s01.rpy:112
translate russian e06s01_adf3e322:

    # lc "This is real. I'm here with you."
    lc "Это реальность. Я здесь, с тобой."

# game/code/e06/e06s01.rpy:113
translate russian e06s01_44306627:

    # lc "I'm here {i}because{/i} of you, [mcname]."
    lc "Я здесь {i}благодаря{/i} тебе, [mcname]."

# game/code/e06/e06s01.rpy:114
translate russian e06s01_cf10dbe1:

    # lc "You're the only one who supported me. Who stood by me."
    lc "Ты единственный, кто поддержал меня. Кто остался рядом со мной."

# game/code/e06/e06s01.rpy:117
translate russian e06s01_e3fb20b7:

    # lc "I saw the look in that Judge's eyes each time she looked at me."
    lc "Каждый раз, когда она смотрела на меня, я чувствовала ее взгляд."

# game/code/e06/e06s01.rpy:118
translate russian e06s01_dae00533:

    # lc "I don't think she ever believed I could be innocent. Not even at the end..."
    lc "Не думаю, что она верила в мою невиновность. Даже в конце..."

# game/code/e06/e06s01.rpy:120
translate russian e06s01_9527a6d3:

    # lc "Boy. I... I really need to take a shower."
    lc "Малыш. Я... Мне действительно нужно принять душ."

# game/code/e06/e06s01.rpy:123
translate russian e06s01_37ddab42:

    # mc "Oh. Feisty already?"
    mc "Оу. Уже взмокла?"

# game/code/e06/e06s01.rpy:126
translate russian e06s01_fef40d9b:

    # lc "Haha. No... nothing like that. After being in that cell and then wearing these clothes again, I feel utterly disgusting."
    lc "Хаха. Нет... ничего такого. После пребывания в той камере, а теперь вновь надев эту одежду, я чувствую себя совершенно отвратительно."

# game/code/e06/e06s01.rpy:128
translate russian e06s01_16d03227:

    # lc "Make yourself at home, I'll be down shortly."
    lc "Чувствуй себя как дома, я скоро вернусь."

# game/code/e06/e06s01.rpy:131
translate russian e06s01_c9e5d250:

    # lc "Oh, can you charge my phone please?"
    lc "О, можешь поставить на зарядку мой телефон?"

# game/code/e06/e06s01.rpy:132
translate russian e06s01_c66f8e1d:

    # lc "Battery must have died while I was on the inside."
    lc "Батарея, должно быть, села, пока я сидела там."

# game/code/e06/e06s01.rpy:135
translate russian e06s01_ae072ba4:

    # mc "I will be right here."
    mc "Я буду здесь."

# game/code/e06/e06s01.rpy:154
translate russian e06s01_3ebd63f1:

    # mc "Wow. You look amazing."
    mc "Вау. Выглядишь потрясающе."

# game/code/e06/e06s01.rpy:157
translate russian e06s01_d3ad0144:

    # lc "Such a flatterer. I just threw this on."
    lc "Какой льстец. Я просто надела это."

# game/code/e06/e06s01.rpy:158
translate russian e06s01_7d8d1212:

    # lc "Didn't want you regretting anything and leaving."
    lc "Не хочу, чтобы ты пожалел о случившемся и решил расстаться."

# game/code/e06/e06s01.rpy:161
translate russian e06s01_7eaac46d:

    # mc "That would never happen. I love you, and I'll always stand by you, Lydia."
    mc "Этому не бывать. Я люблю тебя, и всегда буду с тобой, Лидия."

# game/code/e06/e06s01.rpy:164
translate russian e06s01_c0030af2:

    # lc "You have no idea how happy that makes me."
    lc "Ты даже не представляешь, как счастлива я сейчас."

# game/code/e06/e06s01.rpy:167
translate russian e06s01_c9920062:

    # lc "Having you hold me like this. I feel safe... Like I can breathe again."
    lc "Когда ты прижимаешь меня к себе вот так. Я чувствую себя в безопасности... Словно я снова могу дышать."

# game/code/e06/e06s01.rpy:170
translate russian e06s01_f3b3d28f:

    # mc "What's wrong?"
    mc "Что-то не так?"

# game/code/e06/e06s01.rpy:173
translate russian e06s01_d4465dcc:

    # lc "I just keep playing things over in my mind."
    lc "Просто я всё прокручиваю в голове."

# game/code/e06/e06s01.rpy:174
translate russian e06s01_812193e1:

    # lc "There is no Fetish Locator without me telling Jerome about my secret."
    lc "Если бы я не рассказала Джерому о своем секрете, не было бы никакого Фетиш Локатора."

# game/code/e06/e06s01.rpy:177
translate russian e06s01_200dcac1:

    # mc "You can't think like that. You didn't control the app. And once it was out, you couldn't control its spread."
    mc "Тебе не стоит так переживать. Ты не контролировала приложение. И когда оно вышло, ты не могла контролировать его распространение."

# game/code/e06/e06s01.rpy:178
translate russian e06s01_cdfb4896:

    # mc "It's like trying to catch a snowball after it's been tumbling downhill for ten minutes."
    mc "Это все равно что пытаться поймать снежный ком после того, как он десять минут катился вниз по склону."

# game/code/e06/e06s01.rpy:181
translate russian e06s01_d0468e36:

    # lc "I could have caught it. Maybe if I came clean to you... Instead of-"
    lc "Я могла бы поймать его. Если бы я призналась тебе... Вместо того, чтобы..."

# game/code/e06/e06s01.rpy:184
translate russian e06s01_d28de22e:

    # mc "We can't go back, Lydia."
    mc "Мы не можем вернуть прошлое, Лидия."

# game/code/e06/e06s01.rpy:196
translate russian e06s01_3006cfa2:

    # lc "I guess my phone is alive again."
    lc "Похоже, мой телефон ожил."

# game/code/e06/e06s01.rpy:200
translate russian e06s01_310ee3d8:

    # mct "And that would be mine."
    mct "Это мой."

# game/code/e06/e06s01.rpy:203
translate russian e06s01_d11cf752:

    # mc "Good news?"
    mc "Хорошие новости?"

# game/code/e06/e06s01.rpy:206
translate russian e06s01_4b7bca35:

    # lc "Not so much..."
    lc "Не совсем..."

# game/code/e06/e06s01.rpy:209
translate russian e06s01_efcaf011:

    # mc "Let's see. Messages from AmRose and Stacy. Not too surprising. One really angry rant from Hana."
    mc "Посмотрим. Сообщения от АмРоуз и Стейси. Не особо удивительно. Одно действительно гневное сообщение от Ханы."

# game/code/e06/e06s01.rpy:212
translate russian e06s01_4b1825ec:

    # mc "Who needs them, Lydia?"
    mc "Кому это нужно, Лидия?"

# game/code/e06/e06s01.rpy:215
translate russian e06s01_6be2eaf4:

    # lc "I need... I'm going to need your help with something, [mcname]..."
    lc "Мне нужно... Мне нужна твоя помощь, [mcname]..."

# game/code/e06/e06s01.rpy:218
translate russian e06s01_e59abd2a:

    # mc "Anything."
    mc "Все, что угодно."

# game/code/e06/e06s01.rpy:221
translate russian e06s01_8a792ce7:

    # lc "T-there is just so much... anger. It wasn't like this during the case."
    lc "В... в них так много... злобы. Во время процесса все было иначе."

# game/code/e06/e06s01.rpy:224
translate russian e06s01_96af9ba3:

    # mc "I'm sure a lot of people expected you to get locked up."
    mc "Думаю, многие ожидали, что тебя посадят."

# game/code/e06/e06s01.rpy:225
translate russian e06s01_cd4dafd5:

    # mc "Everything changed when the verdict came out."
    mc "Все изменилось после оглашения приговора."

# game/code/e06/e06s01.rpy:228
translate russian e06s01_05b605d5:

    # lc "I don't want it to change."
    lc "Я не хочу, чтобы все изменилось."

# game/code/e06/e06s01.rpy:231
translate russian e06s01_433c9d2c:

    # mc "I know. You didn't want this to happen."
    mc "Я знаю. Ты не хотела, чтобы это случилось."

# game/code/e06/e06s01.rpy:232
translate russian e06s01_9e7c3fbe:

    # mc "But the elephant in the room is that you're the one who caged my elephant."
    mc "Но слон в комнате оказался потому, что именно ты загнала моего \"слоника\" в клетку."

# game/code/e06/e06s01.rpy:235
translate russian e06s01_9be5f820:

    # lc "That wasn't me, that was Ridley. You know, the A.I."
    lc "Это была не я, это был Ридли. Ну, знаешь, И.И."

# game/code/e06/e06s01.rpy:239
translate russian e06s01_17ec13ae:

    # mc "I know that and you know that. But think of all the people who won't care about the difference."
    mc "Я это знаю, и ты это знаешь. Но подумай обо всех тех, кого не будет волновать разница."

# game/code/e06/e06s01.rpy:240
translate russian e06s01_6ddd330f:

    # mc "Besides... I think you enjoyed having my cock locked up in your own way. You can admit it."
    mc "Кроме того... Думаю, тебе понравилось, когда мой член был заперт по твоему желанию. Ты можешь признать это."

# game/code/e06/e06s01.rpy:243
translate russian e06s01_944d121e:

    # lc "Yes... I liked it. Being assured that no matter what, I couldn't give in."
    lc "Да... Мне понравилось. Была уверенность, что несмотря ни на что, я не смогу поддаться."

# game/code/e06/e06s01.rpy:244
translate russian e06s01_22a1523a:

    # lc "I was safe then. Now... Now it's out of control. I wish I never met Jerome!"
    lc "Тогда я была в безопасности. А теперь... Теперь всё вышло из-под контроля. Лучше бы я никогда не встречала Джерома!"

# game/code/e06/e06s01.rpy:247
translate russian e06s01_853f7d9e:

    # mc "We were both looking for something special. And we found it, thanks to you."
    mc "Мы оба искали что-то особенное. И мы нашли это, благодаря тебе."

# game/code/e06/e06s01.rpy:248
translate russian e06s01_cc7a532e:

    # mc "The journey had some bumps, but it was a trip I'd take all over again, just to get here."
    mc "Путешествие было нелегким, но я готов повторить его вновь, лишь бы оказаться здесь."

# game/code/e06/e06s01.rpy:251
translate russian e06s01_efa636e5:

    # lc "You're the best, [mcname]."
    lc "Ты лучший, [mcname]."

# game/code/e06/e06s01.rpy:259
translate russian e06s01_4486313b:

    # lc "Speaking of bumps, I... remember what I wanted to ask you, [mcname]."
    lc "Говоря о препятствиях, я... вспомнила, о чем хотела спросить тебя, [mcname]."

# game/code/e06/e06s01.rpy:262
translate russian e06s01_b873b9d9:

    # lc "It's Min. I can deal with everyone else calling me a liar and... worse..."
    lc "Дело в Мин. Я могу справиться с остальными, кто называет меня лжецом или... хуже..."

# game/code/e06/e06s01.rpy:263
translate russian e06s01_72f4137a:

    # lc "But not her. Not my best friend."
    lc "Но не она. Не моя лучшая подруга."

# game/code/e06/e06s01.rpy:266
translate russian e06s01_299f6032:

    # mc "Well... you might have to accept that forgiveness will take some time."
    mc "Тогда... тебе придется признать, что прощение может занять некоторое время."

# game/code/e06/e06s01.rpy:269
translate russian e06s01_2daa9175:

    # lc "I do. I swear I do."
    lc "Да. Это точно."

# game/code/e06/e06s01.rpy:270
translate russian e06s01_3c92b8d1:

    # lc " But... I thought, if you came with me to meet her. Maybe she will hear me out."
    lc " Но... Думаю, если ты пойдешь со мной на встречу с ней. Может, она все-таки выслушает меня."

# game/code/e06/e06s01.rpy:273
translate russian e06s01_d883f10f:

    # mc "Haha. You don't want to lock her up in a cage to make her listen to your side?"
    mc "Хаха. Ты же не хочешь запереть ее в камере, чтобы заставить выслушать тебя?"

# game/code/e06/e06s01.rpy:276
translate russian e06s01_1a5f1b10:

    # lc "Nope. And hey..."
    lc "Неа. И эй..."

# game/code/e06/e06s01.rpy:277
translate russian e06s01_f7cce481:

    # lc "Total forgiveness means forgiving me for keeping you that cell while I was talking."
    lc "Полное прощение подразумевает простить меня за то, что я держала тебя в камере, пока я говорила."

# game/code/e06/e06s01.rpy:280
translate russian e06s01_fa0bce90:

    # mc "Of course. Just don't be surprised if I'm more animal than man when we finally make love."
    mc "Конечно. Только не удивляйся, если во время нашей близости я буду похож на зверя, чем на человека."

# game/code/e06/e06s01.rpy:283
translate russian e06s01_22085584:

    # lc "I would never hold that against you. In fact... I'm a little excited just to see what you're capable of..."
    lc "Я не стану сдерживать тебя. На самом деле... Я даже немного возбуждена, ожидая узнать, на что ты способен..."

# game/code/e06/e06s01.rpy:284
translate russian e06s01_d45dc55a:

    # lc "When you're... unrestrained..."
    lc "Когда... неограничен..."

# game/code/e06/e06s01.rpy:295
translate russian e06s01_b3614f1d:

    # lc "Good night, my love. I know I'm making this wait unbearable for you. I just..."
    lc "Приятных снов, любовь моя. Я знаю, что делаю это ожидание невыносимым для тебя. Просто..."

# game/code/e06/e06s01.rpy:298
translate russian e06s01_2f52290d:

    # mc "You don't have to apologize to me, Lydia."
    mc "Тебе не нужно извиняться передо мной, Лидия."

# game/code/e06/e06s01.rpy:299
translate russian e06s01_9a38e384:

    # mc "Good things come to those who wait."
    mc "Хорошее приходит к тем, кто умеет ждать."

# game/code/e06/e06s01.rpy:302
translate russian e06s01_e62ba5ea:

    # lc "I promise you won't have to wait much longer..."
    lc "Обещаю, тебе не придется долго ждать..."

# game/code/e06/e06s01.rpy:310
translate russian e06s01_328b4b2e:

    # mc "Good night, Lydia..."
    mc "Приятных снов, Лидия..."

translate russian strings:

    # game/code/e06/e06s01.rpy:12
    old "Choose optional characters for this ending"
    new "Выберите дополнительных персонажей для этой концовки"

    # game/code/e06/e06s01.rpy:47
    old "Continue"
    new "Продолжить"

    # game/code/e06/e06s01.rpy:77
    old "Ending #6\nQueen's Harem"
    new "Концовка #6\nКоролевский гарем"

    # game/code/e06/e06s01.rpy:82
    old "Later That Day"
    new "Позже в тот же день"

