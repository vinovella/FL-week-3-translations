# game/code/e06/e06s02.rpy:53
translate russian e06s02_87f8ba93:

    # mc "It's going to be fine."
    mc "Всё будет хорошо."

# game/code/e06/e06s02.rpy:58
translate russian e06s02_04c67d06:

    # mct "I hope."
    mct "Надеюсь."

# game/code/e06/e06s02.rpy:60
translate russian e06s02_993c9720:

    # mct "I really hope Min isn't calling security on us."
    mct "Я очень надеюсь, что Мин не вызовет на нас охрану."

# game/code/e06/e06s02.rpy:61
translate russian e06s02_f4a250c1:

    # mct "I don't want to end up in a cell again anytime soon."
    mct "Не хотелось бы оказаться в камере так скоро."

# game/code/e06/e06s02.rpy:64
translate russian e06s02_40511e6c:

    # "*knock knock*"
    "*тук-тук*"

# game/code/e06/e06s02.rpy:76
translate russian e06s02_8af2819e:

    # mes "Son of a bitch."
    mes "Твою мать."

# game/code/e06/e06s02.rpy:81
translate russian e06s02_73457511:

    # lc "Please come out..."
    lc "Пожалуйста, выйди..."

# game/code/e06/e06s02.rpy:88
translate russian e06s02_e2d63846:

    # lc "I'm not leaving. I know you're in there because we're still friends on FriendFinder, Min."
    lc "Я не уйду. Я знаю, что ты там, потому что мы все еще друзья на FriendFinder, Мин."

# game/code/e06/e06s02.rpy:99
translate russian e06s02_5255b884:

    # mes "Shit!"
    mes "Вот дерьмо!"

# game/code/e06/e06s02.rpy:102
translate russian e06s02_cf1acafb:

    # mes "Go away Lydia. I do not want to talk to you."
    mes "Уходи, Лидия. Мне не о чем с тобой говорить."

# game/code/e06/e06s02.rpy:106
translate russian e06s02_bd477cb8:

    # mes "So you can just stop knocking on my door."
    mes "Так что можешь просто прекратить ломиться в дверь."

# game/code/e06/e06s02.rpy:112
translate russian e06s02_d540dfe2:

    # mc "What's next?"
    mc "Что дальше?"

# game/code/e06/e06s02.rpy:115
translate russian e06s02_64fc8122:

    # lc "This isn't going to work..."
    lc "Это не сработало..."

# game/code/e06/e06s02.rpy:118
translate russian e06s02_bb114a5b:

    # mc "Huh? You can't give up hope, Lydia."
    mc "А? Не теряй веру, Лидия."

# game/code/e06/e06s02.rpy:121
translate russian e06s02_f526fd43:

    # lc "I can't even get my best friend to see that I know I made a huge mistake."
    lc "Я не могу даже лучшую подругу убедить в том, что я знаю, что совершила огромную ошибку."

# game/code/e06/e06s02.rpy:122
translate russian e06s02_805f069e:

    # lc "How will I ever convince the world to see me differently? They'll treat me like a criminal, forever."
    lc "Как я смогу убедить других людей посмотреть на меня по-другому? Они всегда будут относиться ко мне как к преступнице."

# game/code/e06/e06s02.rpy:125
translate russian e06s02_857175cd:

    # mc "No. Maybe. I honestly don't know. Sorry Lydia."
    mc "Нет. Может. Честно, я не знаю. Прости, Лидия."

# game/code/e06/e06s02.rpy:128
translate russian e06s02_d590902e:

    # lc "You have nothing to be sorry about."
    lc "Тебе не за что извиняться."

# game/code/e06/e06s02.rpy:129
translate russian e06s02_09ad4618:

    # lc "I started all this. And I..."
    lc "Я всё это заварила. И я..."

# game/code/e06/e06s02.rpy:130
translate russian e06s02_943d676a:

    # lc "I knew what kind of man Jerome was."
    lc "Я знала, что Джером представлял из себя."

# game/code/e06/e06s02.rpy:133
translate russian e06s02_aa451f48:

    # mc "I forgot that beyond having a wonderful voice you have the gift of foresight."
    mc "Я забыл, что помимо прекрасного голоса у тебя есть дар предвидения."

# game/code/e06/e06s02.rpy:135
translate russian e06s02_8882cc1f:

    # lc "..."
    lc "..."

# game/code/e06/e06s02.rpy:138
translate russian e06s02_738698a0:

    # lc "Maybe I should tell Min about the fund."
    lc "Может, мне стоит рассказать Мин о фонде?"

# game/code/e06/e06s02.rpy:141
translate russian e06s02_4769d6f1:

    # mc "That's up to you, Lydia."
    mc "Это тебе решать, Лидия."

# game/code/e06/e06s02.rpy:142
translate russian e06s02_0b12f8b9:

    # mc "But she's not even really listening to you right now."
    mc "Но сейчас она тебя даже не слушает."

# game/code/e06/e06s02.rpy:145
translate russian e06s02_03fb93dd:

    # lc "Then I'll make her listen."
    lc "Значит, я заставлю её слушать."

# game/code/e06/e06s02.rpy:152
translate russian e06s02_d7816bd5:

    # lc "Get me up there, [mcname]."
    lc "Помоги мне подняться, [mcname]."

# game/code/e06/e06s02.rpy:157
translate russian e06s02_aed3a517:

    # mc "I think there's a-"
    mc "Думаю, там..."

# game/code/e06/e06s02.rpy:160
translate russian e06s02_a0faa57d:

    # lc "There is no time. Just lift me up."
    lc "Нет времени. Просто подсади меня."

# game/code/e06/e06s02.rpy:172
translate russian e06s02_030e1fa8:

    # lc "Let me in."
    lc "Позволь мне войти."

# game/code/e06/e06s02.rpy:175
translate russian e06s02_ca78a93c:

    # mes "Not going to happen."
    mes "Ничего не получится."

# game/code/e06/e06s02.rpy:178
translate russian e06s02_874821ba:

    # lc "Please Min. I know I screwed up. I'm so sorry."
    lc "Прошу тебя, Мин. Я знаю, я ошиблась. Мне так жаль."

# game/code/e06/e06s02.rpy:183
translate russian e06s02_6f3788af:

    # mes "Screwed up?"
    mes "Ошиблась?"

# game/code/e06/e06s02.rpy:184
translate russian e06s02_85411c4f:

    # mes "No. When people put onions on your burger when you asked for no onions."
    mes "Нет. Когда тебе в бургер кладут лук, хотя ты просила без лука."

# game/code/e06/e06s02.rpy:186
translate russian e06s02_c6a67ef1:

    # mes "That's a screw-up."
    mes "Вот это ошибка."

# game/code/e06/e06s02.rpy:187
translate russian e06s02_a7db7448:

    # mes "What you did was illegal and duplicitous."
    mes "То, что ты сделала, было незаконным и двуличным."

# game/code/e06/e06s02.rpy:190
translate russian e06s02_eab9aaab:

    # mes "You're a liar, Lydia! You were supposed to be my best friend."
    mes "Ты лгунья, Лидия! Ты ведь могла стать моей лучшей подругой."

# game/code/e06/e06s02.rpy:191
translate russian e06s02_c2698a9b:

    # mes "But you lied right to my face. Like a criminal."
    mes "Но ты врала мне прямо в лицо. Как преступница."

# game/code/e06/e06s02.rpy:194
translate russian e06s02_cfa3eceb:

    # mc "Lydia is not a criminal. The judge declared her 'not guilty', Min."
    mc "Лидия не преступница. Судья признала ее \"невиновной\", Мин."

# game/code/e06/e06s02.rpy:197
translate russian e06s02_3c306ef9:

    # mes "*Sniff* That judge must have been huffing glue in their chambers. Lydia might be innocent in the eyes of the law..."
    mes "*Всхлипывает* Должно быть, этот судья надышался клеем в своем кабинете. Лидия может и невиновна в глазах закона..."

# game/code/e06/e06s02.rpy:200
translate russian e06s02_de7714ea:

    # mes "But that doesn't hold any water for me, [mcname]."
    mes "Но для меня это не имеет никакого значения, [mcname]."

# game/code/e06/e06s02.rpy:201
translate russian e06s02_29994ac1:

    # mes "Now just... just get out of here."
    mes "А теперь просто... просто уйдите отсюда."

# game/code/e06/e06s02.rpy:207
translate russian e06s02_ebb6ca1c:

    # lc "Oh no!"
    lc "Ох, нет!"

# game/code/e06/e06s02.rpy:217
translate russian e06s02_f42241db:

    # mc "Lydia?"
    mc "Лидия?"

# game/code/e06/e06s02.rpy:226
translate russian e06s02_898f3335:

    # lc "H-h-how is it so cold? It's the middle of summer-r-r."
    lc "П-п-почему так холодно? Сейчас же с-с-середина лета..."

# game/code/e06/e06s02.rpy:229
translate russian e06s02_af03f88c:

    # mes "*Chuckles* I've been keeping it cold. A brisk polar bear swim in the morning does wonders for your brain."
    mes "*Хихикает* Я поддерживаю в нем низкую температуру. Бодрый заплыв белого медведя по утрам творит чудеса с мозгом."

# game/code/e06/e06s02.rpy:232
translate russian e06s02_52e54ea8:

    # lc "O-o-of c-c-course. C-c-can you help me? P-p-please?"
    lc "Д-д-да, к-к-конечно. М-м-можешь мне помочь? П-п-пожалуйста?"

# game/code/e06/e06s02.rpy:235
translate russian e06s02_f57937f1:

    # mes "*Sigh* I'll unlock the door."
    mes "*Вздох* Я открою дверь."

# game/code/e06/e06s02.rpy:254
translate russian e06s02_inside_33ca22c3:

    # mes "I guess I need to lay out some clothes for you."
    mes "Полагаю, мне нужно подобрать для тебя одежду."

# game/code/e06/e06s02.rpy:257
translate russian e06s02_inside_3055d669:

    # lc "Thank you. And Min-"
    lc "Спасибо. И Мин..."

# game/code/e06/e06s02.rpy:259
translate russian e06s02_inside_38b32cd4:

    # lc "What am I going to do?"
    lc "Что мне делать?"

# game/code/e06/e06s02.rpy:262
translate russian e06s02_inside_5c8d4ab5:

    # mc "Huh? I guess just wait for Min to come back."
    mc "А? Думаю, просто подожди, пока Мин вернётся."

# game/code/e06/e06s02.rpy:264
translate russian e06s02_inside_700e6051:

    # mc "You alright?"
    mc "Ты в порядке?"

# game/code/e06/e06s02.rpy:267
translate russian e06s02_inside_f42241db:

    # mc "Lydia?"
    mc "Лидия?"

# game/code/e06/e06s02.rpy:270
translate russian e06s02_inside_60bb48e2:

    # lc "Hmm?"
    lc "А, что?"

# game/code/e06/e06s02.rpy:273
translate russian e06s02_inside_865c3c36:

    # mc "I asked if you were alright."
    mc "Я спросил, все ли с тобой в порядке."

# game/code/e06/e06s02.rpy:276
translate russian e06s02_inside_1dc4fa3a:

    # lc "I think I'll be fine. It's just... did you see how she looked at me?"
    lc "Я думаю, со мной все будет в порядке. Просто... ты видел, как она на меня смотрела?"

# game/code/e06/e06s02.rpy:279
translate russian e06s02_inside_e40acce8:

    # mc "Yes. But I also saw how she invited you in to help you dry off."
    mc "Да. Но я также видел, как она пригласила тебя войти, чтобы ты могла обсохнуть."

# game/code/e06/e06s02.rpy:281
translate russian e06s02_inside_2c9b3f1e:

    # mc "She might be mad, but she still came running to help out."
    mc "Может, она и злится, но все равно решила прийти на помощь."

# game/code/e06/e06s02.rpy:286
translate russian e06s02_inside_a0b624f6:

    # mes "I got you some dry clothes."
    mes "Я нашла для тебя сухую одежду."

# game/code/e06/e06s02.rpy:289
translate russian e06s02_inside_12dae7f2:

    # lc "Thank you, Min. But I'd really love to-"
    lc "Спасибо, Мин. Но мне бы очень хотелось..."

# game/code/e06/e06s02.rpy:292
translate russian e06s02_inside_fa58f5c5:

    # mes "Clothes first. Then..."
    mes "Сначала оденься. Потом..."

# game/code/e06/e06s02.rpy:295
translate russian e06s02_inside_27fced0d:

    # mes "Maybe I'll be open to... whatever you have to say."
    mes "Может, я смогу выслушать... всё, что ты скажешь."

# game/code/e06/e06s02.rpy:297
translate russian e06s02_inside_b4454227:

    # mes "They're laid out in the guest bedroom at the end of the hall."
    mes "Она лежит в гостевой спальне в конце коридора."

# game/code/e06/e06s02.rpy:300
translate russian e06s02_inside_bf0ad310:

    # lc "Thank you."
    lc "Спасибо."

# game/code/e06/e06s02.rpy:307
translate russian e06s02_inside_348ccd49:

    # lc "I'll be right back."
    lc "Я скоро вернусь."

# game/code/e06/e06s02.rpy:312
translate russian e06s02_inside_f863d2eb:

    # mes "I don't believe this. She nearly got herself killed trying to talk to me."
    mes "Не могу поверить. Она чуть не погибла, пытаясь поговорить со мной."

# game/code/e06/e06s02.rpy:315
translate russian e06s02_inside_a115f9b2:

    # mc "Friendship and love can make you do crazy things."
    mc "Дружба и любовь могут заставить совершать безумные поступки."

# game/code/e06/e06s02.rpy:321
translate russian e06s02_inside_a741581f:

    # mes "Be honest with me, [mcname]."
    mes "Будь честен со мной, [mcname]."

# game/code/e06/e06s02.rpy:323
translate russian e06s02_inside_0c764b95:

    # mes "Even after everything... Everything you went through with the app, and knowing that Lydia was involved..."
    mes "После всего... Всего, через что ты прошел с приложением, и зная, что Лидия была в этом замешана..."

# game/code/e06/e06s02.rpy:324
translate russian e06s02_inside_80be8b53:

    # mes "You've truly forgiven her?"
    mes "Ты правда простил ее?"

# game/code/e06/e06s02.rpy:327
translate russian e06s02_inside_ffc7e92f:

    # mc "One-hundred percent."
    mc "На все сто процентов."

# game/code/e06/e06s02.rpy:330
translate russian e06s02_inside_18de6647:

    # mes "*Sigh* Two persons cannot long be friends if they cannot forgive each other’s little failings."
    mes "*Вздох* Два человека не смогут быть друзьями, если они не умеют прощать друг другу малейшие ошибки."

# game/code/e06/e06s02.rpy:333
translate russian e06s02_inside_900df7fa:

    # mc "Jean de La Bruyère would be proud."
    mc "Жан де Лабрюйер гордился бы этим.\n\n\n{size=-10}{i}(Жан де Лабрюйер - французский моралист, психолог, писатель. Прим.пер.){/i}{/size}"

# game/code/e06/e06s02.rpy:336
translate russian e06s02_inside_6656d334:

    # mes "Yes. But other people won't be so proud. Not if I forgive Lydia."
    mes "Да. Но другие люди не будут гордиться этим. Конечно, если я прощу Лидию."

# game/code/e06/e06s02.rpy:339
translate russian e06s02_inside_cd98dd5d:

    # mc "It's not up to your family what you do here, Min. This is your decision."
    mc "Не от твоей семьи зависит, что ты будешь делать здесь, Мин. Это твое решение."

# game/code/e06/e06s02.rpy:342
translate russian e06s02_inside_418eb0a1:

    # mes "Hmph."
    mes "Хм."

# game/code/e06/e06s02.rpy:345
translate russian e06s02_inside_86dcc93d:

    # mes "Why did she have this redemption tour with me?"
    mes "Зачем она устроила этот искупительный тур со мной?"

# game/code/e06/e06s02.rpy:348
translate russian e06s02_inside_95eef4a4:

    # mc "You two were so close. It's hitting Lydia hard."
    mc "Вы двое были очень близки. Для Лидии это очень важно."

# game/code/e06/e06s02.rpy:351
translate russian e06s02_inside_ac67bcd1:

    # mes "My family has been calling since word got out about her being in jail."
    mes "Мои родители постоянно названивают мне с тех пор, как узнали, что она в тюрьме."

# game/code/e06/e06s02.rpy:352
translate russian e06s02_inside_59e30423:

    # mes "They want me to go back to Korea and never leave."
    mes "Они хотят, чтобы я вернулась в Корею и никогда не возвращалась."

# game/code/e06/e06s02.rpy:355
translate russian e06s02_inside_11455b53:

    # mc "What?"
    mc "Что?"

# game/code/e06/e06s02.rpy:358
translate russian e06s02_inside_fdcf3625:

    # mes "Yes."
    mes "Да."

# game/code/e06/e06s02.rpy:361
translate russian e06s02_inside_8fcefd64:

    # mc "But you're not going to do that are you?"
    mc "Но ты же не собираешься этого делать?"

# game/code/e06/e06s02.rpy:363
translate russian e06s02_inside_81aef917:

    # mes "..."
    mes "..."

# game/code/e06/e06s02.rpy:366
translate russian e06s02_inside_48001862:

    # lc "Please don't go, Min."
    lc "Пожалуйста, не уезжай, Мин."

# game/code/e06/e06s02.rpy:371
translate russian e06s02_inside_414e4e6f:

    # mes "Hahahahah!"
    mes "Хахахахах!"

# game/code/e06/e06s02.rpy:376
translate russian e06s02_inside_83bf5d23:

    # mes "I'm so sorry. You just look ridiculous, Lydia."
    mes "Прости меня. Лидия, ты выглядишь нелепо."

# game/code/e06/e06s02.rpy:377
translate russian e06s02_inside_2bc8af86:

    # mes "I bet your nipples could cut glass right about now."
    mes "Держу пари, твои соски сейчас могут разрезать стекло."

# game/code/e06/e06s02.rpy:380
translate russian e06s02_inside_90a5de01:

    # lc "Anything looks better than a jumpsuit."
    lc "Что угодно выглядит лучше, чем комбинезон."

# game/code/e06/e06s02.rpy:383
translate russian e06s02_inside_8dffc7e8:

    # mes "Yeah. Except that. Hahaha."
    mes "Да. Кроме этого. Хахаха."

# game/code/e06/e06s02.rpy:386
translate russian e06s02_inside_e28ddb2c:

    # mes "Ahem. Okay..."
    mes "Кхм. Ладно..."

# game/code/e06/e06s02.rpy:389
translate russian e06s02_inside_35f735f5:

    # mes "I need you to listen closely. Both of you."
    mes "Мне нужно, чтобы вы внимательно выслушали меня. Вы оба."

# game/code/e06/e06s02.rpy:390
translate russian e06s02_inside_af9528f7:

    # mes "Now... I don't know if there is the chance for us to go back to how things were."
    mes "Сейчас... Я не знаю, есть ли у нас шанс вернуть всё как было."

# game/code/e06/e06s02.rpy:392
translate russian e06s02_inside_06c00b8b:

    # mes "And I wasn't planning to see you for a while. Maybe even forever."
    mes "И я не планировала встречаться с тобой какое-то время. Возможно, даже никогда."

# game/code/e06/e06s02.rpy:393
translate russian e06s02_inside_827bd663:

    # mes "But... seeing you both... it reminded me about Squizzle. And well..."
    mes "Но... увидев вас обоих... это напомнило мне о Сквизли. И знаете..."

# game/code/e06/e06s02.rpy:396
translate russian e06s02_inside_0f65f7df:

    # mes "Squizzle never would have existed without FL."
    mes "Сквизли никогда бы не существовала без ФЛ."

# game/code/e06/e06s02.rpy:399
translate russian e06s02_inside_a755748b:

    # mes "She certainly neer could have learned all the unique... pleasures of watersports."
    mes "Она, определенно, так и не познала эти уникальные... удовольствия от водных видов спорта."

# game/code/e06/e06s02.rpy:400
translate russian e06s02_inside_5ae2ee77:

    # mes "Few things can match that level of level of freedom and intimacy. Certainly never anything Squizzle experienced."
    mes "Мало что может сравниться с таким уровнем свободы и близости. И уж точно никогда ничего подобного не испытывала Сквизли."

# game/code/e06/e06s02.rpy:402
translate russian e06s02_inside_ad21e7ac:

    # mes "I doubt she would ever feel true affection either... but thanks to [mcname], I think I got pretty close."
    mes "Сомневаюсь, что она могла бы почувствовать настоящую привязанность... но благодаря [mcname], думаю, я подобралась довольно близко."

# game/code/e06/e06s02.rpy:405
translate russian e06s02_inside_12076c0c:

    # mes "For that happiness, it kind of makes sense that there would be some sadness to balance things out."
    mes "Если это радость, то вполне логично, что для баланса найдется немного грусти."

# game/code/e06/e06s02.rpy:406
translate russian e06s02_inside_b016c83e:

    # mes "Say what you're here to say, Lydia."
    mes "Скажи то, ради чего ты пришла, Лидия."

# game/code/e06/e06s02.rpy:409
translate russian e06s02_inside_2ccbd619:

    # lc "I just want to say how sorry I am, Min. I never meant to hurt you. And I don't want you to leave."
    lc "Я хотела сказать, что мне очень жаль, Мин. Я никогда не хотела причинять тебе боль. И я не хочу, чтобы ты уезжала."

# game/code/e06/e06s02.rpy:411
translate russian e06s02_inside_b01c2ee6:

    # lc "I know it won't count for much, but once I realized how far things had gotten, I turned myself in and explained what was happening."
    lc "Я знаю, что это не имеет большого значения, но как только я поняла, как далеко всё зашло, я сдалась и объяснила, что происходит."

# game/code/e06/e06s02.rpy:413
translate russian e06s02_inside_ba401dd3:

    # lc "I should have known Fetish Locator would turn out wicked. It's not like its origin was completely pure."
    lc "Я должна была догадаться, что Фетиш Локатор может оказаться ужасным. Ведь его происхождение не было абсолютно чистым."

# game/code/e06/e06s02.rpy:416
translate russian e06s02_inside_cafdbda1:

    # mes "What do you mean?"
    mes "Что ты имеешь в виду?"

# game/code/e06/e06s02.rpy:419
translate russian e06s02_inside_c2660f6d:

    # lc "I wanted to experience things. And I... I lied to myself. I started getting convinced that using Fetish Locator would finally let me compete with you."
    lc "Я хотела испытать нечто новое. И я... Я лгала себе. Я стала убеждать себя, что использование Фетиш Локатора наконец-то позволит мне соревноваться с тобой."

# game/code/e06/e06s02.rpy:422
translate russian e06s02_inside_77edfd14:

    # mes "Lydia... not everything between us has to be a competition."
    mes "Лидия... не все между нами должно превращаться в соревнование."

# game/code/e06/e06s02.rpy:425
translate russian e06s02_inside_204a9a46:

    # lc "I know but. It's not like a switch. Between us, you picked up all the firsts and..."
    lc "Я знаю, но. Это не похоже на переключатель. Между нами, ты забирала всё первой и..."

# game/code/e06/e06s02.rpy:426
translate russian e06s02_inside_a9ed4e36:

    # lc "Without something like Fetish Locator, how in the world could I ever even hold a candle to you."
    lc "Без чего-то вроде Фетиш Локатора, я бы никогда не смогла сравниться с тобой."

# game/code/e06/e06s02.rpy:429
translate russian e06s02_inside_38bf30c7:

    # mes "*Chuckle* I'll admit, a part of me did get a little bored always being first."
    mes "*Смеётся* Признаюсь, мне немного надоело всегда быть первой."

# game/code/e06/e06s02.rpy:430
translate russian e06s02_inside_339aaba1:

    # mes "But if you ever tell anyone I said that, I'll kill you."
    mes "Но если ты кому-нибудь об этом скажешь, я тебя убью."

# game/code/e06/e06s02.rpy:442
translate russian e06s02_inside_bcaf7f5b:

    # mes "Very well. I... I accept your apology."
    mes "Хорошо. Я... Я принимаю твои извинения."

# game/code/e06/e06s02.rpy:444
translate russian e06s02_inside_f0d25db5:

    # mes "But, you must promise me something."
    mes "Но ты должна мне кое-что пообещать."

# game/code/e06/e06s02.rpy:447
translate russian e06s02_inside_c5a65083:

    # lc "Anything."
    lc "Все, что угодно."

# game/code/e06/e06s02.rpy:450
translate russian e06s02_inside_fc22139e:

    # mes "No more secrets. Or we are done and that's that."
    mes "Больше никаких секретов. Или между нами всё кончено и точка."

# game/code/e06/e06s02.rpy:453
translate russian e06s02_inside_c117bfaa:

    # lc "Of course."
    lc "Конечно."

# game/code/e06/e06s02.rpy:457
translate russian e06s02_inside_8331022c:

    # lc "I promise."
    lc "Я обещаю."

# game/code/e06/e06s02.rpy:460
translate russian e06s02_inside_759a3f9c:

    # mes "Then we are good."
    mes "Тогда всё будет хорошо."

# game/code/e06/e06s02.rpy:463
translate russian e06s02_inside_c2adf8c0:

    # lc "*Sniff* You're so wonderful."
    lc "*Всхлипывает* Ты такая замечательная."

# game/code/e06/e06s02.rpy:466
translate russian e06s02_inside_d37b33a3:

    # mes "Heh. Wonderfully forgiving too."
    mes "Хех. И удивительно прощающая."

# game/code/e06/e06s02.rpy:469
translate russian e06s02_inside_f93fea92:

    # lc "*Happy crying* Yes. That too."
    lc "*Плачет от счастья* Да. И это тоже."

# game/code/e06/e06s02.rpy:472
translate russian e06s02_inside_55fc2538:

    # mes "But I don't understand one thing. You've been to my house so many times."
    mes "Но я не понимаю одного. Ты столько раз была в моем доме."

# game/code/e06/e06s02.rpy:473
translate russian e06s02_inside_7acf4641:

    # mes "How did you forget about the stairs?"
    mes "Как ты могла забыть о лестнице?"

# game/code/e06/e06s02.rpy:476
translate russian e06s02_inside_1ccb6013:

    # lc "Oh well... I figured no matter what, you couldn't resist helping me after I made a fool of myself."
    lc "Ну что ж... Я подумала, что после того, как я выставлю себя посмешищем, ты не сможешь мне отказать в помощи."

# game/code/e06/e06s02.rpy:478
translate russian e06s02_inside_9f413db3:

    # mes "Haha. I should have let you freeze a little more."
    mes "Хаха. Надо было дать тебе еще немного померзнуть."

# game/code/e06/e06s02.rpy:489
translate russian e06s02_inside_cf3d7f8d:

    # mes "Well, in the pursuit of transparency, I'm very happy you came along with Lydia, [mcname]."
    mes "В погоне за откровенностью, я очень рада, что ты пришёл вместе с Лидией, [mcname]."

# game/code/e06/e06s02.rpy:492
translate russian e06s02_inside_d4f61567:

    # lc "Oh. What have you got up your sleeve, Min?"
    lc "Ох. Что ты задумала, Мин?"

# game/code/e06/e06s02.rpy:495
translate russian e06s02_inside_a58d0e37:

    # mes "Do I have to spell it out for you both."
    mes "Неужели я должна объяснять это вам обоим?"

# game/code/e06/e06s02.rpy:497
translate russian e06s02_inside_14982f2a:

    # mes "It's been more than a month since I've seen you. And if you don't give me some dick right now, I'm kicking you both out of my house."
    mes "Прошло больше месяца с тех пор, как я вас видела. И если вы не дадите мне какой-нибудь член прямо сейчас, я вышвырну вас обоих из своего дома."

# game/code/e06/e06s02.rpy:507
translate russian e06s02_inside_c760cdf3:

    # lc "I suppose that he can be one thing we truly don't compete over."
    lc "Полагаю, он может стать тем, из-за кого мы действительно не соперничаем."

# game/code/e06/e06s02.rpy:510
translate russian e06s02_inside_951b3f39:

    # mes "I second that motion."
    mes "Поддерживаю это мнение."

# game/code/e06/e06s02.rpy:513
translate russian e06s02_inside_34223e47:

    # mc "Wait, do I get a say in this?"
    mc "Подождите, у меня есть право голоса?"

# game/code/e06/e06s02.rpy:516
translate russian e06s02_inside_87ab8cd2:

    # mes "Only if you want me to stop."
    mes "Только если ты хочешь, чтобы я остановилась."

# game/code/e06/e06s02.rpy:527
translate russian e06s02_inside_5534bdd1:

    # lc "I don't think he wants that. And neither do I."
    lc "Не думаю, что он этого хочет. Впрочем, как и я."

# game/code/e06/e06s02.rpy:530
translate russian e06s02_inside_e47c4076:

    # mes "*Chuckles playfully*"
    mes "*Игриво хихикает*"

# game/code/e06/e06s02.rpy:552
translate russian e06s02_inside_97779572:

    # mes "I can feel how big you are through your pants."
    mes "Я чувствую, насколько он большой даже через штаны."

# game/code/e06/e06s02.rpy:561
translate russian e06s02_inside_ef31455a:

    # lc "I worried... when I was in a cell, that I'd never get to see something like this again."
    lc "Я боялась... когда сидела в камере, что больше никогда не увижу ничего подобного."

# game/code/e06/e06s02.rpy:571
translate russian e06s02_inside_3a9a551c:

    # mes "Mmhmm. You're going to keep seeing this... and so much more."
    mes "Мммм... Ты будешь наблюдать за этим... и многим другим."

# game/code/e06/e06s02.rpy:574
translate russian e06s02_inside_7ae09767:

    # lc "Yes..."
    lc "Да..."

# game/code/e06/e06s02.rpy:577
translate russian e06s02_inside_12a5aab0:

    # lc "God. This is making me so wet... I can hardly wait..."
    lc "Боже. Это так возбуждает меня... Я едва могу терпеть..."

# game/code/e06/e06s02.rpy:580
translate russian e06s02_inside_0669c0f3:

    # mes "I can't wait any more actually."
    mes "Я больше не могу ждать."

# game/code/e06/e06s02.rpy:596
translate russian e06s02_bj_5d3e3d2e:

    # mc "This is incredible."
    mc "Это невероятно."

# game/code/e06/e06s02.rpy:597
translate russian e06s02_bj_d0dcf03c:

    # mct "But I think we can still spice things up a bit."
    mct "Но я думаю, что можно ещё немного разнообразить ситуацию."

# game/code/e06/e06s02.rpy:619
translate russian e06s02_bj_67d98a7a:

    # mc "I love fucking you in front of your best friend."
    mc "Я люблю трахать тебя на глазах твоей лучшей подруги."

# game/code/e06/e06s02.rpy:622
translate russian e06s02_bj_bebe370f:

    # lc "Yes... keep going... That looks so fun. So primal..."
    lc "Да... продолжай... Это так весело выглядит. Так естественно..."

# game/code/e06/e06s02.rpy:630
translate russian e06s02_bj_a71b0e99:

    # mes "*Stifled moans*"
    mes "*Приглушенно стонет*"

# game/code/e06/e06s02.rpy:633
translate russian e06s02_bj_f84c7857:

    # mes "*Gagging*"
    mes "*Рвотные позывы*"

# game/code/e06/e06s02.rpy:642
translate russian e06s02_bj_7e7f582f:

    # mct "I'll never get tired of watching my girl drooling as I fuck other girls."
    mct "Я никогда не устану наблюдать, как моя девушка пускает слюни, пока я трахаю других девушек."

# game/code/e06/e06s02.rpy:646
translate russian e06s02_bj_e0c9be44:

    # lc "I want to help."
    lc "Я хочу помочь."

# game/code/e06/e06s02.rpy:648
translate russian e06s02_bj_762345b2:

    # mc "Your wishes my command."
    mc "Твое желание - мой приказ."

# game/code/e06/e06s02.rpy:660
translate russian e06s02_bj_d32d92d6:

    # mc "Holy fuck. I'm getting close."
    mc "Твою же мать. Я уже близко."

# game/code/e06/e06s02.rpy:663
translate russian e06s02_bj_22743387:

    # lc "Alright, why don't you let me have some fun before you erupt."
    lc "Ладно, позволь мне немного повеселиться, прежде чем ты взорвёшься."

# game/code/e06/e06s02.rpy:670
translate russian e06s02_bj_71d15ac7:

    # mes "I've missed your cock so fucking much, [mcname]."
    mes "Мне так не хватало твоего члена, [mcname]."

# game/code/e06/e06s02.rpy:684
translate russian e06s02_bj_2c66c77f:

    # lc "Did you miss anything else?"
    lc "Ты упустила что-нибудь ещё?"

# game/code/e06/e06s02.rpy:687
translate russian e06s02_bj_23aa3b29:

    # mes "Hmmm."
    mes "Ммммм."

# game/code/e06/e06s02.rpy:699
translate russian e06s02_bj_f1912be4:

    # lc "Min... that's... ooouhah.."
    lc "Мин... это... ооооухаха..."

# game/code/e06/e06s02.rpy:702
translate russian e06s02_bj_d6cdde8f:

    # mes "You're so wet."
    mes "Ты такая мокрая."

# game/code/e06/e06s02.rpy:705
translate russian e06s02_bj_3176f909:

    # lc "Oh... yes.. Keep touching me there."
    lc "О... да... Продолжай трогать меня там."

# game/code/e06/e06s02.rpy:706
translate russian e06s02_bj_a7c9e738:

    # lc "Your fingers feel as good as your tongue!"
    lc "Твои пальцы так же хороши, как и твой язык!"

# game/code/e06/e06s02.rpy:719
translate russian e06s02_bj_16f9e268:

    # mes "Your taste... I could just eat you up all day."
    mes "Этот вкус... Я могла бы есть тебя весь день."

# game/code/e06/e06s02.rpy:722
translate russian e06s02_bj_1042bf60:

    # lc "Haha."
    lc "Хаха."

# game/code/e06/e06s02.rpy:731
translate russian e06s02_bj_38381b8b:

    # lc "Having the two of you with me. There is nothing like it."
    lc "Когда вы оба со мной. Ничто не сравнится с этим."

# game/code/e06/e06s02.rpy:733
translate russian e06s02_bj_804fa8ac:

    # lc "You make me feel so safe. Secure."
    lc "С вами я чувствую себя в безопасности. Защищённой."

# game/code/e06/e06s02.rpy:749
translate russian e06s02_bj_cfe62b5f:

    # mes "That's what best friends are for."
    mes "Для этого и существуют лучшие друзья."

# game/code/e06/e06s02.rpy:752
translate russian e06s02_bj_7c36fb8f:

    # mc "Best friends and husbands."
    mc "Лучшие друзья и мужья."

# game/code/e06/e06s02.rpy:755
translate russian e06s02_bj_e38161b3:

    # mes "Husband?"
    mes "Муж?"

# game/code/e06/e06s02.rpy:758
translate russian e06s02_bj_d911344e:

    # lc "Yes. After I was freed, [mcname] proposed."
    lc "Да. После моего освобождения [mcname] сделал мне предложение."

# game/code/e06/e06s02.rpy:761
translate russian e06s02_bj_b48ce370:

    # mes "Haha. That is wonderful."
    mes "Хаха. Это чудесно."

# game/code/e06/e06s02.rpy:764
translate russian e06s02_bj_95ec38ce:

    # lc "It was, but as deep as our love goes, I'm still so grateful that I didn't lose you, Min."
    lc "Так и есть, но как бы сильна ни была наша любовь, я все равно очень рада, что не потеряла тебя, Мин."

# game/code/e06/e06s02.rpy:767
translate russian e06s02_bj_2ce90328:

    # mes "I know, Lydia. I love both of you so much..."
    mes "Я знаю, Лидия. Я так сильно люблю вас обоих..."

# game/code/e06/e06s02.rpy:776
translate russian e06s02_sex_7ac303f4:

    # mc "*Hungry slurping*"
    mc "*Жадно причмокивает*"

# game/code/e06/e06s02.rpy:783
translate russian e06s02_sex_c9b902a3:

    # mes "Please... keep pushing your tongue inside me."
    mes "Пожалуйста... продолжай засовывать в меня свой язык."

# game/code/e06/e06s02.rpy:786
translate russian e06s02_sex_1a2e8223:

    # mc "It's not going anywhere."
    mc "Я не собираюсь отступать."

# game/code/e06/e06s02.rpy:798
translate russian e06s02_sex_632589fb:

    # mes "Mhmm... how does he taste, Lydia.?"
    mes "Ммм... каков он на вкус, Лидия?"

# game/code/e06/e06s02.rpy:801
translate russian e06s02_sex_d29c183d:

    # lc "Incredible. He's been leaking precum ever since I started."
    lc "Потрясающе. Он постоянно подтекает спермой с тех пор, как я начала."

# game/code/e06/e06s02.rpy:804
translate russian e06s02_sex_fd9a6df5:

    # mes "We certainly have good taste in men, don't we?"
    mes "У нас определенно хороший вкус на мужчин, не так ли?"

# game/code/e06/e06s02.rpy:814
translate russian e06s02_sex_255368e0:

    # mes "Oh fuck. That's it. You're driving me crazy, [mcname]!"
    mes "Вот черт. Вот оно. Ты сводишь меня с ума, [mcname]!"

# game/code/e06/e06s02.rpy:822
translate russian e06s02_sex_85e59949:

    # lc "Mmm. Do you want this big thick cock to hammer that tight little pussy of yours."
    lc "Ммм... Ты хочешь, чтобы этот большой толстый член долбил твою маленькую тугую киску."

# game/code/e06/e06s02.rpy:825
translate russian e06s02_sex_87ef72ce:

    # mes "You're god-damn right I do."
    mes "Ты чертовски права."

# game/code/e06/e06s02.rpy:830
translate russian e06s02_sex_d1be316a:

    # mes "Kiss me, sir. Kiss me and fuck me."
    mes "Поцелуй меня, сэр. Поцелуй и трахни меня."

# game/code/e06/e06s02.rpy:833
translate russian e06s02_sex_72e0d876:

    # mc "Haha. You didn't say the magic words?"
    mc "Хаха. Ты не произнесла волшебное слово?"

# game/code/e06/e06s02.rpy:836
translate russian e06s02_sex_806c74d8:

    # mes "Please pound my naughty little fuck hole."
    mes "Пожалуйста, оттрахай мою маленькую развратную дырочку."

# game/code/e06/e06s02.rpy:838
translate russian e06s02_sex_bd7c4f82:

    # mes "Make me cum in scream in front of my best friend."
    mes "Заставь меня бурно кончить на глазах моей лучшей подруги."

# game/code/e06/e06s02.rpy:840
translate russian e06s02_sex_58283b86:

    # mes "Even when you've fucked me senseless, just keep using me."
    mes "Даже когда ты затрахаешь меня до бесчувствия, продолжай использовать меня."

# game/code/e06/e06s02.rpy:843
translate russian e06s02_sex_945eb934:

    # mes "You know I'm all yours..."
    mes "Ты знаешь, что я вся твоя..."

# game/code/e06/e06s02.rpy:846
translate russian e06s02_sex_c54eb583:

    # lc "Oh em eff gee. This is so hot!"
    lc "О, черт возьми. Это так горячо!"

# game/code/e06/e06s02.rpy:850
translate russian e06s02_sex_8030eff3:

    # mes "Oh yes!"
    mes "О да!"

# game/code/e06/e06s02.rpy:853
translate russian e06s02_sex_f0d4c776:

    # mc "Oh fuck. Your pussy is locking around my cock."
    mc "Вот блядь. Твоя киска сжимается вокруг моего члена."

# game/code/e06/e06s02.rpy:856
translate russian e06s02_sex_a86a90f4:

    # mes "Well yes. I haven't been fucked in more than a month."
    mes "Ну да. Я не трахалась уже больше месяца."

# game/code/e06/e06s02.rpy:859
translate russian e06s02_sex_8079190a:

    # mc "I should hold off fucking you more often."
    mc "Мне следует чаще трахать тебя."

# game/code/e06/e06s02.rpy:862
translate russian e06s02_sex_9ff35125:

    # mes "Don't you fucking dare..."
    mes "Не смей..."

# game/code/e06/e06s02.rpy:870
translate russian e06s02_sex_44279e60:

    # mc "Like that? Like tasting your pussy off my mouth you dirty girl."
    mc "Нравится? Как будто пробуешь свою киску с моего языка, грязная девчонка."

# game/code/e06/e06s02.rpy:873
translate russian e06s02_sex_b16329a8:

    # mes "Yes. Please sir. Fuck me harder. Don't hold back!"
    mes "Да. Пожалуйста, сэр. Трахай меня сильнее. Не сдерживайся!"

# game/code/e06/e06s02.rpy:876
translate russian e06s02_sex_21394d7b:

    # lc "Oooh. I just got an idea."
    lc "Оооо. У меня появилась идея."

# game/code/e06/e06s02.rpy:879
translate russian e06s02_sex_6341b83f:

    # mes "Huh?"
    mes "А?"

# game/code/e06/e06s02.rpy:882
translate russian e06s02_sex_cb40993e:

    # lc "Please excuse me."
    lc "Прошу простить меня."

# game/code/e06/e06s02.rpy:886
translate russian e06s02_sex_6a4e2a0c:

    # mes "*Gasp*"
    mes "*задыхается*"

# game/code/e06/e06s02.rpy:889
translate russian e06s02_sex_e12b8754:

    # mes "You're getting so into it, Lydia."
    mes "Ты так прониклась этим, Лидия."

# game/code/e06/e06s02.rpy:892
translate russian e06s02_sex_868859cb:

    # lc "Hehe. Well, Master is treating your hole my little toy."
    lc "Хе-хе. Господин обрабатывает твою дырочку как мою маленькую игрушку."

# game/code/e06/e06s02.rpy:893
translate russian e06s02_sex_7d58c44e:

    # lc "You should thank your Mistress."
    lc "Ты должна быть благодарна своей госпоже."

# game/code/e06/e06s02.rpy:896
translate russian e06s02_sex_8b0c4246:

    # mes "I think I can do that."
    mes "Пожалуй, я могу это сделать."

# game/code/e06/e06s02.rpy:908
translate russian e06s02_sex_d9a23329:

    # lc "Fuck. Oh yes... your tongue is already moving so fast!"
    lc "Ебать. О да... твой язык так быстро двигается!"

# game/code/e06/e06s02.rpy:912
translate russian e06s02_sex_83078e78:

    # mc "I love you."
    mc "Я люблю тебя."

# game/code/e06/e06s02.rpy:913
translate russian e06s02_sex_e2cb8e1e:

    # lc "I love you too."
    lc "Я тоже тебя люблю."

# game/code/e06/e06s02.rpy:919
translate russian e06s02_sex_efcb925e:

    # mc "And Min, even when you forget to beg, I'll always love you."
    mc "И Мин, даже когда ты перестанешь умолять, я всегда буду любить тебя."

# game/code/e06/e06s02.rpy:920
translate russian e06s02_sex_9d9492e6:

    # mes "*Happy stifled moans*"
    mes "*Довольные приглушенные стоны*"

# game/code/e06/e06s02.rpy:922
translate russian e06s02_sex_43e2a595:

    # lc "I think I'm getting good at translating Min's words when her mouth is full of pussy."
    lc "Кажется, у меня хорошо получается переводить слова Мин, пока ее рот занят моей киской."

# game/code/e06/e06s02.rpy:923
translate russian e06s02_sex_03993b2e:

    # lc "She says. I love you too. Now break my tiny pussy!"
    lc "Она говорит. Я тоже тебя люблю. Теперь раздолбай мою крошечную киску!"

# game/code/e06/e06s02.rpy:928
translate russian e06s02_sex_ee8f2e08:

    # mc "*Chuckles* Not a problem..."
    mc "*Хихикает* Без проблем..."

# game/code/e06/e06s02.rpy:929
translate russian e06s02_sex_65ca6826:

    # mes "*Muffled moaning*"
    mes "*Приглушенно стонет*"

# game/code/e06/e06s02.rpy:944
translate russian e06s02_sex_f42241db:

    # mc "Lydia?"
    mc "Лидия?"

# game/code/e06/e06s02.rpy:947
translate russian e06s02_sex_02913ee9:

    # lc "I think I have to try licking her down there."
    lc "Думаю, я должна попробовать вылизать ее там."

# game/code/e06/e06s02.rpy:950
translate russian e06s02_sex_e61804b5:

    # mes "*Slowly pants* Yes. Climb every... phew... mountain."
    mes "*Медленно дышит* Да. Заберись на каждую... фух... вершину."

# game/code/e06/e06s02.rpy:965
translate russian e06s02_sex_90446b20:

    # mes "Oouhah... careful. I'm still so... oh fuck."
    mes "Оухах... осторожно. Я все еще так... охренеть."

# game/code/e06/e06s02.rpy:974
translate russian e06s02_sex_3e7a8a95:

    # mes "*Breathless* You're going to make me cum again."
    mes "*Без передышки* Ты заставишь меня кончить снова."

# game/code/e06/e06s02.rpy:977
translate russian e06s02_sex_194b53bf:

    # lc "Not yet. You shouldn't cum again until after [mcname] gets the chance."
    lc "Еще нет. Ты не должна кончать снова, пока [mcname] не позволит."

# game/code/e06/e06s02.rpy:980
translate russian e06s02_sex_573585a0:

    # mes "Okay... I'll try. He just feels so intense when he rails me."
    mes "Ладно... Я попробую. Он просто такой энергичный, когда насилует меня."

# game/code/e06/e06s02.rpy:983
translate russian e06s02_sex_5c5f66e3:

    # lc "I'll be with you. The whole way."
    lc "Я буду с тобой. До конца."

# game/code/e06/e06s02.rpy:985
translate russian e06s02_sex_c9473348:

    # lc "Are you ready?"
    lc "Ты готова?"

# game/code/e06/e06s02.rpy:988
translate russian e06s02_sex_e431c529:

    # mes "More than ready."
    mes "Более чем..."

# game/code/e06/e06s02.rpy:1006
translate russian e06s02_sex_608c7006:

    # lc "Oh god."
    lc "О боже."

# game/code/e06/e06s02.rpy:1010
translate russian e06s02_sex_05977308:

    # mes "What? What's wrong?"
    mes "Что? Что случилось?"

# game/code/e06/e06s02.rpy:1013
translate russian e06s02_sex_ce9d233d:

    # lc "Nothing. This is perfect."
    lc "Ничего. Это превосходно."

# game/code/e06/e06s02.rpy:1029
translate russian e06s02_sex_19c7db08:

    # lc "This position. Watching [mcname]'s cock spear into you."
    lc "Эта поза. Наблюдать, как член [mcname]а вонзается в тебя."

# game/code/e06/e06s02.rpy:1030
translate russian e06s02_sex_e99a3f55:

    # lc "It's almost like he's fucking my pussy."
    lc "Такое ощущение, что он трахает мою киску."

# game/code/e06/e06s02.rpy:1033
translate russian e06s02_sex_30a17289:

    # mes "Haha. God... oh fuck. You don't know... what you're missing."
    mes "Хаха. Боже... ох бля. Ты даже не представляешь... что теряешь."

# game/code/e06/e06s02.rpy:1039
translate russian e06s02_sex_c353c64b:

    # lc "It's okay."
    lc "Всё прекрасно."

# game/code/e06/e06s02.rpy:1040
translate russian e06s02_sex_aaf4f8ec:

    # lc "*Panting* I will... very soon."
    lc "*Тяжело дышит* Я сделаю это... очень скоро."

# game/code/e06/e06s02.rpy:1046
translate russian e06s02_sex_05a75e57:

    # mes "Yes... keep knocking on my door, [mcname]!"
    mes "Да... продолжай ломиться в мою щёлку, [mcname]!"

# game/code/e06/e06s02.rpy:1050
translate russian e06s02_sex_9b9c29e2:

    # mes "Please... let me cum, Lydia. I feel like my brain is melting."
    mes "Пожалуйста... позволь мне кончить, Лидия. Я чувствую, что мой мозг плавится."

# game/code/e06/e06s02.rpy:1053
translate russian e06s02_sex_a57d201d:

    # lc "Okay... This time, I'll let you cum."
    lc "Хорошо... В этот раз я дам тебе кончить."

# game/code/e06/e06s02.rpy:1054
translate russian e06s02_sex_681b46c9:

    # lc " But I think we're going to have to work on your endurance."
    lc "Но думаю, нам стоит поработать над твоей выносливостью."

# game/code/e06/e06s02.rpy:1059
translate russian e06s02_sex_2d78a730:

    # lc "Or [mcname] will always beat us."
    lc "Или [mcname] всегда будет нас обыгрывать."

# game/code/e06/e06s02.rpy:1060
translate russian e06s02_sex_4f89cc72:

    # mes "*Laughs and pants*"
    mes "*Смеётся и тяжело дышит*"

# game/code/e06/e06s02.rpy:1063
translate russian e06s02_sex_96dcddf2:

    # lc "So cum. Let him feel your juices splashing over his cock."
    lc "Кончай. Пусть он почувствует, как твои соки растекаются по его члену."

# game/code/e06/e06s02.rpy:1064
translate russian e06s02_sex_67f06d9d:

    # mes "Yes. Oh thank you."
    mes "Да. Ах, спасибо."

# game/code/e06/e06s02.rpy:1072
translate russian e06s02_sex_cf716fe7:

    # mes "*Moans heartily*"
    mes "*Душевные стоны*"

# game/code/e06/e06s02.rpy:1078
translate russian e06s02_sex_851f594e:

    # mes "You wouldn't be mean to me and stop without filing me up would you?"
    mes "Ты же не будешь груб со мной и не остановишься, не заполнив меня?"

# game/code/e06/e06s02.rpy:1081
translate russian e06s02_sex_f5ee3dd1:

    # lc "Yes... give her that warm... thick cum."
    lc "Да... дай ей эту теплую... густую сперму."

# game/code/e06/e06s02.rpy:1082
translate russian e06s02_sex_618acf43:

    # lc "I want to see it oozing out of her."
    lc "Хочу увидеть, как она сочится из неё."

# game/code/e06/e06s02.rpy:1098
translate russian e06s02_sex_1430ec61:

    # mc "That felt incredible. I think you milked me dry, Min."
    mc "Это было невероятно. Думаю, ты выдоила меня досуха, Мин."

# game/code/e06/e06s02.rpy:1101
translate russian e06s02_sex_e55d83b4:

    # mes "*Soft breathing.*"
    mes "*Мягкое дыхание.*"

# game/code/e06/e06s02.rpy:1103
translate russian e06s02_sex_d693d9ed:

    # lc "Min?"
    lc "Мин?"

# game/code/e06/e06s02.rpy:1106
translate russian e06s02_sex_dcd7932a:

    # mes "I don't think I'm going to be able to walk for a few days."
    mes "Кажется, я не смогу ходить несколько дней."

# game/code/e06/e06s02.rpy:1109
translate russian e06s02_sex_369ce2c8:

    # lc "Hehe."
    lc "Хехе."

# game/code/e06/e06s02.rpy:1112
translate russian e06s02_sex_f7a5e610:

    # mc "I guess I could carry you to the shower."
    mc "Думаю, я могу отнести тебя в душ."

# game/code/e06/e06s02.rpy:1115
translate russian e06s02_sex_fa5295ee:

    # mes "Mmm. That sounds perfect."
    mes "Ммм. Звучит идеально."

# game/code/e06/e06s02.rpy:1134
translate russian e06s02_sex_4d1ce34d:

    # mes "Lydia. Since you are serious about making amends, the thought occurs that you cannot stop at just my forgiveness."
    mes "Лидия. Если ты всерьез намерена загладить свою вину, мне пришла мысль, что ты не можешь остановиться только на моем прощении."

# game/code/e06/e06s02.rpy:1135
translate russian e06s02_sex_ab066432:

    # mes "Fetish Locator cast a wide net."
    mes "Фетиш Локатор широко раскинул сеть."

# game/code/e06/e06s02.rpy:1138
translate russian e06s02_sex_d8c51d34:

    # lc "It did. But... getting others to forgive me is going to be very difficult."
    lc "Так и есть. Но... заставить других простить меня будет очень сложно."

# game/code/e06/e06s02.rpy:1141
translate russian e06s02_sex_2ed4c0f4:

    # mes "Of course it will be."
    mes "Конечно, так и будет."

# game/code/e06/e06s02.rpy:1143
translate russian e06s02_sex_f7a873ae:

    # mes "But try not to worry, Lydia. In the end, I'm sure you'll feel better about everything."
    mes "Но постарайся не волноваться, Лидия. В конце концов, я уверена, тебе станет легче."

# game/code/e06/e06s02.rpy:1146
translate russian e06s02_sex_b13a261f:

    # lc "Okay. I hope you're right, Min."
    lc "Хорошо. Надеюсь, ты права, Мин."

# game/code/e06/e06s02.rpy:1149
translate russian e06s02_sex_6e3fb5a0:

    # mes "Of course I am."
    mes "Конечно я права."

# game/code/e06/e06s02.rpy:1151
translate russian e06s02_sex_cfdfa501:

    # mes "Besides, I'm sure you can ask [mcname] to help smooth out the rough edges."
    mes "Кроме того, я уверена, что ты можешь попросить [mcname] помочь сгладить неровности."

# game/code/e06/e06s02.rpy:1152
translate russian e06s02_sex_0dc69c3d:

    # mes "Right?"
    mes "Правильно?"

# game/code/e06/e06s02.rpy:1160
translate russian e06s02_sex_bd6c66f2:

    # lc "Why don't you come stay with us, Min?"
    lc "Почему бы тебе не остаться с нами, Мин?"

# game/code/e06/e06s02.rpy:1163
translate russian e06s02_sex_86368cbc:

    # mes "You sure about that?"
    mes "Ты уверена в этом?"

# game/code/e06/e06s02.rpy:1166
translate russian e06s02_sex_caca1607:

    # lc "Yes! That way, right now, it feels way too empty. Even with [mcname] around."
    lc "Да! Сейчас я чувствую себя слишком одинокой. Даже когда рядом [mcname]."

# game/code/e06/e06s02.rpy:1169
translate russian e06s02_sex_fe320a36:

    # mc "And you'd never have to go so long without getting a taste of your favorite treat."
    mc "И тебе больше не придется подолгу обходиться без любимого лакомства."

# game/code/e06/e06s02.rpy:1173
translate russian e06s02_sex_e405fecc:

    # mes "Alright, you make a very convincing argument."
    mes "Ладно, вы привели очень убедительный аргумент."

# game/code/e06/e06s02.rpy:1176
translate russian e06s02_sex_f3fb6e46:

    # lc "Hehehe."
    lc "Хехе."

# game/code/e06/e06s02.rpy:1179
translate russian e06s02_sex_e8d5948f:

    # mes "Besides... someone has to keep you two out of trouble."
    mes "Кроме того... кто-то должен держать вас двоих подальше от неприятностей."

# game/code/e06/e06s02.rpy:1182
translate russian e06s02_sex_74c498aa:

    # mc "Sounds good to me."
    mc "По-моему, звучит неплохо."

# game/code/e06/e06s02.rpy:1185
translate russian e06s02_sex_b1cd4d51:

    # lc "What would I do without you two?"
    lc "Что бы я без вас делала?"

translate russian strings:

    # game/code/e06/e06s02.rpy:40
    old "The Next Day"
    new "На следующий день"

    # game/code/e06/e06s02.rpy:244
    old "Inside Min's house"
    new "Внутри дома Мин"

