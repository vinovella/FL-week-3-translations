# TODO: Translation updated at 2022-06-06 10:42

# game/code/d17/d17s08.rpy:49
translate russian d17s08_b0017cfc:

    # "Later that evening"
    "Позже тем же вечером."

# game/code/d17/d17s08.rpy:51
translate russian d17s08_15d395d8:

    # lc "I'm glad you could come over tonight. {w}I like sleeping with my boyfriend, even if I'm not {i}sleeping{/i} with my boyfriend."
    lc "Я рада, что ты смог прийти. {w}Мне нравится спать со своим парнем, даже если я не {i}сплю{/i} со своим парнем."

# game/code/d17/d17s08.rpy:53
translate russian d17s08_382fea90:

    # mc "I enjoy it too."
    mc "Мне это тоже нравится."

# game/code/d17/d17s08.rpy:55
translate russian d17s08_7aa1ce4b:

    # lc "I hope that doesn't upset you too much. We'll get there, I'm just not sure-"
    lc "Надеюсь, это тебя не слишком расстраивает. Когда-нибудь мы сделаем это, просто я пока не уверена..."

# game/code/d17/d17s08.rpy:58
translate russian d17s08_8d1a0b46:

    # mc "Don't worry about it. I love you, and I enjoy every moment we're together."
    mc "Не беспокойся об этом. Я люблю тебя, и я наслаждаюсь каждым моментом, когда мы вместе."

# game/code/d17/d17s08.rpy:60
translate russian d17s08_2c2aaf9f:

    # mc "Don't worry about it. I enjoy every moment we're together. We don't have to rush things."
    mc "Не беспокойся об этом. Я наслаждаюсь каждым моментом, когда мы вместе. Нам не нужно торопиться."

# game/code/d17/d17s08.rpy:62
translate russian d17s08_6b138f69:

    # lc "Can I ask you a weird question?"
    lc "Могу я задать тебе вопрос?"

# game/code/d17/d17s08.rpy:64
translate russian d17s08_1bf60051:

    # mc "Of course."
    mc "Конечно."

# game/code/d17/d17s08.rpy:66
translate russian d17s08_0f7db042:

    # lc "So, I was looking it up, and apparently I'm what they call a \"voyeur\". I like watching other people having sex."
    lc "Так вот, я посмотрела в интернете, и оказалось, что я та, кого называют \"вуайерист\". Мне нравится смотреть, как другие люди занимаются сексом."

# game/code/d17/d17s08.rpy:67
translate russian d17s08_9ed7cd70:

    # mc "Sure, I know what a voyeur is."
    mc "Конечно, я знаю, кто такой вуайерист."

# game/code/d17/d17s08.rpy:68
translate russian d17s08_39448b0e:

    # lc "Do you think that's what I am?"
    lc "Ты думаешь, я такая?"

# game/code/d17/d17s08.rpy:70
translate russian d17s08_126a07ec:

    # mc "If it walks like a duck and quacks like a duck, yeah. I'd say you're a voyeur."
    mc "Если бы ты ходила как утка и крякала как утка, то да. Я бы сказал, что ты вуайеристка."

# game/code/d17/d17s08.rpy:72
translate russian d17s08_06c4f335:

    # lc "Okay, so the other side of voyeur is an exhibitionist. Do you think of yourself as an exhibitionist?"
    lc "Ладно, а другая сторона вуайериста - эксгибиционист. Считаешь ли ты себя эксгибиционистом?"

# game/code/d17/d17s08.rpy:74
translate russian d17s08_6c0d3478:

    # mc "I mean, I like it when you're watching me fuck other people, but I wouldn't call myself an exhibitionist."
    mc "Ну да, мне нравится, когда ты смотришь, как я трахаюсь с другими, но я бы не назвал себя эксгибиционистом."

# game/code/d17/d17s08.rpy:76
translate russian d17s08_2579ed25:

    # lc "But you were with all those people on Sunday - especially mud wrestling in front of that audience."
    lc "Но ты был со всеми этими людьми на вечеринке в воскресенье - особенно когда боролся в грязи перед аудиторией."

# game/code/d17/d17s08.rpy:78
translate russian d17s08_2553d9ad:

    # mc "True. I don't have a problem with other people watching, but I was always focused on my partner."
    mc "Верно. У меня нет проблем с тем, что другие люди смотрят, но я всегда сосредоточен на своем партнере."

# game/code/d17/d17s08.rpy:80
translate russian d17s08_426091d1:

    # lc "Hmm. {w}So, what if I ran into some real exhibitionists on campus?"
    lc "Хмм. {w}Но что если я столкнусь с настоящими эксгибиционистами в кампусе?"

# game/code/d17/d17s08.rpy:82
translate russian d17s08_24383b45:

    # mc "Oh, well. I mean, I suppose I've been in that situation."
    mc "Ну что ж. Полагаю, что я уже был в такой ситуации."

# game/code/d17/d17s08.rpy:84
translate russian d17s08_957d88b3:

    # lc "Of course you have. I should've known."
    lc "Конечно, да. Я должна немедленно это услышать."

# game/code/d17/d17s08.rpy:88
translate russian d17s08_4d01cb71:

    # mc "I'm not going to say who I was with, because they might not want people to know."
    mc "Я не могу сказать, с кем я был, потому что тот человек, возможно, не захочет, чтобы другие узнали."

# game/code/d17/d17s08.rpy:90
translate russian d17s08_822cfe95:

    # lc "Oh?"
    lc "Да?"

# game/code/d17/d17s08.rpy:92
translate russian d17s08_d9113434:

    # mc "You'll understand in a minute. So, I was at the beach with someone a week or two ago."
    mc "Ты поймешь через минуту. Значит так, неделю или две назад я был с кое-кем на пляже."

# game/code/d17/d17s08.rpy:93
translate russian d17s08_ed0d2941:

    # mc "We were swimming - naked - having fun. Got out of the water, had a bit more fun on the sand."
    mc "Мы купались - голышом - и веселились. Выбрались из воды, и еще немного повеселились на песке."

# game/code/d17/d17s08.rpy:94
translate russian d17s08_37cc0643:

    # mc "We thought we were all alone until someone else walked by."
    mc "Мы думали, что совсем одни, пока кто-то не прошел мимо."

# game/code/d17/d17s08.rpy:96
translate russian d17s08_3e858d5a:

    # lc "O.M.G. Did you freak out?"
    lc "Ого. И что ты сделал?"

# game/code/d17/d17s08.rpy:98
translate russian d17s08_894f0408:

    # mc "I didn't, but the woman I was with got completely terrified."
    mc "Я ничего, но девушка, с которой я был, пришла в полный ужас."

# game/code/d17/d17s08.rpy:99
translate russian d17s08_6b69129b:

    # mc "She started thinking about her life, her career, her reputation, and all of that."
    mc "Она начала переживать о своей жизни, своей карьере, своей репутации и обо всем этом."

# game/code/d17/d17s08.rpy:101
translate russian d17s08_f27ad558:

    # lc "Oh, wow."
    lc "Ох, ничего себе."

# game/code/d17/d17s08.rpy:103
translate russian d17s08_1f67de6f:

    # mc "It was no big deal, really. The person who walked by is a friend of mine and wouldn't say anything to anyone."
    mc "На самом деле в этом не было ничего особенного. Человек, который проходил мимо, - мой друг, и он никому бы ничего не рассказал."

# game/code/d17/d17s08.rpy:104
translate russian d17s08_1c8641ed:

    # mc "Still, for the person I was with - the situation went from relaxed and freeing to scary in an instant."
    mc "Тем не менее, для человека, с которым я был, ситуация в одно мгновение превратилась из расслабленной в нервную."

# game/code/d17/d17s08.rpy:107
translate russian d17s08_a7b504b1:

    # mc "So, a couple of the girls{w} - sorry - women that I've been with have enjoyed taking risks like that in public."
    mc "Значит пара девушек, {w}с которыми я был, нравилось так рисковать на публике."

# game/code/d17/d17s08.rpy:108
translate russian d17s08_b48aef40:

    # mc "One of them loves to be seen. She intentionally flashes people and would run around bareass naked everywhere if she could."
    mc "Одна из них любит, чтобы её видели обнаженной. Она намеренно случайно оголяется перед людьми и ходила бы повсюду голышом, если бы могла."

# game/code/d17/d17s08.rpy:110
translate russian d17s08_45f000f5:

    # lc "Is that Polly?"
    lc "Это Полли?"

# game/code/d17/d17s08.rpy:112
translate russian d17s08_5df4435f:

    # mc "Yeah, how did you know?"
    mc "Да, как ты догадалась?"

# game/code/d17/d17s08.rpy:114
translate russian d17s08_c300e12c:

    # lc "She was here on Sunday."
    lc "Она была здесь в воскресенье."

# game/code/d17/d17s08.rpy:116
translate russian d17s08_49840bce:

    # mc "Oh, right. {w}Polly's girlfriend, on the other hand, isn't as comfortable with being seen in public."
    mc "Ох, верно. {w}С другой стороны, подруге Полли не будет так комфортно появляться на публике."

# game/code/d17/d17s08.rpy:118
translate russian d17s08_ff3faf4d:

    # lc "I didn't know she had a girlfriend. {w}Anyone I know?"
    lc "Я не знала, что у нее есть подруга. {w}Я ее знаю?"

# game/code/d17/d17s08.rpy:120
translate russian d17s08_9754d78a:

    # mc "Yes, but I'm not sure if they're public with their relationship yet."
    mc "Да, но я не уверен, что они уже обнародовали свои отношения."

# game/code/d17/d17s08.rpy:122
translate russian d17s08_b20a4127:

    # lc "Mysterious."
    lc "Загадочно."

# game/code/d17/d17s08.rpy:124
translate russian d17s08_532b6d10:

    # mc "Anyway, her girlfriend is pretty cool with doing risky things, but I'm almost certain she would freak the fuck out if we got caught."
    mc "В любом случае, ее подруга довольно спокойно относится к рискованным поступкам, но я уверен, что она разозлится, если нас поймают."

# game/code/d17/d17s08.rpy:128
translate russian d17s08_43beede3:

    # mc "I can't speak for everyone, but I can tell you what I did."
    mc "Я не могу говорить про других, но могу рассказать, что я делал."

# game/code/d17/d17s08.rpy:130
translate russian d17s08_a0e05134:

    # lc "Please do."
    lc "Пожалуйста, расскажи."

# game/code/d17/d17s08.rpy:132
translate russian d17s08_1f7d3857:

    # mc "So, the first day I got back from being sick - shortly after I found out about Fetish Locator..."
    mc "Значит так, в первый день, когда я вернулся из дома - вскоре после того, как узнал о Фетиш Локаторе..."

# game/code/d17/d17s08.rpy:133
translate russian d17s08_01bf0ad5:

    # mc "I was sitting there during class when Min ate Antony's cum right there in the front row."
    mc "Я сидел на уроке, и увидел как Мин съела сперму Энтони прямо передо мной, в первом ряду."

# game/code/d17/d17s08.rpy:135
translate russian d17s08_10eefcb6:

    # lc "She did what?!"
    lc "Что она сделала?!"

# game/code/d17/d17s08.rpy:137
translate russian d17s08_711264e3:

    # mc "She jerked him off under the desk, and then licked his cum off her hand."
    mc "Она дрочила ему под столом, а затем слизала его сперму со своей руки."

# game/code/d17/d17s08.rpy:139
translate russian d17s08_d504498b:

    # lc "During class?!"
    lc "Во время урока?!"

# game/code/d17/d17s08.rpy:141
translate russian d17s08_39aeb091:

    # mc "Yep. I'm pretty certain I was the only person that noticed at the time."
    mc "Ага. Я уверен, что был единственным человеком, который увидел это."

# game/code/d17/d17s08.rpy:143
translate russian d17s08_fc575ea8:

    # lc "How is that possible?"
    lc "Как это возможно?"

# game/code/d17/d17s08.rpy:145
translate russian d17s08_6ec69743:

    # mc "I was just sitting in the right place at the right time, I guess."
    mc "Наверное, я просто оказался в нужном месте в нужное время."

# game/code/d17/d17s08.rpy:147
translate russian d17s08_12dfdedf:

    # lc "Did they notice you watching?"
    lc "Они заметили, что ты наблюдаешь за ними?"

# game/code/d17/d17s08.rpy:149
translate russian d17s08_ff97718e:

    # mc "Nope. Class just went on like nothing had happened."
    mc "Нет. Урок просто продолжался, как ни в чем не бывало."

# game/code/d17/d17s08.rpy:151
translate russian d17s08_9a16fa05:

    # lc "Did you ever say anything to them about that?"
    lc "Ты потом не говорил им что-нибудь про это?"

# game/code/d17/d17s08.rpy:153
translate russian d17s08_b5672634:

    # mc "It didn't come up."
    mc "Не было повода."

# game/code/d17/d17s08.rpy:155
translate russian d17s08_c5d1a1c0:

    # lc "Not even after we all saw them together at Min's Party?"
    lc "Даже после того, как мы видели их вместе на вечеринке Мин?"

# game/code/d17/d17s08.rpy:157
translate russian d17s08_3b16a04a:

    # mc "That was kinda different. They knew they had an audience there."
    mc "Там было по-другому. Они знали, что у них есть зрители."

# game/code/d17/d17s08.rpy:160
translate russian d17s08_43beede3_1:

    # mc "I can't speak for everyone, but I can tell you what I did."
    mc "Я не могу говорить про других, но могу рассказать, что я делал."

# game/code/d17/d17s08.rpy:162
translate russian d17s08_a0e05134_1:

    # lc "Please do."
    lc "Пожалуйста, расскажи."

# game/code/d17/d17s08.rpy:164
translate russian d17s08_5e6b4c26:

    # mc "I was in the locker room getting dressed after my shower and heard some noises coming from the other side of the lockers."
    mc "Я был в раздевалке, одевался после душа и услышал какие-то звуки, доносящиеся с другой стороны шкафчиков."

# game/code/d17/d17s08.rpy:165
translate russian d17s08_c7a05202:

    # mc "It turned out to be a couple of people fucking."
    mc "Оказалось, что там трахалась парочка."

# game/code/d17/d17s08.rpy:167
translate russian d17s08_92c2a729:

    # lc "Oh, wow. Did they notice you?"
    lc "Ох, ничего себе. Они заметили тебя?"

# game/code/d17/d17s08.rpy:169
translate russian d17s08_558d79f6:

    # mc "Nope. I just went about my business and then left the locker room without interrupting them."
    mc "Нет. Я просто занялся своими делами, а затем вышел из раздевалки, не мешая им."

# game/code/d17/d17s08.rpy:171
translate russian d17s08_5bc49217:

    # lc "Wow. I'm not sure I could do that. {w}I'd have to either run away or stay & watch."
    lc "Вау. Я не уверена, что смогла бы это сделать. {w}Я бы убежала, либо осталась и смотрела."

# game/code/d17/d17s08.rpy:173
translate russian d17s08_a649c4ff:

    # mc "Anyway, I realized I had forgotten something - I think it was my phone - so I went back for it."
    mc "Как бы то ни было, я заметил, что кое-что забыл - кажется, это был мой телефон, - и вернулся за ним."

# game/code/d17/d17s08.rpy:175
translate russian d17s08_96b530c9:

    # lc "Did you-?"
    lc "И ты...?"

# game/code/d17/d17s08.rpy:177
translate russian d17s08_01c3c599:

    # mc "No, but a friend of mine was standing there and jerking off watching them."
    mc "Нет, но мой друг стоял там и дрочил, наблюдая за ними."

# game/code/d17/d17s08.rpy:179
translate russian d17s08_ee882241:

    # lc "Did that bother them at all?"
    lc "Их это не беспокоило?"

# game/code/d17/d17s08.rpy:181
translate russian d17s08_5f912cb7:

    # mc "Not at all. I found out later that was his kink. He liked watching that girl having sex with other people and she enjoyed having him watch."
    mc "Нисколько. Позже я узнал, что это был его фетиш. Ему нравится смотреть, как эта девушка занимается сексом с другими парнями, а ей нравится, когда он смотрит."

# game/code/d17/d17s08.rpy:183
translate russian d17s08_365b7d81:

    # lc "So, it sounds like I shouldn't watch people - even if they are in public - unless I'm sure they're okay with me being there."
    lc "Значит мне не следует наблюдать за людьми - даже если они на публике, - если я не уверена, что они не против моего присутствия там."

# game/code/d17/d17s08.rpy:185
translate russian d17s08_5ebfe654:

    # mc "Sounds about right."
    mc "Примерно так."

# game/code/d17/d17s08.rpy:187
translate russian d17s08_8d8099b9:

    # lc "But then, why would they do something like that in the first place?"
    lc "Но зачем им вообще делать что-то подобное?"

# game/code/d17/d17s08.rpy:189
translate russian d17s08_6ad233fc:

    # mc "Oh, hmm.{w}.. well, I think the thrill for most exhibitionists is the chance of getting caught."
    mc "Хмм.{w}.. ну, я думаю, что удовольствие, которое испытывают большинство эксгибиционистов - это риск быть пойманным."

# game/code/d17/d17s08.rpy:190
translate russian d17s08_0949eed3:

    # mc "I don't think they actually want to be caught. They just want that risk they might be."
    mc "Я не думаю, что они на самом деле хотят, чтобы их поймали. Они просто хотят ощутить этот риск."

# game/code/d17/d17s08.rpy:193
translate russian d17s08_cc19cac6:

    # lc "Could I watch discreetly while I'm pretending to do something else?"
    lc "А можно незаметно наблюдать, если притворится, что занята чем-то своим?"

# game/code/d17/d17s08.rpy:195
translate russian d17s08_ac50f58f:

    # mc "You sound like this isn't just a fantasy."
    mc "Ты говоришь так, словно это не просто фантазия."

# game/code/d17/d17s08.rpy:197
translate russian d17s08_16dd4f34:

    # lc "Maybe I have a neighbor that doesn't always close their curtains."
    lc "Может у меня есть сосед, который не всегда закрывает свои шторы."

# game/code/d17/d17s08.rpy:198
translate russian d17s08_ee87e6f3:

    # mc "Oh?"
    mc "Да?"

# game/code/d17/d17s08.rpy:200
translate russian d17s08_6952e266:

    # lc "Anyway, I'm going to get a shower before we go to bed. I'll be back."
    lc "В любом случае, мне нужно принять душ, прежде чем мы ляжем спать. Я скоро вернусь."

# game/code/d17/d17s08.rpy:202
translate russian d17s08_a49ed4f8:

    # mc "Okay."
    mc "Хорошо."

# game/code/d17/d17s08.rpy:209
translate russian replay_d17s08_d9fc70c6:

    # mc "Stacy?"
    mc "Стейси?"

# game/code/d17/d17s08.rpy:211
translate russian replay_d17s08_f453bbd3:

    # sy "I'm creeping."
    sy "Я тихонечко."

# game/code/d17/d17s08.rpy:212
translate russian replay_d17s08_9d83a8e2:

    # mc "What are you doing here?"
    mc "Что ты здесь делаешь?"

# game/code/d17/d17s08.rpy:214
translate russian replay_d17s08_39848f85:

    # sy "I need to get the data off your {i}USB device{/i}."
    sy "Мне нужно получить данные с твоего {i}USB-девайса{/i}."

# game/code/d17/d17s08.rpy:216
translate russian replay_d17s08_60740d9f:

    # mc "But, I mean, how are you here?"
    mc "Нет, я имею в виду, как ты здесь оказалась?"

# game/code/d17/d17s08.rpy:218
translate russian replay_d17s08_3fc417d4:

    # sy "I let myself in."
    sy "Я вошла в дверь."

# game/code/d17/d17s08.rpy:220
translate russian replay_d17s08_1c43d221:

    # mc "I'm certain the door was locked."
    mc "Я уверен, что дверь была заперта."

# game/code/d17/d17s08.rpy:222
translate russian replay_d17s08_dc931c83:

    # sy "And it will be locked again after I leave."
    sy "И она снова будет заперта после того, как я уйду."

# game/code/d17/d17s08.rpy:224
translate russian replay_d17s08_11455b53:

    # mc "What?"
    mc "Что?"

# game/code/d17/d17s08.rpy:225
translate russian replay_d17s08_f2d3d375:

    # sy "Just give me the device."
    sy "Просто отдай мне устройство."

# game/code/d17/d17s08.rpy:227
translate russian replay_d17s08_ba347f77:

    # mc "Uh, sure."
    mc "Ух, конечно."

# game/code/d17/d17s08.rpy:229
translate russian replay_d17s08_ee35b364:

    # sy "How long does your girlfriend take in the shower?"
    sy "Как долго твоя девушка принимает душ?"

# game/code/d17/d17s08.rpy:231
translate russian replay_d17s08_e8f7b771:

    # mc "I mean, she's a girl, so..."
    mc "Ну, она же девушка, так что..."

# game/code/d17/d17s08.rpy:233
translate russian replay_d17s08_91362fe5:

    # sy "Hmm. I don't take very long in the shower."
    sy "Хмм. Я не очень долго нахожусь в душе."

# game/code/d17/d17s08.rpy:235
translate russian replay_d17s08_1f519986:

    # mc "Yeah, but she's..."
    mc "Да, но она..."

# game/code/d17/d17s08.rpy:236
translate russian replay_d17s08_315d7bee:

    # mc "Probably 30 minutes to an hour?"
    mc "Наверное, от 30 минут до часа."

# game/code/d17/d17s08.rpy:238
translate russian replay_d17s08_06cc84b6:

    # sy "That should be plenty of time for the data transfer."
    sy "Этого должно быть достаточно для передачи данных."

# game/code/d17/d17s08.rpy:240
translate russian replay_d17s08_bc4fd44d:

    # mc "Oh, okay."
    mc "О, ладно."

# game/code/d17/d17s08.rpy:244
translate russian replay_d17s08_a637bdbf:

    # sy "Actually... that should also be plenty of time for a little fun."
    sy "Вообще-то... тут достаточно времени даже для небольшого веселья."

# game/code/d17/d17s08.rpy:247
translate russian replay_d17s08_04e7cdbb:

    # mc "I mean, we could just wait for Lydia to get back if you want-"
    mc "Ну мы могли бы просто подождать возвращения Лидии, если ты хочешь..."

# game/code/d17/d17s08.rpy:249
translate russian replay_d17s08_f1c647ff:

    # mc "Yeah, probably don't want Lydia to know about us just yet."
    mc "Да, наверное, я пока не хочу, чтобы Лидия узнала о нас."

# game/code/d17/d17s08.rpy:251
translate russian replay_d17s08_ef44867b:

    # sy "Shhh. {w}We're being sneaky."
    sy "Тшшш. {w}Мы действуем скрытно."

# game/code/d17/d17s08.rpy:253
translate russian replay_d17s08_bc4fd44d_1:

    # mc "Oh, okay."
    mc "О, ладно."

# game/code/d17/d17s08.rpy:255
translate russian replay_d17s08_36cdf563:

    # sy "This looks delicious."
    sy "Он выглядит восхитительно."

# game/code/d17/d17s08.rpy:257
translate russian replay_d17s08_ebfe863e:

    # mc "I'll take your word for it."
    mc "Поверю тебе на слово."

# game/code/d17/d17s08.rpy:259
translate russian replay_d17s08_d9d0b6cc:

    # sy "Let me show you."
    sy "Позволь мне показать."

# game/code/d17/d17s08.rpy:277
translate russian replay_d17s08_0dbea11a:

    # sy "How was that?"
    sy "Как тебе?"

# game/code/d17/d17s08.rpy:279
translate russian replay_d17s08_dfe3b92b:

    # mc "Your mouth is amazing."
    mc "У тебя потрясающий рот."

# game/code/d17/d17s08.rpy:281
translate russian replay_d17s08_ece54638:

    # sy "Just my mouth?"
    sy "Только рот?"

# game/code/d17/d17s08.rpy:284
translate russian replay_d17s08_d81ea2ae:

    # mc "You are absolutely amazing, sis."
    mc "Ты вся потрясающая, сестренка."

# game/code/d17/d17s08.rpy:286
translate russian replay_d17s08_1e0a0800:

    # mc "You are absolutely amazing."
    mc "Ты вся потрясающая."

# game/code/d17/d17s08.rpy:288
translate russian replay_d17s08_6f9707dc:

    # sy "Damn right."
    sy "Чертовски верно."

# game/code/d17/d17s08.rpy:289
translate russian replay_d17s08_7e66a919:

    # mc "You're a fucking goddess."
    mc "Ты гребаная богиня."

# game/code/d17/d17s08.rpy:290
translate russian replay_d17s08_c0c2337e:

    # sy "You said it."
    sy "Ты сам это сказал."

# game/code/d17/d17s08.rpy:292
translate russian replay_d17s08_04478ac2:

    # mc "Just a little bit more..."
    mc "Еще чуть-чуть..."

# game/code/d17/d17s08.rpy:293
translate russian replay_d17s08_76346d47:

    # sy "Only if you promise to come down my throat."
    sy "Только если пообещаешь кончить мне в горло."

# game/code/d17/d17s08.rpy:295
translate russian replay_d17s08_33d2db8d:

    # mc "You want me to fuck your face?"
    mc "Ты хочешь, чтобы я трахнул тебя в горло?"

# game/code/d17/d17s08.rpy:296
translate russian replay_d17s08_b1d2190a:

    # sy "No. I want you to cum in my mouth so intensely that it shoots down my throat."
    sy "Нет. Я хочу, чтобы ты кончил мне в рот так сильно, чтобы залетело прямо в горло."

# game/code/d17/d17s08.rpy:297
translate russian replay_d17s08_36552c30:

    # mc "Oh, fuck yes."
    mc "Ох, черт, да."

# game/code/d17/d17s08.rpy:299
translate russian replay_d17s08_1f5aa9f1:

    # mc "What a fucking goddess you are..."
    mc "Какая же ты, бля, богиня..."

# game/code/d17/d17s08.rpy:301
translate russian replay_d17s08_c1dff962:

    # mc "Oh..."
    mc "Ох..."

# game/code/d17/d17s08.rpy:302
translate russian replay_d17s08_715b63e4:

    # mc "OH-"
    mc "ОХ..."

# game/code/d17/d17s08.rpy:306
translate russian replay_d17s08_cff9e1b2:

    # mc "OH FUCK YESSS"
    mc "ОХ, БЛЯ ДААА!"

# game/code/d17/d17s08.rpy:308
translate russian replay_d17s08_69bea1a5:

    # mc "That's-"
    mc "Это..."

# game/code/d17/d17s08.rpy:309
translate russian replay_d17s08_d64a2696:

    # mc "That's fucking terrific."
    mc "Это чертовски потрясающе."

# game/code/d17/d17s08.rpy:310
translate russian replay_d17s08_8678e9c7:

    # mc "How did you get so good at that?"
    mc "Где ты так научилась?"

# game/code/d17/d17s08.rpy:312
translate russian replay_d17s08_01f0c2ac:

    # sy "Research."
    sy "Изучила."

# game/code/d17/d17s08.rpy:314
translate russian replay_d17s08_0a7ca4c7:

    # mc "Research?"
    mc "Изучила?"

# game/code/d17/d17s08.rpy:316
translate russian replay_d17s08_fd1425ca:

    # sy "I had six months to go through your porn box."
    sy "У меня было шесть месяцев, чтобы просмотреть твою коробку с порнографией."

# game/code/d17/d17s08.rpy:318
translate russian replay_d17s08_7488ae3e:

    # mc "Oh yeah."
    mc "Ах, да."

# game/code/d17/d17s08.rpy:320
translate russian replay_d17s08_e3c7e8fb:

    # sy "I know exactly what you want and how to give it to you."
    sy "Я точно знаю, чего ты хочешь и как тебе это дать."

# game/code/d17/d17s08.rpy:322
translate russian replay_d17s08_9270f444:

    # mc "You are incredible."
    mc "Ты невероятна."

# game/code/d17/d17s08.rpy:326
translate russian replay_d17s08_40ba64dd:

    # sy "Oh, look at that, we have got [d17s08_points] points for..."
    sy "О, посмотри, нам дали [d17s08_points] очков за..."

# game/code/d17/d17s08.rpy:329
translate russian replay_d17s08_386a2729:

    # sy "So, how's things?"
    sy "Ну, как дела?"

# game/code/d17/d17s08.rpy:331
translate russian replay_d17s08_7187a437:

    # mc "Um... good?"
    mc "Эм... хорошо?"

# game/code/d17/d17s08.rpy:333
translate russian replay_d17s08_f10db3de:

    # sy "You own clothes, don't you?"
    sy "У тебя есть своя одежда, верно?"

# game/code/d17/d17s08.rpy:335
translate russian replay_d17s08_fcaf3220:

    # mc "It was just my girlfriend & I.{w}.. I was getting ready to sleep."
    mc "Тут только я и моя девушка.{w}.. Я готовился ко сну."

# game/code/d17/d17s08.rpy:337
translate russian replay_d17s08_4d7f0c0b:

    # sy "I'm not complaining. I'm just saying, after Sunday..."
    sy "Я не жалуюсь. Просто говорю, что после воскресенья..."

# game/code/d17/d17s08.rpy:339
translate russian replay_d17s08_e46b7572:

    # mc "You and AmRose stole my clothes on Sunday."
    mc "Ты с АмРоуз украла мою одежду в воскресенье."

# game/code/d17/d17s08.rpy:341
translate russian replay_d17s08_9af6b524:

    # sy "Are you even allowed to wear clothing in this house?"
    sy "Тебе вообще разрешено носить одежду в этом доме?"

# game/code/d17/d17s08.rpy:343
translate russian replay_d17s08_2f5731d3:

    # mc "Yes. I am allowed to wear clothing."
    mc "Да. Мне разрешено носить одежду."

# game/code/d17/d17s08.rpy:345
translate russian replay_d17s08_bddb1774:

    # mc "*mumbles* probably"
    mc "*бормочет* Наверное."

# game/code/d17/d17s08.rpy:347
translate russian replay_d17s08_0acdabbb:

    # sy "Alright. I'll give you the benefit of the doubt."
    sy "Хорошо. Я дам тебе преимущество "

# game/code/d17/d17s08.rpy:349
translate russian replay_d17s08_afad4af1:

    # mc "So, when did you modify your phone to download data on-?"
    mc "Значит ты модифицировала свой телефон для загрузки данных с...?"

# game/code/d17/d17s08.rpy:351
translate russian replay_d17s08_a49d7f40:

    # sy "Finding Mira? Oh, earlier today."
    sy "Найти Миру? О, еще сегодня с утра."

# game/code/d17/d17s08.rpy:352
translate russian replay_d17s08_ccdf5709:

    # mc "Mira?"
    mc "Мира?"

# game/code/d17/d17s08.rpy:353
translate russian replay_d17s08_ac4ccd5d:

    # sy "That shiba inu we're trying to find. You know the one I'm talking about."
    sy "Та, которую мы пытаемся найти. Ты знаешь, о ком я говорю."

# game/code/d17/d17s08.rpy:355
translate russian replay_d17s08_60658749:

    # mct "Oh, right. Retention might be listening."
    mct "Ох, точно. Локатор же подслушивает."

# game/code/d17/d17s08.rpy:356
translate russian replay_d17s08_5850c7f6:

    # mc "Right, that lost... dog. I didn't realize it had a name."
    mc "Верно, та... собака. Я не знал, что у нее есть имя."

# game/code/d17/d17s08.rpy:358
translate russian replay_d17s08_be98a638:

    # sy "Don't worry. We'll find it."
    sy "Не волнуйся. Мы найдем ее."

# game/code/d17/d17s08.rpy:360
translate russian replay_d17s08_d289b245:

    # mc "So, is there any progress on finding... Mira?"
    mc "Есть ли прогресс в поиске... Миры?"

# game/code/d17/d17s08.rpy:362
translate russian replay_d17s08_4bd7bded:

    # sy "Well, the data transfer goes both ways. There's an update to the search software."
    sy "Передача данных идет полным ходом. У нас вышло обновление поискового программного обеспечения."

# game/code/d17/d17s08.rpy:363
translate russian replay_d17s08_c7ca3906:

    # mc "That sounds nice."
    mc "Звучит многообещающе."

# game/code/d17/d17s08.rpy:365
translate russian replay_d17s08_864e5c06:

    # sy "Hopefully we'll find Mira tomorrow."
    sy "Надеюсь, уже завтра мы найдем эту Миру."

# game/code/d17/d17s08.rpy:368
translate russian replay_d17s08_a1289911:

    # mc "Uh, sis. My girlfriend's coming this way."
    mc "Ух, сестренка. Моя девушка идет сюда."

# game/code/d17/d17s08.rpy:370
translate russian replay_d17s08_242324f2:

    # mc "Uh, Stacy. Lydia's coming this way."
    mc "Ух, Стейси. Лидия идет сюда."

# game/code/d17/d17s08.rpy:372
translate russian replay_d17s08_96f753dd:

    # mc "Hey sexy lady."
    mc "Привет, сексуальная леди."

# game/code/d17/d17s08.rpy:373
translate russian replay_d17s08_63a926ed:

    # lc "Hey yourself, naked man."
    lc "И тебе привет, голый парниша!"

# game/code/d17/d17s08.rpy:376
translate russian replay_d17s08_2376c239:

    # lc "Oh good, you jerked off while I was in the shower."
    lc "О, хорошо, ты подрочил, пока я была в душе."

# game/code/d17/d17s08.rpy:378
translate russian replay_d17s08_59332753:

    # lc "Not that I didn't want to be here when you came, but I'm tired."
    lc "Не то чтобы я не хотела быть здесь, когда ты кончил, но я устала."

# game/code/d17/d17s08.rpy:380
translate russian replay_d17s08_653e0ff8:

    # lc "Damn, you're so fucking sexy."
    lc "Проклятье, ты так чертовски сексуален."

# game/code/d17/d17s08.rpy:382
translate russian replay_d17s08_dfc6e236:

    # lc "If I wasn't so fucking tired I would throw you on the bed and have my way with you."
    lc "Если бы я не была так чертовски уставшей, я бы бросила тебя на кровать и занялась тобой."

# game/code/d17/d17s08.rpy:384
translate russian replay_d17s08_8280a992:

    # lc "Wow. You are so incredibly sexy right now."
    lc "Вау. Ты сейчас так невероятно сексуален."

# game/code/d17/d17s08.rpy:386
translate russian replay_d17s08_61f2fd92:

    # lc "If I wasn't so tired I would do such horrible things with you."
    lc "Если бы я не была такой уставшей, я бы сделала с тобой самые ужасные вещи."

# game/code/d17/d17s08.rpy:388
translate russian replay_d17s08_64559936:

    # mc "Too much time studying for finals?"
    mc "Слишком много времени уделяешь подготовке к экзаменам?"

# game/code/d17/d17s08.rpy:390
translate russian replay_d17s08_8e9ab5b6:

    # lc "You have no idea how much my throat hurts from singing practice."
    lc "Ты даже не представляешь, как сильно у меня болит горло от занятий пением."

# game/code/d17/d17s08.rpy:392
translate russian replay_d17s08_bac69fdd:

    # lc "Did you hear-?"
    lc "Ты слышал...?"

# game/code/d17/d17s08.rpy:400
translate russian replay_d17s08_daeb626d:

    # lc "What was that for?"
    lc "Зачем это?"

# game/code/d17/d17s08.rpy:401
translate russian replay_d17s08_6195c941:

    # mc "I was just overwhelmed with the desire to kiss you."
    mc "Меня просто переполняло желание поцеловать тебя."

# game/code/d17/d17s08.rpy:403
translate russian replay_d17s08_e8044916:

    # lc "I like that."
    lc "Мне это понравилось."

# game/code/d17/d17s08.rpy:405
translate russian replay_d17s08_43ddfa89:

    # mc "So, you were telling me about your neighbor?"
    mc "Так, ты рассказывала о своем соседе?"

# game/code/d17/d17s08.rpy:407
translate russian replay_d17s08_9f4d732b:

    # lc "Oh, you remembered that."
    lc "Ох, ты запомнил."

# game/code/d17/d17s08.rpy:408
translate russian replay_d17s08_cc9c6425:

    # mc "Was I supposed to forget about it?"
    mc "А я должен был забыть об этом?"

# game/code/d17/d17s08.rpy:410
translate russian replay_d17s08_3cc7853c:

    # lc "If you don't mind... I'll tell you about it some other time."
    lc "Если не возражаешь... То я расскажу об этом в другой раз."

# game/code/d17/d17s08.rpy:411
translate russian replay_d17s08_3270b79d:

    # mc "I dunno..."
    mc "Даже не знаю..."

# game/code/d17/d17s08.rpy:412
translate russian replay_d17s08_da60c4c6:

    # lc "Yeah?"
    lc "Что?"

# game/code/d17/d17s08.rpy:414
translate russian replay_d17s08_7cad9df1:

    # mc "It seems like you've been cheating on me by watching other people..."
    mc "Похоже, ты изменила мне, наблюдая за другим парнем..."

# game/code/d17/d17s08.rpy:416
translate russian replay_d17s08_ffcf64bc:

    # lc "Ha! It's not like that-"
    lc "Ха! Это не так..."

# game/code/d17/d17s08.rpy:418
translate russian replay_d17s08_442b70ed:

    # mc "I might have to punish you for not telling me about it."
    mc "Возможно, мне придется наказать тебя за то, что ты не рассказала мне об этом."

# game/code/d17/d17s08.rpy:420
translate russian replay_d17s08_6274a69b:

    # lc "Okay, okay! {w}I promise to make it up to you in the morning."
    lc "Ладно, ладно! {w}Я обещаю загладить свою вину утром."

# game/code/d17/d17s08.rpy:421
translate russian replay_d17s08_5e2b3f7d:

    # mc "I hope so."
    mc "Надеюсь, что так и будет."

# game/code/d17/d17s08.rpy:423
translate russian replay_d17s08_8c716c86:

    # lc "Good night, my lovely."
    lc "Спокойной ночи, моя прелесть."

# game/code/d17/d17s08.rpy:425
translate russian replay_d17s08_7eb2a637:

    # mc "Rest well, my love."
    mc "Отдыхай хорошо, любовь моя."

# game/code/d17/d17s08.rpy:427
translate russian replay_d17s08_a5dc5d88:

    # mc "*mumbles* These women are going to be the end of me."
    mc "*бормочет* Эти женщины сведут меня в могилу."

# game/code/d17/d17s08.rpy:428
translate russian replay_d17s08_e18b2360:

    # lc "What?!"
    lc "Что?!"

# game/code/d17/d17s08.rpy:429
translate russian replay_d17s08_05500ff6:

    # mc "Huh?"
    mc "Что?"

# game/code/d17/d17s08.rpy:430
translate russian replay_d17s08_f53778bb:

    # lc "What did you say?"
    lc "Что ты сказал?"

# game/code/d17/d17s08.rpy:431
translate russian replay_d17s08_4ccba40f:

    # mc "Rest well, beautiful."
    mc "Отдыхай хорошо, красавица."

# game/code/d17/d17s08.rpy:432
translate russian replay_d17s08_89c46f33:

    # lc "Oh. {w}Thank you."
    lc "А. {w}Спасибо."

# game/code/d17/d17s08.rpy:434
translate russian replay_d17s08_4ccba40f_1:

    # mc "Rest well, beautiful."
    mc "Отдыхай хорошо, красавица."

# game/code/d17/d17s08.rpy:436
translate russian replay_d17s08_3e257e18:

    # lc "Oh, remind me in the morning..."
    lc "Ох, напомни мне утром..."

# game/code/d17/d17s08.rpy:437
translate russian replay_d17s08_c17ef8ec:

    # mc "What's that?"
    mc "О чем?"

# game/code/d17/d17s08.rpy:438
translate russian replay_d17s08_78cf9394:

    # lc "I want you to help me create my Fetish Locator account."
    lc "Я хочу, чтобы ты помог мне создать аккаунт на Фетиш Локаторе."

# game/code/d17/d17s08.rpy:440
translate russian replay_d17s08_55d60f89:

    # mct "Oh, crap."
    mct "Вот дерьмо."

# TODO: Translation updated at 2022-08-24 12:18

# game/code/d17/d17s08.rpy:360
translate russian replay_d17s08_46a1f2b0:

    # mct "Oh, right. Fetish Locator might be listening."
    mct "Ох, точно. Локатор же подслушивает."

