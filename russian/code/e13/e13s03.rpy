# TODO: Translation updated at 2022-08-06 21:12

# game/code/e13/e13s03.rpy:30
translate russian e13s03_9a2dcee3:

    # nk "[mcname], I think my water broke."
    nk "[mcname], кажется у меня отошли воды."

# game/code/e13/e13s03.rpy:33
translate russian e13s03_ad09b856:

    # mc "Shit."
    mc "Черт!"

# game/code/e13/e13s03.rpy:34
translate russian e13s03_7f2e7742:

    # mc "What is the time?"
    mc "Сколько сейчас времени?"

# game/code/e13/e13s03.rpy:37
translate russian e13s03_336a25ad:

    # "Jim" "It's 12:49."
    "Джим" "Сейчас 12:49."

# game/code/e13/e13s03.rpy:40
translate russian e13s03_d6d7ffb9:

    # mc "Okay, come on. We need to get to the hospital. Do you feel any con—"
    mc "Ладно, поехали. Нам нужно попасть в больницу. Ты чувствуешь какие—нибудь ..."

# game/code/e13/e13s03.rpy:46
translate russian e13s03_d464a456:

    # nk "{i}Yep{/i}. She's coming."
    nk "{i}Да{/i}. Роды начинаются."

# game/code/e13/e13s03.rpy:49
translate russian e13s03_bce47427:

    # mc "Alright, let's go."
    mc "Хорошо, поехали."

# game/code/e13/e13s03.rpy:52
translate russian e13s03_082f7386:

    # mc "Jim, close down the cafe and look after it while we're gone. I'll give you a call when I get to the hospital."
    mc "Джим, закрой кафе и присматривай за ним, пока нас не будет. Я позвоню тебе, когда приеду в больницу."

# game/code/e13/e13s03.rpy:55
translate russian e13s03_e99cc41f:

    # "Jim" "Aye, aye, Captain. Good luck, Nora!"
    "Джим" "Есть капитан. Удачи, Нора!"

# game/code/e13/e13s03.rpy:58
translate russian e13s03_71e1c3e4:

    # nk "Thanks."
    nk "Спасибо."

# game/code/e13/e13s03.rpy:82
translate russian e13s03_after_clip_887df9c7:

    # "Doctor" "Mr. Young? [mcname] Young?"
    "Doctor" "Мистер Янг? [mcname] Янг?"

# game/code/e13/e13s03.rpy:85
translate russian e13s03_after_clip_7c927b78:

    # mc "Yes?"
    mc "Да?"

# game/code/e13/e13s03.rpy:88
translate russian e13s03_after_clip_aacd8544:

    # "Doctor" "Congratulations, you have one healthy baby boy!"
    "Doctor" "Поздравляю, у вас родился здоровый мальчик!"

translate russian strings:

    # game/code/e13/e13s03.rpy:6
    old "A month later"
    new "Месяц спустя"

