# TODO: Translation updated at 2023-05-27 13:27

# game/code/d21/d21s07.rpy:48
translate russian d21s07_84c0422b:

    # lc "That was when my boyfriend-"
    lc "Это произошло, когда мой парень..."

# game/code/d21/d21s07.rpy:51
translate russian d21s07_81f21ce2:

    # "Defense Attorney" "That would be [mcname] Young?"
    "Адвокат" "Это был [mcname] Янг?"

# game/code/d21/d21s07.rpy:58
translate russian d21s07_90cecf9a:

    # lc "Yes, we were taking a shower together and [mcname] Young told me-"
    lc "Да, мы вместе принимали душ, и [mcname] Янг рассказал мне..."

# game/code/d21/d21s07.rpy:62
translate russian d21s07_b81bdb87:

    # mc "Um, sorry."
    mc "Прошу прощения."

# game/code/d21/d21s07.rpy:65
translate russian d21s07_fc5b685e:

    # "Defense Attorney" "Please continue, Miss Cox."
    "Адвокат" "Пожалуйста, продолжайте, мисс Кокс."

# game/code/d21/d21s07.rpy:68
translate russian d21s07_81ca21c3:

    # lc "Yes, of course. That was [mcname] Young told me about Fetish Locator's dark secret."
    lc "Да, конечно. Именно [mcname] Янг рассказал мне о темном секрете Фетиш Локатора."

# game/code/d21/d21s07.rpy:71
translate russian d21s07_1a571b26:

    # "Defense Attorney" "And what was that secret, in your opinion?"
    "Адвокат" "В чем заключался этот секрет, по вашему мнению?"

# game/code/d21/d21s07.rpy:74
translate russian d21s07_dbaad169:

    # mc "*whispers* Sorry I'm late. What did I miss?"
    mc "*шепотом* Прости, я опоздал. Что я пропустил?"

# game/code/d21/d21s07.rpy:77
translate russian d21s07_34bbcff1:

    # hr "*whispers* Your girlfriend is up there crying crocodile tears. A jury might have bought it, but the judge won't fall for that."
    hr "*шепотом* Твоя девушка стоит там и льет крокодиловы слезы. Присяжные, может, и купились бы на это, но судья на это не поведется."

# game/code/d21/d21s07.rpy:82
translate russian d21s07_ad83d3c7:

    # mc "*whispers* I think we broke up. If we were ever really dating..."
    mc "*шепотом* Думаю, мы расстались. Если мы действительно встречались..."

# game/code/d21/d21s07.rpy:84
translate russian d21s07_3c3adf41:

    # mc "*whispers* Did it ever occur to you that she might be telling the truth - from her point of view."
    mc "*шепотом* Тебе не приходило в голову, что она может говорить правду - с ее точки зрения."

# game/code/d21/d21s07.rpy:87
translate russian d21s07_4e0b18b9:

    # hr "*whispers* Whatever."
    hr "*шепотом* Неважно."

# game/code/d21/d21s07.rpy:90
translate russian d21s07_95880b2e:

    # jdg "I will ask once for silence in this courtroom. If I have to ask again I will have the bailiff remove the audience."
    jdg "Прошу тишины в этом зале суда в первый раз. Если мне придется просить еще раз, я попрошу судебного пристава удалить зрителей."

# game/code/d21/d21s07.rpy:93
translate russian d21s07_45e8d8e6:

    # "Bailiff" "SILENCE IN THE COURTROOM."
    "Судебный пристав" "ТИШИНА В ЗАЛЕ СУДА."

# game/code/d21/d21s07.rpy:96
translate russian d21s07_a7c6db06:

    # jdg "Thank you. Please continue."
    jdg "Спасибо. Пожалуйста, продолжайте."

# game/code/d21/d21s07.rpy:99
translate russian d21s07_783c2174:

    # "Defense Attorney" "I think that is sufficient. No further questions, your honor."
    "Адвокат" "Думаю, этого достаточно. Больше вопросов нет, ваша честь."

# game/code/d21/d21s07.rpy:102
translate russian d21s07_584aa10e:

    # lc "Does that mean I can leave?"
    lc "Значит ли это, что я могу уйти?"

# game/code/d21/d21s07.rpy:105
translate russian d21s07_0defe80f:

    # jdg "Remain seated.{w} Prosecutor, would you like to cross examine the witness?"
    jdg "Оставайтесь на прежнем месте.{w} Прокурор, хотите ли вы провести перекрестный допрос свидетеля?"

# game/code/d21/d21s07.rpy:111
translate russian d21s07_c3045b05:

    # "Prosecutor" "I would, your honor.{w} Miss Cox, you testified that you did not know what Mister Skinner was doing."
    "Прокурор" "Да, ваша честь. Мисс Кокс, вы дали показания, что не знали, что делал мистер Скиннер."

# game/code/d21/d21s07.rpy:114
translate russian d21s07_ff765414:

    # lc "Who?"
    lc "Кто?"

# game/code/d21/d21s07.rpy:117
translate russian d21s07_72931960:

    # "Prosecutor" "Jerome Skinner. Your co-defendant and the man you claim was the mastermind behind \"Fetish Locator\"."
    "Прокурор" "Джером Скиннер. Ваш сообщник и человек, который, как вы утверждаете, был организатором \"Фетиш Локатор\"."

# game/code/d21/d21s07.rpy:120
translate russian d21s07_498a6091:

    # lc "Oh, correct. He created and controlled that horrible app without my knowledge or consent."
    lc "Да, все верно. Он создал и контролировал это ужасное приложение без моего ведома или согласия."

# game/code/d21/d21s07.rpy:123
translate russian d21s07_286931f9:

    # "Prosecutor" "So, you had no reason to be in the campus server room which happened to administer that app?"
    "Прокурор" "Значит, у вас не было причин находиться в серверной комнате кампуса, где находилось это приложение?"

# game/code/d21/d21s07.rpy:126
translate russian d21s07_0924a76a:

    # lc "No, of course not.{w} I'm a music major."
    lc "Нет, конечно, нет. Я занимаюсь музыкой."

# game/code/d21/d21s07.rpy:129
translate russian d21s07_250fb5b5:

    # "Prosecutor" "So, you did not gain the access codes to that room a semester prior to the \"app's\" launch - as a prior witness testified?"
    "Прокурор" "Значит, вы не получали коды доступа в эту комнату за семестр до запуска \"приложения\", как показал предыдущий свидетель?"

# game/code/d21/d21s07.rpy:132
translate russian d21s07_66da9489:

    # lc "I have no recollection of that."
    lc "Я об этом ничего не знаю."

# game/code/d21/d21s07.rpy:135
translate russian d21s07_13fd1863:

    # "Prosecutor" "And on that day when Mister Young informed you - in the shower - about Fetish Locator's nefarious actions...{w} You were not present in the server room as another witness has testified?"
    "Прокурор" "И в тот день, когда мистер Янг сообщил вам - в душе - о незаконных действиях Фетиш Локатора...{w} Вы не присутствовали в серверной комнате, как показал другой свидетель?"

# game/code/d21/d21s07.rpy:138
translate russian d21s07_bd8ffe4f:

    # lc "No, sir."
    lc "Нет, сэр."

# game/code/d21/d21s07.rpy:141
translate russian d21s07_496a5ea5:

    # "Prosecutor" "And after the server room was sealed by investigators you did not enter the server room and delete evidence?"
    "Прокурор" "А после того, как серверная была опечатана следователями, вы не заходили в серверную и не удаляли улики?"

# game/code/d21/d21s07.rpy:144
translate russian d21s07_ac225b0e:

    # lc "Of course not!"
    lc "Нет, конечно!"

# game/code/d21/d21s07.rpy:147
translate russian d21s07_a3ce6ce7:

    # "Prosecutor" "Your fingerprints were found throughout the administrator section of that server room by investigators. How do you explain that?"
    "Прокурор" "Следователи нашли ваши отпечатки пальцев в администраторской части серверной комнаты. Как вы это объясните?"

# game/code/d21/d21s07.rpy:150
translate russian d21s07_f61edb51:

    # lc "They were?"
    lc "Они там были?"

# game/code/d21/d21s07.rpy:153
translate russian d21s07_a7e25541:

    # "Defense Attorney" "Objection, your honor! Defense was not made aware of any fingerprints."
    "Адвокат" "Протестую, ваша честь! Защита не была ознакомлена с отпечатками пальцев."

# game/code/d21/d21s07.rpy:156
translate russian d21s07_b4bc6c15:

    # jdg "Overruled. They were entered into evidence and made available to you during discovery."
    jdg "Отклонено. Они были приобщены к делу и предоставлены вам во время ознакомления."

# game/code/d21/d21s07.rpy:159
translate russian d21s07_c72f53d6:

    # "Defense Attorney" "I never saw anything about this!"
    "Адвокат" "Я ничего об этом не слышала!"

# game/code/d21/d21s07.rpy:162
translate russian d21s07_292b072f:

    # jdg "Did you sleep through the testimony by the fingerprint expert?{w} OVERRULED."
    jdg "Вы спали во время дачи показаний экспертом по отпечаткам пальцев? ОТКЛОНЕНО."

# game/code/d21/d21s07.rpy:165
translate russian d21s07_a4bb28f8:

    # "Prosecutor" "As I was saying, Miss Cox. How do you explain your fingerprints in that server room?"
    "Прокурор" "Как я уже говорил, мисс Кокс. Как вы объясните наличие ваших отпечатков пальцев в серверной?"

# game/code/d21/d21s07.rpy:168
translate russian d21s07_745314b1:

    # lc "I...{w} Alright, I was there, but it wasn't what you think!"
    lc "Я...{w} Ладно, я была там, но по другой причине!"

# game/code/d21/d21s07.rpy:170
translate russian d21s07_b86dd748:

    # lc "Pete would take me there to have sex. I didn't realize it was the same room."
    lc "Пит приводил меня туда, чтобы заняться сексом. Я не знала, что это была одна и та же комната."

# game/code/d21/d21s07.rpy:173
translate russian d21s07_0d696740:

    # "Prosecutor" "We are well aware of Mister Butler's involvement. He has already plead guilty and testified against both of you."
    "Прокурор" "Нам хорошо известно о причастности мистера Батлера. Он уже признал себя виновным и дал показания против вас обоих."

# game/code/d21/d21s07.rpy:174
translate russian d21s07_342c26e6:

    # "Prosecutor" "But that doesn't explain why your fingerprints were found on the server administrator's keyboard."
    "Прокурор" "Но это не объясняет, как ваши отпечатки пальцев оказались на клавиатуре администратора сервера."

# game/code/d21/d21s07.rpy:177
translate russian d21s07_60348f3d:

    # lc "Um...{w} Oh! Sometimes we watched videos while getting intimate. That's probably how my fingerprints got on that keyboard."
    lc "Эм...{w} Иногда мы смотрели видео во время близости. Возможно, поэтому мои отпечатки пальцев оказались на клавиатуре."

# game/code/d21/d21s07.rpy:179
translate russian d21s07_e4e44275:

    # lc "I did go to that room, but only after [mcname] told me about it. I was trying to shut it down!"
    lc "Я действительно заходила в ту комнату, но только после того, как [mcname] рассказал мне о ней. Я пыталась отключить его!"

# game/code/d21/d21s07.rpy:182
translate russian d21s07_ab40b45f:

    # "Prosecutor" "Uh huh. So, when you said you never went to that room... were you lying then or are you lying now?"
    "Прокурор" "Ммм... Значит, когда вы сказали, что никогда не ходили в ту комнату... вы лгали или лжете сейчас?"

# game/code/d21/d21s07.rpy:185
translate russian d21s07_95c3598f:

    # lc "I...{w} I made a mistake."
    lc "Я...{w} Я совершила ошибку."

# game/code/d21/d21s07.rpy:188
translate russian d21s07_fc9885c8:

    # "Prosecutor" "Would you have continued lying to this court if you hadn't been aware of the fingerprint evidence?"
    "Прокурор" "Продолжали бы вы лгать суду, если бы не знали об отпечатках пальцев?"

# game/code/d21/d21s07.rpy:191
translate russian d21s07_3eeb8422:

    # "Defense Attorney" "Objection!"
    "Адвокат" "Протестую!"

# game/code/d21/d21s07.rpy:194
translate russian d21s07_186a6b02:

    # "Prosecutor" "Withdrawn. No further questions."
    "Прокурор" "Вопрос снят. Вопросов больше нет."

# game/code/d21/d21s07.rpy:197
translate russian d21s07_b200f701:

    # jdg "Defense, redirect?"
    jdg "Защита, у вас есть вопросы?"

# game/code/d21/d21s07.rpy:200
translate russian d21s07_64862fc8:

    # "Defense Attorney" "No, your honor.{w} No further questions for this witness."
    "Адвокат" "Нет, ваша честь.{w} Больше вопросов к свидетелю нет."

# game/code/d21/d21s07.rpy:203
translate russian d21s07_27e3931c:

    # jdg "You may step down, Miss Cox. In fact, would a guard please escort this defendant someplace where she can wash her face?"
    jdg "Вы можете покинуть трибуну, мисс Кокс. И вообще, не мог бы сотрудник охраны сопроводить подсудимую туда, где она сможет умыться?"

# game/code/d21/d21s07.rpy:206
translate russian d21s07_6912b8c1:

    # lc "Thank you, your honor."
    lc "Спасибо, ваша честь."

# game/code/d21/d21s07.rpy:210
translate russian d21s07_f479bbae:

    # jdg "We are going to take a 1 hour recess, then return for closing arguments."
    jdg "Мы сделаем перерыв на 1 час, а затем вернемся к окончательному рассмотрению дела."

# game/code/d21/d21s07.rpy:214
translate russian d21s07_4843c2e5:

    # hr "I'm so excited to see that bitch hang. I hope the judge throws the book at her."
    hr "Я так хочу увидеть, как эту суку повесят. Надеюсь, судья засадит ее за решетку."

# game/code/d21/d21s07.rpy:217
translate russian d21s07_0ba3bcf3:

    # mc "Watch it. That's my girlfriend you're talking about."
    mc "Осторожно. Ты говоришь о моей девушке."

# game/code/d21/d21s07.rpy:220
translate russian d21s07_58e9ba4f:

    # hr "So, you don't want to find someplace to have a quickie during this break?"
    hr "Значит, ты не хочешь найти место для быстрого секса во время перерыва?"

# game/code/d21/d21s07.rpy:224
translate russian d21s07_e8bb50d6:

    # mc "No, I'm too nauseous. I feel like I betrayed her."
    mc "Нет, мне очень плохо. Я чувствую, что предал ее."

# game/code/d21/d21s07.rpy:226
translate russian d21s07_4b0b1950:

    # mc "No, I'm too worried about the verdict. I hope she's not guilty."
    mc "Нет, я слишком беспокоюсь о приговоре. Надеюсь, она не виновна."

# game/code/d21/d21s07.rpy:229
translate russian d21s07_4bfebbab:

    # hr "Really? After what she did to you? All those people that she hurt."
    hr "Правда? После того, что она сделала с тобой? Все те люди, которым она причинила боль."

# game/code/d21/d21s07.rpy:232
translate russian d21s07_0e6d1cb4:

    # mc "She wasn't the app. Jerome was behind all that awful shit."
    mc "Она не занималась приложением. За всем этим дерьмом стоял Джером."

# game/code/d21/d21s07.rpy:235
translate russian d21s07_ded718df:

    # hr "Disagree.{w} We'll just have to see how the judge feels about that."
    hr "Не согласна.{w} Посмотрим, как к этому отнесется судья."

# game/code/d21/d21s07.rpy:238
translate russian d21s07_3bc1a514:

    # mc "That's what I'm worried about."
    mc "Это меня и беспокоит."

# game/code/d21/d21s07.rpy:245
translate russian d21s07_867e3fa6:

    # jdg "Is Mr [mcname] Young in attendance?"
    jdg "Присутствует ли в зале мистер [mcname] Янг?"

# game/code/d21/d21s07.rpy:248
translate russian d21s07_3692be19:

    # mc "Yes, your honor."
    mc "Да, ваша честь."

# game/code/d21/d21s07.rpy:251
translate russian d21s07_f479bbae_1:

    # jdg "We are going to take a 1 hour recess, then return for closing arguments."
    jdg "Мы сделаем перерыв на 1 час, а затем вернемся к окончательному рассмотрению дела."

# game/code/d21/d21s07.rpy:253
translate russian d21s07_cb664128:

    # jdg "[mcname] Young, please remain. Prosecutor, please remain as well."
    jdg "[mcname] Янг, пожалуйста, останьтесь. Прокурор, пожалуйста, тоже останьтесь."

# game/code/d21/d21s07.rpy:255
translate russian d21s07_3c28db56:

    # jdg "[mcname] Young, please remain."
    jdg "[mcname] Янг, пожалуйста, останьтесь."

# game/code/d21/d21s07.rpy:256
translate russian d21s07_bbefde22:

    # jdg "Bailiff, please escort everyone else out."
    jdg "Пристав, пожалуйста, уведите всех остальных."

# game/code/d21/d21s07.rpy:260
translate russian d21s07_ae1a978f:

    # "Bailiff" "You heard the Judge!{w} EVERYBODY MOVE!!!"
    "Судебный пристав" "Вы слышали судью!{w} ВСЕМ УЙТИ!!!"

# game/code/d21/d21s07.rpy:268
translate russian d21s07_b116f7cc:

    # mc "Yes, your honor? You wanted to see me?"
    mc "Да, ваша честь? Вы хотели меня видеть?"

# game/code/d21/d21s07.rpy:271
translate russian d21s07_bb0b652d:

    # jdg "I just wanted you to know that if things were slightly different there right now we'd be talking about a very interesting offer."
    jdg "Я просто хотела, чтобы ты знал, что если бы обстоятельства сложились несколько иначе, то сейчас мы бы говорили об очень интересном предложении."

# game/code/d21/d21s07.rpy:274
translate russian d21s07_83c6a848:

    # mc "If things were different?"
    mc "Если бы все было по-другому?"

# game/code/d21/d21s07.rpy:277
translate russian d21s07_ebf4f4a8:

    # jdg "Yes. Have you heard of indentured servitude?"
    jdg "Да. Ты слышал о подневольном труде?"

# game/code/d21/d21s07.rpy:280
translate russian d21s07_2a3b881c:

    # mc "You mean slavery?"
    mc "Ты имеешь в виду рабство?"

# game/code/d21/d21s07.rpy:283
translate russian d21s07_460c1cb2:

    # jdg "Similar concept, but slavery is illegal and immoral."
    jdg "Схожая концепция, но рабство незаконно и аморально."

# game/code/d21/d21s07.rpy:284
translate russian d21s07_43f563bc:

    # jdg "The prosecutor extended an offer to Miss Cox to serve her time under house arrest with a personal warden. Someone to oversee her punishments."
    jdg "Прокурор предложил мисс Кокс отбывать срок под домашним арестом с личным надзирателем. Кто-то, кто будет следить за ее наказанием."

# game/code/d21/d21s07.rpy:285
translate russian d21s07_f87834b5:

    # jdg "Miss Cox said she would only agree if you were assigned as the warden."
    jdg "Мисс Кокс сказала, что согласится, только если тебя назначат надзирателем."

# game/code/d21/d21s07.rpy:288
translate russian d21s07_12578c2b:

    # mc "What? Really?"
    mc "Что? Правда?"

# game/code/d21/d21s07.rpy:291
translate russian d21s07_1fe9fb92:

    # jdg "Yes, but the investigation into your background was disappointing..."
    jdg "Да, но изучение твоей биографии разочаровало..."

# game/code/d21/d21s07.rpy:293
translate russian d21s07_866d26fc:

    # mc "Oh, shit. Am I in trouble?"
    mc "Вот черт. У меня проблемы?"

# game/code/d21/d21s07.rpy:294
translate russian d21s07_16a7213a:

    # jdg "Quite the opposite. The prosecutor decided you were too nice of a person."
    jdg "Совсем наоборот. Прокурор решил, что ты слишком хороший человек."

# game/code/d21/d21s07.rpy:297
translate russian d21s07_881078ea:

    # mc "Too... nice?"
    mc "Слишком... хороший?"

# game/code/d21/d21s07.rpy:299
translate russian d21s07_5cdb1b48:

    # jdg "You were not found suitable to administer a private prison."
    jdg "Тебя признали непригодным для управления личной тюрьмой."

# game/code/d21/d21s07.rpy:300
translate russian d21s07_1f09153a:

    # mc "Oh, so... is there anything I could do?"
    mc "Значит... я могу что-нибудь сделать?"

# game/code/d21/d21s07.rpy:303
translate russian d21s07_ade1821e:

    # jdg "Maybe next time. Your next life, maybe. Who knows? That's the way things go."
    jdg "Может быть, в следующий раз. Может быть, в следующей жизни. Кто знает? Так бывает."

# game/code/d21/d21s07.rpy:305
translate russian d21s07_56945ea0:

    # mct "Did she just quote the Matrix?{w} Whoa."
    mct "Она только что процитировала \"Матрицу\"? Вау."

# game/code/d21/d21s07.rpy:308
translate russian d21s07_15e8bd3e:

    # jdg "Anyway, you can go. I just wanted you to know that it was close."
    jdg "В любом случае, ты можешь идти. Я просто хотел, чтобы ты знал, что это было близко."

# game/code/d21/d21s07.rpy:310
translate russian d21s07_ffc2b8de:

    # mc "You just wanted me to know... about a missed opportunity that I can't do anything about now?"
    mc "Ты просто хотела, чтобы я знал... об упущенной возможности, с которой я теперь ничего не могу поделать?"

# game/code/d21/d21s07.rpy:311
translate russian d21s07_a67ec575:

    # jdg "Exactly."
    jdg "Именно."

# game/code/d21/d21s07.rpy:312
translate russian d21s07_e142ff49:

    # mc "Uh, okay."
    mc "Хм, понятно."

# game/code/d21/d21s07.rpy:320
translate russian d21s07_offer_8f95ad24:

    # jdg "Prosecutor, Mr. Young, please approach the bench."
    jdg "Прокурор, мистер Янг, пожалуйста, подойдите к трибуне."

# game/code/d21/d21s07.rpy:323
translate russian d21s07_offer_891536b4:

    # "Prosecutor" "Of course, your honor."
    "Прокурор" "Конечно, ваша честь."

# game/code/d21/d21s07.rpy:327
translate russian d21s07_offer_8acd4954:

    # mc "Um, sure... What's this about?"
    mc "Конечно... В чём дело?"

# game/code/d21/d21s07.rpy:330
translate russian d21s07_offer_25991953:

    # jdg "I'll let the Prosecutor explain."
    jdg "Я позволю прокурору объяснить."

# game/code/d21/d21s07.rpy:333
translate russian d21s07_offer_19439a3e:

    # "Prosecutor" "You've been watching the trial. You have some idea about how the case is going for your girlfriend."
    "Прокурор" "Вы следили за судебным процессом. У вас есть некоторое представление о том, как продвигается дело вашей девушки."

# game/code/d21/d21s07.rpy:334
translate russian d21s07_offer_f7c497fb:

    # "Prosecutor" "I have offered Miss Cox an unconventional alternative to prison."
    "Прокурор" "Я предложил мисс Кокс нестандартную альтернативу тюрьме."

# game/code/d21/d21s07.rpy:335
translate russian d21s07_offer_452cd0fa:

    # "Prosecutor" "I'm sure you have heard of private prisons."
    "Прокурор" "Я уверен, что вы слышали о частных тюрьмах."

# game/code/d21/d21s07.rpy:337
translate russian d21s07_offer_8852e77f:

    # mc "Uh huh."
    mc "Да."

# game/code/d21/d21s07.rpy:340
translate russian d21s07_offer_04b00c36:

    # "Prosecutor" "And I'm sure you've heard of house arrest."
    "Прокурор" "И я уверен, что вы слышали о домашнем аресте."

# game/code/d21/d21s07.rpy:342
translate russian d21s07_offer_3178d2e6:

    # mc "Well, sure."
    mc "Конечно."

# game/code/d21/d21s07.rpy:346
translate russian d21s07_offer_b474956b:

    # "Prosecutor" "Well, in rare cases such as this we have the opportunity for a combination thereof."
    "Прокурор" "Ну, в редких случаях, как этот, у нас есть возможность для их комбинации."

# game/code/d21/d21s07.rpy:348
translate russian d21s07_offer_528accdb:

    # mc "I don't understand."
    mc "Я не понимаю."

# game/code/d21/d21s07.rpy:351
translate russian d21s07_offer_43bcb953:

    # jdg "The basic concept is that Lydia Cox would be placed in a private prison in her own home, but with you as the warden."
    jdg "Основная концепция заключается в том, что Лидия Кокс будет помещена в частную тюрьму в своем собственном доме, но с вами в качестве надзирателя."

# game/code/d21/d21s07.rpy:354
translate russian d21s07_offer_a6699a69:

    # mc "I'm sorry, what?!"
    mc "Простите, что?!"

# game/code/d21/d21s07.rpy:357
translate russian d21s07_offer_fcf9e8b2:

    # "Prosecutor" "As one of the aggrieved victims of her malfeasance you are eligible to serve as warden of her house arrest."
    "Прокурор" "Как одна из пострадавших сторон от ее неправомерных действий, вы можете стать надзирателем ее домашней тюрьмы."

# game/code/d21/d21s07.rpy:359
translate russian d21s07_offer_434f4098:

    # mc "So, I would be responsible for keeping her fed and making sure she stays within the property limits?"
    mc "Значит, я буду отвечать за то, чтобы ее кормить и следить за тем, чтобы она не выходила за пределы дома?"

# game/code/d21/d21s07.rpy:361
translate russian d21s07_offer_8f868d09:

    # "Prosecutor" "*chuckle* Have you seen what private prisons feed their inmates? You could let her starve if you want."
    "Прокурор" "*хихикает* Вы видели, чем кормят заключенных в частных тюрьмах? Вы можете позволить ей голодать, если хотите."

# game/code/d21/d21s07.rpy:364
translate russian d21s07_offer_08d4aea5:

    # jdg "There would be an electronic implant - much better than those old ankle bracelets - to ensure she stayed within her property limits."
    jdg "Ей вживят электронный имплантат - это гораздо лучше, чем те старые браслеты на лодыжках, - чтобы она не выходила за пределы дома."

# game/code/d21/d21s07.rpy:367
translate russian d21s07_offer_465aab05:

    # "Prosecutor" "She would remain responsible for paying any bills associated with her incarceration. You'd basically get to live there, get paid for it, and punish her however you see fit."
    "Прокурор" "Она останется ответственной за оплату всех счетов, связанных с ее заключением. По сути, вы будете жить там, получать за это деньги и наказывать ее, как сочтете нужным."

# game/code/d21/d21s07.rpy:368
translate russian d21s07_offer_b9357a12:

    # "Prosecutor" "It's a reasonable deal. She would only serve 8 years. She'll be 30 when you are done with her."
    "Прокурор" "Это разумная сделка. Она будет отбывать наказание всего 8 лет. Ей будет 30 лет, когда вы с ней закончите."

# game/code/d21/d21s07.rpy:371
translate russian d21s07_offer_7934d50d:

    # mc "Wait! So, you're going to find her guilty?"
    mc "Подождите! Так вы собираетесь признать ее виновной?"

# game/code/d21/d21s07.rpy:374
translate russian d21s07_offer_2eabed2b:

    # jdg "I can't say what the verdict will be until it is public and official. Besides, I still haven't heard the closing arguments."
    jdg "Я не могу сказать, каким будет вердикт, пока он не станет публичным и официальным. Кроме того, я все еще не слышала заключительных аргументов."

# game/code/d21/d21s07.rpy:377
translate russian d21s07_offer_f9900417:

    # mc "I can't believe this! Why me?"
    mc "Я не могу в это поверить! Почему я?"

# game/code/d21/d21s07.rpy:380
translate russian d21s07_offer_abb437b5:

    # "Prosecutor" "Well, there are three types of victims in this case."
    "Прокурор" "В этом деле есть три группы пострадавших."

# game/code/d21/d21s07.rpy:381
translate russian d21s07_offer_f99cb58f:

    # "Prosecutor" "One group of victims actually liked what happened to them, which makes them unacceptable to me."
    "Прокурор" "Одной группе жертв действительно понравилось то, что с ними произошло, и это делает их неприемлемыми для меня."

# game/code/d21/d21s07.rpy:382
translate russian d21s07_offer_9f986e27:

    # "Prosecutor" "Another group of victims want vengeance - preferably aggressive & violent."
    "Прокурор" "Другая группа - жаждущие мести - желательно агрессивной и жестокой."

# game/code/d21/d21s07.rpy:385
translate russian d21s07_offer_41fbc69a:

    # mc "Are those unacceptable to you as well?"
    mc "Для вас это тоже неприемлемо?"

# game/code/d21/d21s07.rpy:388
translate russian d21s07_offer_9cb76c2c:

    # "Prosecutor" "*chuckle* Heavens No, but there's no way to convince her to choose that over jail."
    "Прокурор" "*хихикает* Боже, нет, но нет способа убедить ее выбрать это вместо тюрьмы."

# game/code/d21/d21s07.rpy:390
translate russian d21s07_offer_e1494031:

    # mc "I assume there's a third group."
    mc "Полагаю, есть и третья группа."

# game/code/d21/d21s07.rpy:391
translate russian d21s07_offer_38c8cce7:

    # "Prosecutor" "The third group is just you."
    "Прокурор" "Третья группа - это только вы."

# game/code/d21/d21s07.rpy:394
translate russian d21s07_offer_d38b27dc:

    # "Prosecutor" "Miss Cox has already signed the paperwork for the plea deal on the condition that you are her warden."
    "Прокурор" "Мисс Кокс уже подписала бумаги о признании вины при условии, что вы будете ее надзирателем."

# game/code/d21/d21s07.rpy:395
translate russian d21s07_offer_65520dd7:

    # "Prosecutor" "She believes that she can manipulate you, but I'm willing to bet you'll turn the tables on her."
    "Прокурор" "Она считает, что может манипулировать вами, но я готов поспорить, что вы ее перехитрите."

# game/code/d21/d21s07.rpy:398
translate russian d21s07_offer_fe495c12:

    # mc "This is insane."
    mc "Это безумие."

# game/code/d21/d21s07.rpy:401
translate russian d21s07_offer_0733447f:

    # jdg "I can assure you that the offer is completely legal."
    jdg "Я могу заверить вас, что это предложение полностью законно."

# game/code/d21/d21s07.rpy:404
translate russian d21s07_offer_7dd7007a:

    # mc "You're asking me to devote the next 8 years of my life to being her warden?"
    mc "Вы просите меня посвятить следующие 8 лет моей жизни тому, чтобы быть ее надзирателем?"

# game/code/d21/d21s07.rpy:407
translate russian d21s07_offer_565af34a:

    # "Prosecutor" "Living rent free, getting paid for it, and otherwise doing whatever you want."
    "Прокурор" "Жить бесплатно, получать за это деньги и делать все, что захочешь."

# game/code/d21/d21s07.rpy:410
translate russian d21s07_offer_d06d682c:

    # jdg "Of course, there will be health & wellness checks every 6 months, but you'll be allowed to schedule those whenever it is convenient."
    jdg "Конечно, каждые 6 месяцев будут проводиться проверки здоровья и самочувствия, но вы сможете назначать их в любое удобное для вас время."

# game/code/d21/d21s07.rpy:412
translate russian d21s07_offer_03954793:

    # mc "What you're asking is immoral, unethical, and completely wrong in every possible way."
    mc "То, о чем вы просите, аморально, неэтично и совершенно неправильно во всех возможных отношениях."

# game/code/d21/d21s07.rpy:414
translate russian d21s07_offer_3e93130c:

    # "Prosecutor" "Perhaps, but consider the alternative."
    "Прокурор" "Возможно, но подумайте об альтернативе."

# game/code/d21/d21s07.rpy:416
translate russian d21s07_offer_6574713b:

    # "Prosecutor" "Women's prisons are few and far between - and massively overcrowded."
    "Прокурор" "Женские тюрьмы немногочисленны и переполнены."

# game/code/d21/d21s07.rpy:417
translate russian d21s07_offer_bdf485ad:

    # "Prosecutor" "If she's lucky enough to get into a women's prison she'll be in gen pop with everyone from insider stock traders to convicted mass murderers."
    "Прокурор" "Если ей повезет попасть в женскую тюрьму, она окажется в одной камере со всеми - от инсайдеров биржевых торговцев до осужденных массовых убийц."

# game/code/d21/d21s07.rpy:418
translate russian d21s07_offer_1ba87b87:

    # "Prosecutor" "More likely, she'll be assigned to either a women's wing or a coed wing of a minimum security prison."
    "Прокурор" "Скорее всего, ее определят либо в женское, либо в совместное отделение тюрьмы с минимальным уровнем контроля."

# game/code/d21/d21s07.rpy:419
translate russian d21s07_offer_c5dc7eee:

    # "Prosecutor" "Sure, she won't be side-by-side with murderers, but she's a pretty girl and... you know how it is."
    "Прокурор" "Конечно, она не будет стоять бок о бок с убийцами, но она красивая девушка и... вы знаете, как это бывает."

# game/code/d21/d21s07.rpy:422
translate russian d21s07_offer_7d2692a3:

    # mc "I need a few minutes to think about it."
    mc "Мне нужно несколько минут, чтобы обдумать это."

# game/code/d21/d21s07.rpy:425
translate russian d21s07_offer_188f7bbe:

    # jdg "Take your time, but I recommend that you don't leave the room."
    jdg "Не торопитесь, но я рекомендую вам не выходить из зала."

# game/code/d21/d21s07.rpy:427
translate russian d21s07_offer_c2251ffe:

    # "Prosecutor" "If you are going to take advantage of this offer, I need your signature on these papers before the end of the recess."
    "Прокурор" "Если вы собираетесь воспользоваться этим предложением, мне нужна ваша подпись на этих бумагах до конца перерыва."

# game/code/d21/d21s07.rpy:430
translate russian d21s07_offer_b8d1d155:

    # mc "Recess? Makes me feel like a kid again."
    mc "Перерыва? Это заставляет меня почувствовать себя ребёнком."

# game/code/d21/d21s07.rpy:433
translate russian d21s07_offer_95501597:

    # jdg "Mr Prosecutor... Phil, would you be so kind as to leave the paperwork and exit. I would like to have a conversation with this Young man."
    jdg "Господин прокурор... Фил, будьте добры, оставьте бумаги и выйдите. Я бы хотела поговорить с этим молодым человеком."

# game/code/d21/d21s07.rpy:436
translate russian d21s07_offer_891536b4_1:

    # "Prosecutor" "Of course, your honor."
    "Прокурор" "Конечно, ваша честь."

# game/code/d21/d21s07.rpy:439
translate russian d21s07_offer_a5aa459f:

    # jdg "Don't worry about the bailiff. They're completely loyal to me."
    jdg "Не волнуйся о приставе. Он абсолютно лоялен ко мне."

# game/code/d21/d21s07.rpy:440
translate russian d21s07_offer_b3525b02:

    # jdg "*chuckle* I could murder you and they'd clean up the body and the mess before court resumes."
    jdg "*улыбается* Я могу убить тебя, и он уберет тело и беспорядок до возобновления суда."

# game/code/d21/d21s07.rpy:443
translate russian d21s07_offer_df2004e3:

    # mc "That's terrifying."
    mc "Это пугает."

# game/code/d21/d21s07.rpy:446
translate russian d21s07_offer_ee8076db:

    # jdg "Relax. I only want to tell you something you shouldn't know."
    jdg "Расслабься. Я лишь хочу сказать тебе то, что тебе не следует знать."

# game/code/d21/d21s07.rpy:449
translate russian d21s07_offer_a49ed4f8:

    # mc "Okay."
    mc "Ладно."

# game/code/d21/d21s07.rpy:452
translate russian d21s07_offer_dbadbbda:

    # jdg "Lydia is going away for a long, long time. Jerome too."
    jdg "Лидия сядет на очень, очень долгое время. Джером тоже."

# game/code/d21/d21s07.rpy:453
translate russian d21s07_offer_da0fddc7:

    # jdg "Jerome is a piece of shit, but if you want to... well, sign the papers."
    jdg "Джером - кусок дерьма, но если ты хочешь... подписывай бумаги."

# game/code/d21/d21s07.rpy:456
translate russian d21s07_offer_44d1e1ee:

    # mc "Wow. That's..."
    mc "Ничего себе. Это..."

# game/code/d21/d21s07.rpy:459
translate russian d21s07_offer_46da830b:

    # jdg "I'm sorry, did you hear something?"
    jdg "Простите, вы что-то слышали?"

# game/code/d21/d21s07.rpy:461
translate russian d21s07_offer_c6bbab0c:

    # "Bailiff" "No, your honor. Just a fly on the wall."
    "Судебный пристав" "Нет, ваша честь. Просто муха на стене."

# game/code/d21/d21s07.rpy:464
translate russian d21s07_offer_72fe20c5:

    # mc "When it says that I can perform any punishments that I see fit..."
    mc "Здесь говорится, что я могу применять любые наказания, которые сочту нужными..."

# game/code/d21/d21s07.rpy:467
translate russian d21s07_offer_e7fb6615:

    # jdg "Use your imagination."
    jdg "Используй свое воображение."

# game/code/d21/d21s07.rpy:469
translate russian d21s07_offer_9aff12c5:

    # jdg "While you decide...{w} don't mind me."
    jdg "Пока ты решаешь...{w} не обращай на меня внимания."

# game/code/d21/d21s07.rpy:483
translate russian d21s07_jdg_quickie_8addf686:

    # jdg "I'm just going to take a moment for myself."
    jdg "Я просто хочу уделить время самой себе."

# game/code/d21/d21s07.rpy:486
translate russian d21s07_jdg_quickie_c5fbdb59:

    # jdg "Do you ever think about it? The people who stand trial here before me?"
    jdg "Ты когда-нибудь думал об этом? О людях, которые стоят передо мной в суде?"

# game/code/d21/d21s07.rpy:487
translate russian d21s07_jdg_quickie_0055fab5:

    # jdg "The defendants. Most confessed for a plea deal, but every once in a while I get a real case like this one."
    jdg "Обвиняемые. Большинство признались в содеянном ради сделки о признании вины, но время от времени мне попадаются настоящие дела, как это."

# game/code/d21/d21s07.rpy:491
translate russian d21s07_jdg_quickie_2a918e6d:

    # jdg "Their fear and anxiety? While some justice of the peace holds their fate in their hands?"
    jdg "Их страх и тревога? Пока какой-то мировой судья вершит их судьбу?"

# game/code/d21/d21s07.rpy:492
translate russian d21s07_jdg_quickie_17264cfe:

    # jdg "Sure, I don't get to actually see the punishments. I have to stay impartial."
    jdg "Конечно, я не вижу, как именно наказывают. Я должна оставаться беспристрастной."

# game/code/d21/d21s07.rpy:493
translate russian d21s07_jdg_quickie_2a2cb083:

    # jdg "But sometimes - late at night - I can't help jilling myself off to fantasies."
    jdg "Но порой - поздно ночью - я не могу не предаваться фантазиям."

# game/code/d21/d21s07.rpy:496
translate russian d21s07_jdg_quickie_650300ee:

    # jdg "Come over here. I want you to shove your cock down my throat."
    jdg "Подойди ко мне. Я хочу, чтобы ты засунул свой член в моё горло."

# game/code/d21/d21s07.rpy:499
translate russian d21s07_jdg_quickie_777fb667:

    # mc "You want me to pretend to rape you right here on the defense table?"
    mc "Ты хочешь, чтобы я притворился, что насилую тебя прямо здесь, на столе защиты?"

# game/code/d21/d21s07.rpy:502
translate russian d21s07_jdg_quickie_ebaf1721:

    # jdg "Just my mouth. We don't have time for more."
    jdg "Только мой рот. У нас нет времени на большее."

# game/code/d21/d21s07.rpy:505
translate russian d21s07_jdg_quickie_72ae1b9d:

    # mc "You're just going to watch?"
    mc "Вы просто будете смотреть?"

# game/code/d21/d21s07.rpy:507
translate russian d21s07_jdg_quickie_4b01d68d:

    # "Bailiff" "Pretend I'm not even here."
    "Судебный пристав" "Представьте, что меня здесь нет."

# game/code/d21/d21s07.rpy:513
translate russian d21s07_jdg_quickie_9712f9d0:

    # mc "Okay then."
    mc "Тогда ладно."

# game/code/d21/d21s07.rpy:516
translate russian d21s07_jdg_quickie_05cfbda1:

    # jdg "It's such a shame we don't have time for more."
    jdg "Как жаль, что у нас нет времени на большее."

# game/code/d21/d21s07.rpy:518
translate russian d21s07_jdg_quickie_6d367bf0:

    # mc "You're the judge. I'm pretty sure recess doesn't end until you say so."
    mc "Ты судья. Я уверен, что перерыв не закончится, пока ты не скажешь."

# game/code/d21/d21s07.rpy:520
translate russian d21s07_jdg_quickie_055c4aaf:

    # jdg "True.{w} Now, show me that hard cock."
    jdg "Верно.{w} А теперь покажи мне свой твердый член."

# game/code/d21/d21s07.rpy:523
translate russian d21s07_jdg_quickie_32193238:

    # jdg "God damn. I've never really taken the time to admire your manhood before."
    jdg "Черт возьми. Я никогда раньше не тратила время на то, чтобы полюбоваться твоим мужским достоинством."

# game/code/d21/d21s07.rpy:524
translate russian d21s07_jdg_quickie_2ddd50aa:

    # jdg "Impressive. You must have women lining up to fuck your magnificent cock."
    jdg "Впечатляет. Должно быть, женщины выстраиваются в очередь, чтобы трахнуть твой великолепный член."

# game/code/d21/d21s07.rpy:527
translate russian d21s07_jdg_quickie_92b666f0:

    # mc "Well, kinda."
    mc "Вроде того."

# game/code/d21/d21s07.rpy:528
translate russian d21s07_jdg_quickie_6cea2c9f:

    # mc "Anyway, I think it's time you shut up now."
    mc "В любом случае, думаю, тебе пора заткнуться."

# game/code/d21/d21s07.rpy:532
translate russian d21s07_jdg_quickie_43452d27:

    # mc "Damn. There is something to be said for an experienced woman."
    mc "Проклятье. Опытной женщине всегда есть что сказать."

# game/code/d21/d21s07.rpy:545
translate russian d21s07_jdg_quickie_05a96cd9:

    # mc "I'm going to cummm so far down your throat!"
    mc "Я собираюсь кончить в твою глотку!"

# game/code/d21/d21s07.rpy:561
translate russian d21s07_jdg_quickie_25e734cb:

    # mc "FUCK YES YOU PSYCHOPATHIC BITCH!!!"
    mc "ДА, БЛЯДЬ, ДА, ПСИХОПАТИЧЕСКАЯ СУКА!!!"

# game/code/d21/d21s07.rpy:568
translate russian d21s07_jdg_quickie_beeee0e5:

    # mc "FUCCKCKCKCKCCKKCCKCKCKCKKK"
    mc "БЛЯЯЯЯЯЯЯ"

# game/code/d21/d21s07.rpy:573
translate russian d21s07_jdg_quickie_134ede17:

    # mc "Hell yes.{w} I could do this all day."
    mc "Да, черт возьми.{w} Я могу заниматься этим весь день."

# game/code/d21/d21s07.rpy:576
translate russian d21s07_jdg_quickie_caa746e4:

    # "Bailiff" "You might want to let her breathe.{w} So I don't have to shoot you."
    "Судебный пристав" "Возможно, вы должны дать ей отдышаться. Чтобы мне не пришлось пристрелить вас."

# game/code/d21/d21s07.rpy:580
translate russian d21s07_jdg_quickie_66596601:

    # jdg "So close...{w} I was so close..."
    jdg "Так близко...{w} Я была так близко..."

# game/code/d21/d21s07.rpy:583
translate russian d21s07_jdg_quickie_62bb87df:

    # mc "To an orgasm?"
    mc "К оргазму?"

# game/code/d21/d21s07.rpy:585
translate russian d21s07_jdg_quickie_7ecba4e5:

    # jdg "I was either going to cum or pass out."
    jdg "Я либо кончу, либо потеряю сознание."

# game/code/d21/d21s07.rpy:587
translate russian d21s07_jdg_quickie_59e1fda0:

    # "Bailiff" "Apologies for interrupting, your honor."
    "Судебный пристав" "Простите, что прерываю, ваша честь."

# game/code/d21/d21s07.rpy:594
translate russian d21s07_jdg_quickie_11d11d3c:

    # jdg "Either way, thank you."
    jdg "В любом случае, спасибо."

# game/code/d21/d21s07.rpy:595
translate russian d21s07_jdg_quickie_0cbdddca:

    # jdg "Thank you both."
    jdg "Спасибо вам обоим."

# game/code/d21/d21s07.rpy:601
translate russian d21s07_jdg_quickie_c0ceae77:

    # mc "No thanks, I'm good."
    mc "Не надо, я в порядке."

# game/code/d21/d21s07.rpy:604
translate russian d21s07_jdg_quickie_cc0cc89a:

    # jdg "Well, I hope you don't mind if I continue solo..."
    jdg "Надеюсь, вы не возражаете, если я продолжу соло..."

# game/code/d21/d21s07.rpy:607
translate russian d21s07_jdg_quickie_784ee8c7:

    # mc "Go right ahead."
    mc "Действуй."

# game/code/d21/d21s07.rpy:608
translate russian d21s07_jdg_quickie_30f037c1:

    # mct "How did a psychopathic bitch like her get to be a judge?"
    mct "Как такая психопатическая сука, стала судьей?"

# game/code/d21/d21s07.rpy:622
translate russian d21s07_offer_p2_bb95ae6d:

    # jdg "Recess is almost over. You only have another minute or two to sign those papers... if that's what you want to do."
    jdg "Перерыв почти закончился. У тебя есть еще минута или две, чтобы подписать эти бумаги... если ты этого хочешь."

# game/code/d21/d21s07.rpy:625
translate russian d21s07_offer_p2_e483c2b8:

    # mc "Alright, well. I've decided."
    mc "Ладно, хорошо. Я решил."

# game/code/d21/d21s07.rpy:633
translate russian d21s07_offer_p2_adb616a9:

    # mc "I'm sure this is in everyone's best interest."
    mc "Думаю, это в интересах каждого."

# game/code/d21/d21s07.rpy:639
translate russian d21s07_offer_p2_b2daac71:

    # mc "No.{w} As Hartigan famously said, \"There's wrong, and there's wrong, and there's *this*\"."
    mc "Нет.{w} Как сказал Хартиган, \"Есть неправильное, есть неправильное, и есть *это*\"."

# game/code/d21/d21s07.rpy:640
translate russian d21s07_offer_p2_6444dae3:

    # mc "I won't sign it."
    mc "Я не буду подписывать."

# game/code/d21/d21s07.rpy:644
translate russian d21s07_offer_p2_16f63cdd:

    # jdg "Well, I can't say that I would have done the same, but... Good Choice."
    jdg "Не могу сказать, что я поступила бы так же, но... Хороший выбор."

# game/code/d21/d21s07.rpy:645
translate russian d21s07_offer_p2_683d73b8:

    # jdg "Please return to your seat in the audience."
    jdg "Пожалуйста, возвращайся на свое место в зале."

# game/code/d21/d21s07.rpy:654
translate russian d21s07_closing_875bd45e:

    # "Bailiff" "All Rise!{w} The People v. Lydia Cox and Jerome Skinner will now resume."
    "Судебный пристав" "Всем встать!{w} Процесс по делу Лидии Кокс и Джерома Скиннера продолжается."

# game/code/d21/d21s07.rpy:655
translate russian d21s07_closing_9d59a917:

    # "Bailiff" "Be seated."
    "Судебный пристав" "Садитесь."

# game/code/d21/d21s07.rpy:659
translate russian d21s07_closing_009a7012:

    # jdg "Lydia Cox, I have before me signed documents for your plea agreement."
    jdg "Лидия Кокс, передо мной подписанные вами документы о признании вины."

# game/code/d21/d21s07.rpy:662
translate russian d21s07_closing_cebf5262:

    # lc "You do, your honor?"
    lc "Правда, ваша честь?"

# game/code/d21/d21s07.rpy:665
translate russian d21s07_closing_30410363:

    # jdg "I do. [mcname] Young has accepted responsibility for your punishments for a duration of 8 years."
    jdg "Да. [mcname] Янг согласился взять на себя ответственность за ваше наказание сроком на 8 лет."

# game/code/d21/d21s07.rpy:666
translate russian d21s07_closing_0265ba44:

    # jdg "During this time you will be under house arrest and required to perform and endure whatever punishments he determines appropriate."
    jdg "В течение этого срока вы будете находиться под домашним арестом и должны будете выполнять и принимать любые наказания, которые он сочтет нужными."

# game/code/d21/d21s07.rpy:667
translate russian d21s07_closing_89e6c1cb:

    # jdg "You will effectively be a felon without rights and an indentured servant with [mcname] Young as your master."
    jdg "Вы фактически станете заключенным без прав и подневольным слугой, хозяином которого будет [mcname] Янг."

# game/code/d21/d21s07.rpy:668
translate russian d21s07_closing_bbb9e04e:

    # jdg "Do you agree to this?"
    jdg "Вы согласны с этим?"

# game/code/d21/d21s07.rpy:671
translate russian d21s07_closing_3437dd76:

    # "Defense Attorney" "She does, your honor. Her signature is already-"
    "Адвокат" "Да, ваша честь. Ее подпись уже..."

# game/code/d21/d21s07.rpy:674
translate russian d21s07_closing_2b5605d7:

    # jdg "Shut up.{w} You are her attorney, but I need to hear it from her."
    jdg "Помолчите.{w} Вы ее адвокат, но мне нужно услышать это от нее."

# game/code/d21/d21s07.rpy:677
translate russian d21s07_closing_abd891f9:

    # lc "I do, your honor."
    lc "Да, ваша честь."

# game/code/d21/d21s07.rpy:680
translate russian d21s07_closing_8964dc44:

    # jdg "In that case, how do you plead?"
    jdg "В таком случае, вы признаете свою вину?"

# game/code/d21/d21s07.rpy:683
translate russian d21s07_closing_e54100a1:

    # lc "I plead guilty, your honor. According to the terms of my plea deal."
    lc "Я признаю свою вину, ваша честь. В соответствии с условиями моего соглашения о признании вины."

# game/code/d21/d21s07.rpy:686
translate russian d21s07_closing_0a1afa0a:

    # jdg "Mr. [mcname] Young, please approach the podium."
    jdg "Мистер [mcname] Янг, пожалуйста, поднимитесь на трибуну."

# game/code/d21/d21s07.rpy:689
translate russian d21s07_closing_fa180e1d:

    # mc "Um, yes...{w} That is me, your honor."
    mc "Эм, да...{w} Это я, ваша честь."

# game/code/d21/d21s07.rpy:692
translate russian d21s07_closing_2adcde9a:

    # jdg "Are you certain?"
    jdg "Вы уверены?"

# game/code/d21/d21s07.rpy:695
translate russian d21s07_closing_41e626a4:

    # mc "Yes, your honor. I am [mcname] Young.{w} My signature is on those documents."
    mc "Да, ваша честь. Я - [mcname] Янг.{w} Моя подпись стоит на этих документах."

# game/code/d21/d21s07.rpy:698
translate russian d21s07_closing_b6c96550:

    # jdg "You agree to the terms of this agreement as well?"
    jdg "Вы также согласны с условиями этого соглашения?"

# game/code/d21/d21s07.rpy:701
translate russian d21s07_closing_595ddc63:

    # mc "I do, your honor."
    mc "Да, ваша честь."

# game/code/d21/d21s07.rpy:704
translate russian d21s07_closing_813ad80e:

    # jdg "Very well. Please be at Miss Cox's house tomorrow afternoon at 3pm for delivery."
    jdg "Очень хорошо. Пожалуйста, будьте в доме мисс Кокс завтра в 3 часа дня для доставки."

# game/code/d21/d21s07.rpy:707
translate russian d21s07_closing_d0d381e6:

    # mc "Delivery, your honor?"
    mc "Доставки, ваша честь?"

# game/code/d21/d21s07.rpy:710
translate russian d21s07_closing_ea46fe6d:

    # jdg "Miss Cox will be stripped, shackled, and prepared for your tender loving care."
    jdg "Мисс Кокс будет раздета, закована в кандалы и подготовлена для вашей тщательной заботы."

# game/code/d21/d21s07.rpy:713
translate russian d21s07_closing_7a9a4091:

    # mc "Oh, wow. Thank you, your honor."
    mc "О, ничего себе. Спасибо, ваша честь."

# game/code/d21/d21s07.rpy:716
translate russian d21s07_closing_f31f7e83:

    # jdg "We will resume Jerome Skinner's case tomorrow at 9am."
    jdg "Мы продолжим рассмотрение дела Джерома Скиннера завтра в 9 утра."

# game/code/d21/d21s07.rpy:719
translate russian d21s07_closing_5906b4ac:

    # js "Wait! Can I get a plea deal too?!"
    js "Подождите! Я могу получить сделку о признании вины?!"

# game/code/d21/d21s07.rpy:722
translate russian d21s07_closing_6c575c75:

    # jdg "Mister Prosecutor?"
    jdg "Господин прокурор?"

# game/code/d21/d21s07.rpy:725
translate russian d21s07_closing_2f0f1d81:

    # "Prosecutor" "No, thanks. I believe he will be better served in a traditional prison."
    "Прокурор" "Нет. Я считаю, что ему будет лучше в обычной тюрьме."

# game/code/d21/d21s07.rpy:729
translate russian d21s07_closing_d9e116f6:

    # js "Nooooooo!"
    js "Нееет!"

# game/code/d21/d21s07.rpy:737
translate russian d21s07_closing_f2e63004:

    # jdg "We will now hear closing arguments. Prosecutor, you may go first."
    jdg "Теперь мы выслушаем заключительные аргументы. Прокурор, вы можете выступать первым."

# game/code/d21/d21s07.rpy:740
translate russian d21s07_closing_ec8da15b:

    # "Prosecutor" "Thank you, your honor.{w} Ladies and gentlemen of the jury..."
    "Прокурор" "Спасибо, ваша честь.{w} Дамы и господа присяжные..."

# game/code/d21/d21s07.rpy:743
translate russian d21s07_closing_aac04d0f:

    # jdg "Ahem.{w} Do I need to remind you that the defendants waived their right to a jury of their peers?"
    jdg "Кхм.{w} Нужно ли напоминать вам, что обвиняемые отказались от своего права на суд присяжных?"

# game/code/d21/d21s07.rpy:746
translate russian d21s07_closing_d428027f:

    # "Prosecutor" "Right, right. Sorry, your honor. Force of habit."
    "Прокурор" "Да, да. Простите, ваша честь. Сила привычки."

# game/code/d21/d21s07.rpy:747
translate russian d21s07_closing_b0a769a0:

    # jdg "You may continue."
    jdg "Продолжайте."

# game/code/d21/d21s07.rpy:748
translate russian d21s07_closing_5b188515:

    # "Prosecutor" "Thank you, your honor..."
    "Прокурор" "Спасибо, ваша честь..."

# game/code/d21/d21s07.rpy:751
translate russian d21s07_closing_8a075ae6:

    # hr "*whispers* Hey! [mcname]! Wake up!"
    hr "*шепчет* Эй! [mcname]! Проснись!"

# game/code/d21/d21s07.rpy:754
translate russian d21s07_closing_851cfffb:

    # mc "Huh? What? Oh.{w}.. sorry, was I snoring?"
    mc "А? Что? О.{w}.. прости, я храпел?"

# game/code/d21/d21s07.rpy:757
translate russian d21s07_closing_00beb01d:

    # hr "*whispers* Shut up and listen. The Judge is about to announce the verdict."
    hr "*шепотом* Заткнись и слушай. Судья вот-вот объявит приговор."

# game/code/d21/d21s07.rpy:760
translate russian d21s07_closing_835e91f9:

    # mc "*whisper* Thanks for waking me."
    mc "*шепотом* Спасибо, что разбудила меня."

# game/code/d21/d21s07.rpy:764
translate russian d21s07_closing_eb030173:

    # jdg "Lydia Cox and Jerome Skinner I find you both guilty on all charges."
    jdg "Лидия Кокс и Джером Скиннер я признаю вас обоих виновными по всем пунктам обвинения."

# game/code/d21/d21s07.rpy:765
translate russian d21s07_closing_78d8a154:

    # jdg "In light of your youth and that this is the first offense for either of you I am inclined to be lenient."
    jdg "Учитывая вашу молодость и то, что это первое правонарушение для каждого из вас, я могу проявить снисхождение."

# game/code/d21/d21s07.rpy:768
translate russian d21s07_closing_b56f86fd:

    # jdg "However, due to the malicious nature of your offenses I am inclined to throw the book at you."
    jdg "Однако из-за злонамеренного характера ваших действий я вынуждена обвинить вас в этом."

# game/code/d21/d21s07.rpy:769
translate russian d21s07_closing_8f0fe270:

    # jdg "Therefore, I sentence you both to 15 years each."
    jdg "Поэтому я приговариваю вас обоих к 15 годам лишения свободы каждого."

# game/code/d21/d21s07.rpy:772
translate russian d21s07_closing_58f66b5f:

    # jdg "Further, I recommend your sentence be carried out in general population at a labor camp."
    jdg "Кроме того, я рекомендую отбывать наказание в трудовом лагере общего режима."

# game/code/d21/d21s07.rpy:773
translate russian d21s07_closing_62fa4df6:

    # jdg "May God have mercy on your anuses."
    jdg "Да смилостивится Господь над вашими анусами."

# game/code/d21/d21s07.rpy:777
translate russian d21s07_closing_6af26fb5:

    # hr "Ah ha! TAKE THAT YOU POND SCUM!"
    hr "Ах-ха! ПОЛУЧИТЕ ЭТО, МРАЗИ!"

# game/code/d21/d21s07.rpy:780
translate russian d21s07_closing_3f0b4122:

    # mc "I didn't think I would feel good about anything today, but I was wrong."
    mc "Не думал, что сегодня буду чувствовать себя хорошо, но я ошибался."

# game/code/d21/d21s07.rpy:781
translate russian d21s07_closing_50e00d70:

    # mc "They are getting exactly what they deserve."
    mc "Они получили именно то, что заслужили."

# game/code/d21/d21s07.rpy:786
translate russian d21s07_closing_eb030173_1:

    # jdg "Lydia Cox and Jerome Skinner I find you both guilty on all charges."
    jdg "Лидия Кокс и Джером Скиннер Я признаю вас обоих виновными по всем пунктам обвинения."

# game/code/d21/d21s07.rpy:787
translate russian d21s07_closing_6ab3369d:

    # jdg "However, I sentence Lydia Cox to 2 years in prison and suspend the sentence."
    jdg "Тем не менее, я приговариваю Лидию Кокс к 2 годам лишения свободы с отсрочкой исполнения приговора."

# game/code/d21/d21s07.rpy:790
translate russian d21s07_closing_794639d0:

    # lc "Your honor? What does that mean?"
    lc "Ваша честь? Что это значит?"

# game/code/d21/d21s07.rpy:793
translate russian d21s07_closing_24fc9002:

    # jdg "You will be on parole for no less than 2 years. If you complete the conditions of your parole you will not need to serve any time."
    jdg "Вы будете освобождены условно-досрочно не менее чем на 2 года. Если вы выполните условия условно-досрочного освобождения, вам не придется отбывать срок."

# game/code/d21/d21s07.rpy:794
translate russian d21s07_closing_2c90be27:

    # jdg "If you fail to comply with the conditions of your parole, you will serve the entire time as required."
    jdg "Если вы не выполните условия условно-досрочного освобождения, вы отбудете весь положенный срок."

# game/code/d21/d21s07.rpy:797
translate russian d21s07_closing_884dc75b:

    # lc "Oh!{w} Thank you, your honor!"
    lc "О!{w} Спасибо, ваша честь!"

# game/code/d21/d21s07.rpy:800
translate russian d21s07_closing_419b97b4:

    # jdg "As for Jerome Skinner, I sentence you to 15 years manual labor."
    jdg "Что касается Джерома Скиннера, я приговариваю вас к 15 годам исправительных работ."

# game/code/d21/d21s07.rpy:804
translate russian d21s07_closing_d9e116f6_1:

    # js "Nooooooo!"
    js "Неееет!"

# game/code/d21/d21s07.rpy:808
translate russian d21s07_closing_5bdbc059:

    # hr "She only got 2 years? I think I'm going to throw up."
    hr "Она получила только 2 года? Кажется, меня сейчас вырвет."

# game/code/d21/d21s07.rpy:811
translate russian d21s07_closing_5611b186:

    # mc "Overall, I feel quite good about the sentences."
    mc "В целом, я совершенно согласен с этим приговором."

# game/code/d21/d21s07.rpy:814
translate russian d21s07_closing_95bf8383:

    # hr "I am never fucking you again."
    hr "Я больше никогда не буду с тобой трахаться."

# game/code/d21/d21s07.rpy:817
translate russian d21s07_closing_d5ae2301:

    # mc "Fair enough."
    mc "Справедливо."

# game/code/d21/d21s07.rpy:821
translate russian d21s07_closing_8e4c1965:

    # jdg "Jerome Skinner, I find you Guilty on all charges."
    jdg "Джером Скиннер, я признаю вас виновным по всем пунктам обвинения."

# game/code/d21/d21s07.rpy:822
translate russian d21s07_closing_02d1a331:

    # jdg "I sentence you to 15 years manual labor. May God have mercy on your anus."
    jdg "Я приговариваю вас к 15 годам исправительных работ. Да смилостивится Господь над вашим анусом."

# game/code/d21/d21s07.rpy:825
translate russian d21s07_closing_4311f40b:

    # lc "Oh my goodness! That's horrible!"
    lc "О, Боже мой! Это ужасно!"

# game/code/d21/d21s07.rpy:828
translate russian d21s07_closing_087b684d:

    # jdg "BE SILENT. As for you, Miss Cox..."
    jdg "СОХРАНЯЙТЕ ТИШИНУ. Что касается вас, мисс Кокс..."

# game/code/d21/d21s07.rpy:830
translate russian d21s07_closing_10b31f51:

    # lc "*gulp*"
    lc "*вздох*"

# game/code/d21/d21s07.rpy:832
translate russian d21s07_closing_0e946468:

    # jdg "Lydia Cox, I find you Not Guilty on all charges."
    jdg "Лидия Кокс, я признаю вас невиновной по всем пунктам обвинения."

# game/code/d21/d21s07.rpy:835
translate russian d21s07_closing_3536779d:

    # lc "I'm... what?!"
    lc "Я... что?!"

# game/code/d21/d21s07.rpy:837
translate russian d21s07_closing_bb9dd4a0:

    # jdg "You are free to go."
    jdg "Вы свободны."

# game/code/d21/d21s07.rpy:839
translate russian d21s07_closing_a0b9b51d:

    # lc "OMG! W00T!!!"
    lc "ОМГ! В00Т!!!"

# game/code/d21/d21s07.rpy:843
translate russian d21s07_closing_32a1953e:

    # hr "Did she just fucking say W-zero-zero-T?"
    hr "Она что, блядь, только что сказала В-зеро-зеро-Т?"

# game/code/d21/d21s07.rpy:846
translate russian d21s07_closing_f62477ed:

    # mc "And she said O-M-G."
    mc "И она сказала О-М-Г."

# game/code/d21/d21s07.rpy:849
translate russian d21s07_closing_43e3ee5f:

    # hr "I fucking hate the universe and everything in it."
    hr "Я чертовски ненавижу эту вселенную и все, что в ней есть."

# game/code/d21/d21s07.rpy:852
translate russian d21s07_closing_9bc68098:

    # mc "At least an innocent woman was set free."
    mc "По крайней мере, невиновную девушку освободили."

# game/code/d21/d21s07.rpy:855
translate russian d21s07_closing_4bb129e6:

    # hr "Innocent?{w} I am never fucking you again."
    hr "Невиновную?{w} Я больше никогда не буду с тобой трахаться."

# game/code/d21/d21s07.rpy:858
translate russian d21s07_closing_7015b377:

    # mc "Did we fuck before?"
    mc "Мы разве трахались раньше?"

# game/code/d21/d21s07.rpy:860
translate russian d21s07_closing_772c0494:

    # hr "Are you shitting me?"
    hr "Ты что, охренел?"

# game/code/d21/d21s07.rpy:862
translate russian d21s07_closing_3cd2cb39:

    # mc "Sorry, there was so much excitement during the weeks Fetish Locator was running... I kinda forgot who I've been with and who I haven't."
    mc "Прости, было столько волнений в течение нескольких недель, пока работал Фетиш Локатор... Я как бы забыл, с кем я был, а с кем нет."

# game/code/d21/d21s07.rpy:865
translate russian d21s07_closing_d3427954:

    # hr "You are so...{w} NEVER AGAIN."
    hr "Ты такой...{w} НИКОГДА."

# game/code/d21/d21s07.rpy:878
translate russian d21s07_closing_d6138452:

    # hr "That twisted cunt caused all this and got off?"
    hr "Эта дрянь все это устроила и так легко отделалась?"

# game/code/d21/d21s07.rpy:881
translate russian d21s07_closing_c338ece6:

    # mc "Hey, at least Jerome is going away for-"
    mc "Эй, по крайней мере, Джером сядет на..."

# game/code/d21/d21s07.rpy:884
translate russian d21s07_closing_9df8f4a4:

    # hr "Screw this.{w} I'm outta here."
    hr "К черту все.{w} Я ухожу отсюда."

# game/code/d21/d21s07.rpy:890
translate russian d21s07_closing_330d1221:

    # mc "Whoa! What the-?"
    mc "Ух ты! Что за...?"

# game/code/d21/d21s07.rpy:892
translate russian d21s07_closing_3535541c:

    # lc "I'm free! I'm Free!! I'M FREE!!!"
    lc "Я свободна! Я свободна!!! Я СВОБОДНА!!!"

# game/code/d21/d21s07.rpy:903
translate russian d21s07_closing_c300e0e6:

    # lc "I've never been so happy. And I love you so much!"
    lc "Я никогда не была так счастлива. И я так сильно тебя люблю!"

# game/code/d21/d21s07.rpy:906
translate russian d21s07_closing_adf57fb8:

    # mc "I never had any doubt that the court would find you innocent."
    mc "Я не сомневался, что суд признает тебя невиновной."

# game/code/d21/d21s07.rpy:909
translate russian d21s07_closing_1deee88a:

    # lc "Innocent, eh? Let me tell you something about that."
    lc "Невиновной, да? Позволь мне рассказать тебе кое-что об этом."

# game/code/d21/d21s07.rpy:910
translate russian d21s07_closing_7800f73f:

    # lc "*whisper* If there weren't so many people here I'd tell you to throw me over that table and have your way with me right now."
    lc "*шепотом* Если бы здесь не было столько людей, я бы сказала тебе перекинуть меня через этот стол и заняться мной прямо сейчас."

# game/code/d21/d21s07.rpy:913
translate russian d21s07_closing_8fea2db2:

    # mc "Hmm..."
    mc "Мммм..."

# game/code/d21/d21s07.rpy:940
translate russian d21s07_closing_4bfa1e82:

    # mc "*whisper* Ignore them. Let's do it anyway."
    mc "*шепотом* Не обращай на них внимания. Давай сделаем это в любом случае."

# game/code/d21/d21s07.rpy:943
translate russian d21s07_closing_3e24eac9:

    # lc "*laughing* You're incorrigible."
    lc "*смеется* Ты неисправим."

# game/code/d21/d21s07.rpy:946
translate russian d21s07_closing_45d8981d:

    # mc "Don't tell that to the judge."
    mc "Не говори об этом судье."

# game/code/d21/d21s07.rpy:947
translate russian d21s07_closing_8f3ae07f:

    # mc "As long as the judge is here, why don't you marry me?"
    mc "Пока судья здесь, может, ты выйдешь за меня замуж?"

# game/code/d21/d21s07.rpy:950
translate russian d21s07_closing_49566215:

    # lc "Are you serious?"
    lc "Ты серьезно?"

# game/code/d21/d21s07.rpy:953
translate russian d21s07_closing_1ae64310:

    # mc "Maybe I am."
    mc "Может, и так."

# game/code/d21/d21s07.rpy:955
translate russian d21s07_closing_dc6d9ee1:

    # lc "Ask me. I'll say, \"yes\". We can do it right now."
    lc "Спроси меня. Я скажу: \"да\". Мы можем сделать это прямо сейчас."

# game/code/d21/d21s07.rpy:957
translate russian d21s07_closing_51e4b633:

    # mc "I was going to wait for a more opportune moment, but..."
    mc "Я собирался подождать более подходящего момента, но..."

# game/code/d21/d21s07.rpy:960
translate russian d21s07_closing_f63f7a5f:

    # lc "You're kidding me! Really?!"
    lc "Ты шутишь! Правда?!"

# game/code/d21/d21s07.rpy:964
translate russian d21s07_closing_7a61b22c:

    # mc "I guess now is as good a time as any."
    mc "Полагаю, сейчас самое подходящее время."

# game/code/d21/d21s07.rpy:965
translate russian d21s07_closing_ef79c4c9:

    # mc "Lydia Cox, will you be my wife?"
    mc "Лидия Кокс, станешь ли ты моей женой?"

# game/code/d21/d21s07.rpy:968
translate russian d21s07_closing_6e034c69:

    # lc "O-M-G!!!"
    lc "О-М-Г!!!"

# game/code/d21/d21s07.rpy:971
translate russian d21s07_closing_1546f760:

    # jdg "Oh, hell. I hate when this happens."
    jdg "О, черт. Ненавижу, когда это происходит."

# game/code/d21/d21s07.rpy:972
translate russian d21s07_closing_2de5af9d:

    # jdg "Alright, if you want to get married here, now, then let's hurry this up-"
    jdg "Ладно, если вы хотите пожениться здесь и сейчас, то давайте поторопимся..."

# game/code/d21/d21s07.rpy:975
translate russian d21s07_closing_d80dc68f:

    # lc "Wait! No!"
    lc "Подождите! Нет!"

# game/code/d21/d21s07.rpy:978
translate russian d21s07_closing_4aef3a38:

    # mc "No?!"
    mc "Нет?!"

# game/code/d21/d21s07.rpy:982
translate russian d21s07_closing_93c29f66:

    # lc "Not that, silly. Of course I'll marry you. I'm the happiest woman in the world right now!"
    lc "Не это, дурачок. Конечно, я выйду за тебя замуж. Я сейчас самая счастливая девушка на свете!"

# game/code/d21/d21s07.rpy:985
translate russian d21s07_closing_b7849be1:

    # mc "Then what did you mean, \"No\"?"
    mc "Тогда что ты имела в виду, сказав \"нет\"?"

# game/code/d21/d21s07.rpy:989
translate russian d21s07_closing_bccceea6:

    # lc "I don't want to get married to you right here, right now. We don't even have a marriage license."
    lc "Я не хочу выходить за тебя прямо здесь и сейчас. У нас даже нет разрешения на брак."

# game/code/d21/d21s07.rpy:992
translate russian d21s07_closing_84f16542:

    # mc "Okay..."
    mc "Ладно..."

# game/code/d21/d21s07.rpy:995
translate russian d21s07_closing_f5807241:

    # lc "*whispers* Besides, I've got a better idea."
    lc "*шепчет* Кроме того, у меня есть идея получше."

# game/code/d21/d21s07.rpy:998
translate russian d21s07_closing_b6a0c879:

    # "*Lydia whispers softly into [mcname]'s ear*"
    "*Лидия тихо шепчет на ухо [mcname]*"

# game/code/d21/d21s07.rpy:1001
translate russian d21s07_closing_82c62797:

    # mc "Oh you dirty bitch. I love you so much right now."
    mc "Ах ты грязная сучка. Я так сильно люблю тебя."

# game/code/d21/d21s07.rpy:1004
translate russian d21s07_closing_72f931db:

    # lc "Ditto, baby. I love you too."
    lc "И я тоже, детка. Я тоже тебя люблю."

# game/code/d21/d21s07.rpy:1015
translate russian d21s07_breakup_6b6dc6f1:

    # mc "Hon, I really don't want to ruin your moment, but-"
    mc "Милая, я действительно не хочу портить тебе момент, но..."

# game/code/d21/d21s07.rpy:1017
translate russian d21s07_breakup_99ee4460:

    # lc "Oh..."
    lc "Ой..."

# game/code/d21/d21s07.rpy:1019
translate russian d21s07_breakup_239cda54:

    # mc "Yeah. I'm sorry. I'm really really sorry."
    mc "Да. Мне жаль. Мне действительно очень жаль."

# game/code/d21/d21s07.rpy:1021
translate russian d21s07_breakup_81cec5f9:

    # lc "But there's someone else..."
    lc "Но есть еще кое-кто..."

# game/code/d21/d21s07.rpy:1024
translate russian d21s07_breakup_d6d51910:

    # mc "I mean, I hope we can still be friends... and you know I'll always be around for the fun times."
    mc "Я имею в виду, надеюсь, что мы все еще можем остаться друзьями... и ты знаешь, что я всегда буду рядом для приятных моментов."

# game/code/d21/d21s07.rpy:1028
translate russian d21s07_breakup_b55446e0:

    # lc "Is it AmRose?"
    lc "Это АмРоуз?"

# game/code/d21/d21s07.rpy:1030
translate russian d21s07_breakup_d1653a94:

    # lc "Is it Stacy?"
    lc "Это Стейси?"

# game/code/d21/d21s07.rpy:1036
translate russian d21s07_breakup_17e5d073:

    # mc "Yeah. Is it that obvious?"
    mc "Да. Это так очевидно?"

# game/code/d21/d21s07.rpy:1039
translate russian d21s07_breakup_71168c22:

    # lc "I could tell. I kinda knew that ever since she came with you to visit me in that holding cell."
    lc "Можно сказать. Я поняла это после того, как она пришла с тобой навестить меня в камере."

# game/code/d21/d21s07.rpy:1042
translate russian d21s07_breakup_c2b5e542:

    # mc "I didn't mean to get your hopes up, but I didn't want to-"
    mc "Я не хотел обнадеживать тебя, но я не хотел..."

# game/code/d21/d21s07.rpy:1045
translate russian d21s07_breakup_f9fcd8d1:

    # lc "Thank you, for that. Not crushing my hopes sooner."
    lc "Спасибо тебе за это. Что не разрушил мои надежды раньше."

# game/code/d21/d21s07.rpy:1046
translate russian d21s07_breakup_747f0a0e:

    # lc "That didn't come out right. I really want to thank you."
    lc "Это вышло неправильно. Я очень хочу поблагодарить тебя."

# game/code/d21/d21s07.rpy:1047
translate russian d21s07_breakup_0ccd157f:

    # lc "Sometimes the fantasy about us together was all that got me through my time in jail."
    lc "Иногда фантазии о нас вместе были единственным, что помогало мне пережить время пребывания в тюрьме."

# game/code/d21/d21s07.rpy:1050
translate russian d21s07_breakup_d326afed:

    # mc "I'll always love you. I hope you know that."
    mc "Я всегда буду любить тебя. Надеюсь, ты это знаешь."

# game/code/d21/d21s07.rpy:1053
translate russian d21s07_breakup_1213baad:

    # lc "Same. You are now, and ever shall be, my friend. One of my favorite people."
    lc "То же самое. Отныне и навсегда ты мой друг. Один из моих любимых людей."

# game/code/d21/d21s07.rpy:1055
translate russian d21s07_breakup_f36e5aec:

    # mc "Exactly."
    mc "Именно."

translate russian strings:

    # game/code/d21/d21s07.rpy:38
    old "Several Weeks Later"
    new "Несколько недель спустя"

    # game/code/d21/d21s07.rpy:508
    old "Facefuck the Judge During Recess"
    new "Оттрахать судью во время перерыва"

    # game/code/d21/d21s07.rpy:508
    old "Refuse Her Request"
    new "Отказать ей в просьбе"

    # game/code/d21/d21s07.rpy:628
    old "Enslave Lydia in Her Own Dungeon"
    new "Отправить Лидию в рабство в ее собственную темницу"

    # game/code/d21/d21s07.rpy:628
    old "Reject the Offer"
    new "Отклонить предложение"

    # game/code/d21/d21s07.rpy:918
    old "Propose Marriage to Lydia"
    new "Предложить Лидии выйти замуж"

    # game/code/d21/d21s07.rpy:918
    old "Let Lydia Down Gently for AmRose"
    new "Расстаться с Лидией и быть с АмРоуз"

    # game/code/d21/d21s07.rpy:918
    old "Let Lydia Down Gently for Stacy"
    new "Расстаться с Лидией и быть со Стейси"

    # game/code/d21/d21s07.rpy:918
    old "Let Lydia Down"
    new "Расстаться с Лидией"

