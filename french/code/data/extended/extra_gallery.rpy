translate french strings:

    # game/code/data/extended/extra_gallery.rpy:17
    old "01. Worst Nightmare"
    new "01. Pire cauchemar"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Unlock the hidden 19th ending"
    new "Débloque la 19ème fin cachée"

    # game/code/data/extended/extra_gallery.rpy:17
    old "02. GG EZ Videogame"
    new "02. Jeu vidéo GG EZ"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Get a high score in the minigame"
    new "Obtenez un score élevé dans le mini-jeu"

    # game/code/data/extended/extra_gallery.rpy:17
    old "03. Somebody that I Used to Know"
    new "03. Quelqu'un que je connaissais"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Unlock Lydia's music video"
    new "Débloque le clip de Lydia"

    # game/code/data/extended/extra_gallery.rpy:17
    old "04. Full Course"
    new "04. Cours complet"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Unlock 18 endings"
    new "Débloque 18 fins"

    # game/code/data/extended/extra_gallery.rpy:17
    old "05. Front Nine"
    new "05. Avant neuf"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Unlock 9 endings"
    new "Débloque 9 fins"

    # game/code/data/extended/extra_gallery.rpy:17
    old "06. What a Wild Ride"
    new "06. Quelle course folle"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Complete all 6 base endings"
    new "Termine les 6 fins de base"

    # game/code/data/extended/extra_gallery.rpy:17
    old "07. WTF Bro?"
    new "07. WTF frère?"

    # game/code/data/extended/extra_gallery.rpy:17
    old "See everything you never wanted to"
    new "Vois tout ce que tu n'as jamais voulu voir"

    # game/code/data/extended/extra_gallery.rpy:17
    old "08. Hacker"
    new "08. Pirate informatique"

    # game/code/data/extended/extra_gallery.rpy:17
    old "View all the secret door codes"
    new "Regarde tous les codes des portes secrètes"

    # game/code/data/extended/extra_gallery.rpy:17
    old "09. In the Beginning..."
    new "09. Au début..."

    # game/code/data/extended/extra_gallery.rpy:17
    old "Make it all the way to Day 15"
    new "Va jusqu'au jour 15"

    # game/code/data/extended/extra_gallery.rpy:17
    old "10. Release the Kraken"
    new "10. Libère le Kraken"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Get free from the cage"
    new "Libère-toi de la cage"

    # game/code/data/extended/extra_gallery.rpy:17
    old "11. Sudo Admin"
    new "11. Administrateur Sudo"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Enter the server room"
    new "Entre dans la salle des serveurs"

    # game/code/data/extended/extra_gallery.rpy:17
    old "12. rm -rf"
    new "12. rm -rf"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Destroy the app... or not"
    new "Détruis l'application... Ou pas"

    # game/code/data/extended/extra_gallery.rpy:17
    old "13. The People Versus..."
    new "13. Le peuple contre..."

    # game/code/data/extended/extra_gallery.rpy:17
    old "Go to the court trial"
    new "Va au procès"

    # game/code/data/extended/extra_gallery.rpy:17
    old "14. Cherry Popped"
    new "14. Cerise sautée"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Finish your first time"
    new "Finis ta première fois"

    # game/code/data/extended/extra_gallery.rpy:17
    old "15. Miss White, with the Candlestick"
    new "15. Miss White, avec le chandelier"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Discover who is behind the app on the LC path"
    new "Découvre qui se cache derrière l'application sur le chemin LC"

    # game/code/data/extended/extra_gallery.rpy:17
    old "16. Keyser Soze"
    new "16. Keyser Soze"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Discover who is behind the app on the NTR path"
    new "Découvrez qui se cache derrière l'application sur le chemin NTR"

    # game/code/data/extended/extra_gallery.rpy:17
    old "17. Real Genius"
    new "17. Un vrai génie"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Finish final exams perfectly"
    new "Termine parfaitement les examens finaux"

    # game/code/data/extended/extra_gallery.rpy:17
    old "18. Keep Pounding That Gavel"
    new "18. Continue de marteler ce marteau"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Get both scenes with the judge"
    new "Prends les deux scènes avec le juge"

    # game/code/data/extended/extra_gallery.rpy:17
    old "19. What Are Friends For"
    new "19. à quoi servent les amis"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Knock up your best friend's gal"
    new "Mets en cloque la fille de ton meilleur ami"

    # game/code/data/extended/extra_gallery.rpy:17
    old "20. Are You Not Entertained"
    new "20. Tu n'es pas amusé"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Play all three scenes at the amusement park"
    new "Joue les trois scènes du parc d'attractions"

    # game/code/data/extended/extra_gallery.rpy:17
    old "21. Oh, the Humanity!"
    new "21. Oh, l'humanité!"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Play all three versions of Lyssa's Wednesday scenes"
    new "Joue les trois versions des scènes du mercredi de Lyssa."

    # game/code/data/extended/extra_gallery.rpy:17
    old "22. Why Would You Do That"
    new "22. Pourquoi ferais-tu ça"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Don't remove the cage, even though you could"
    new "N'enlève pas la cage, même si tu pourrais"

    # game/code/data/extended/extra_gallery.rpy:17
    old "23. Fantastic Fivesome"
    new "23. Fantastic Fivesome"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Maria and some friends are very thankful"
    new "Maria et quelques amis sont très reconnaissants"

    # game/code/data/extended/extra_gallery.rpy:17
    old "24. Craving Pickles and Ice-cream"
    new "24. Envie de cornichons et de glace"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Impregnate every possible person in the queen's harem"
    new "Imprègne toutes les personnes possibles dans le harem de la reine."

    # game/code/data/extended/extra_gallery.rpy:17
    old "25. I Need Sexual Healing"
    new "25. J'ai besoin d'une guérison sexuelle"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Helped all 3 couples at the ski resort"
    new "J'ai aidé les 3 couples à la station de ski"

    # game/code/data/extended/extra_gallery.rpy:17
    old "26. This Magic Moment"
    new "26. Ce moment magique"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Went on the ferris wheel with Daisy"
    new "Je suis allé sur la grande roue avec Daisy."

    # game/code/data/extended/extra_gallery.rpy:17
    old "27. May I panic now, Mistress?"
    new "27. Puis-je paniquer maintenant, Maîtresse?"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Braved the funhouse with Dahlia"
    new "J'ai bravé le funhouse avec Dahlia"

    # game/code/data/extended/extra_gallery.rpy:17
    old "28. Fifty-Five Foot Club"
    new "28. Fifty-Five Foot Club"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Join Polly and Nora on the ferris wheel"
    new "Rejoins Polly et Nora sur la grande roue"

    # game/code/data/extended/extra_gallery.rpy:17
    old "29. That Was Cathartic... for Them"
    new "29. C'était cathartique... Pour eux"

    # game/code/data/extended/extra_gallery.rpy:17
    old "Fix Dahlia and Samiya's relationship"
    new "Répare la relation entre Dahlia et Samiya"

    # game/code/data/extended/extra_gallery.rpy:51
    old "30. What? You wanted more?"
    new "30. Quoi? Vous en vouliez plus?"

    # game/code/data/extended/extra_gallery.rpy:51
    old "Finish all 6 Endings from DLC-1"
    new "Termine les 6 fins du DLC-1"

    # game/code/data/extended/extra_gallery.rpy:51
    old "31. Go Debbie Go"
    new "31. Vas-y Debbie, vas-y"

    # game/code/data/extended/extra_gallery.rpy:51
    old "Have sex with Debbie"
    new "Fais l'amour avec Debbie"

    # game/code/data/extended/extra_gallery.rpy:51
    old "32. Fully Cooked"
    new "32. Entièrement cuit"

    # game/code/data/extended/extra_gallery.rpy:51
    old "Get max points in Lydia's challenges in e07"
    new "Obtient le maximum de points dans les défis de Lydia en e07"

    # game/code/data/extended/extra_gallery.rpy:51
    old "33. Full House"
    new "33. Salle comble"

    # game/code/data/extended/extra_gallery.rpy:51
    old "Play with all the characters in e04"
    new "Joue avec tous les personnages de e04"

    # game/code/data/extended/extra_gallery.rpy:51
    old "34. Day and Night"
    new "34. Jour et nuit"

    # game/code/data/extended/extra_gallery.rpy:51
    old "Finish e04 and e07"
    new "Termine e04 et e07"

    # game/code/data/extended/extra_gallery.rpy:51
    old "35. A lucky dozen"
    new "35. Une douzaine de chanceux"

    # game/code/data/extended/extra_gallery.rpy:51
    old "Finish 12 endings"
    new "Termine 12 fins"

    # game/code/data/extended/extra_gallery.rpy:269
    old "HIGH SCORE :\n[persistent.minigame_max_score]"
    new "SCORE ÉLEVÉ :\n[persistent.minigame_max_score]"

    # game/code/data/extended/extra_gallery.rpy:301
    old "HOW MANY ENDINGS HAVE YOU UNLOCKED?"
    new "COMBIEN DE FINS AS-TU DÉBLOQUÉ?"

    # game/code/data/extended/extra_gallery.rpy:302
    old "9 ENDINGS"
    new "9 FINS"

    # game/code/data/extended/extra_gallery.rpy:303
    old "YOU GOTTA PUMP THOSE NUMBERS UP.\nTHOSE ARE ROOKIE NUMBERS!"
    new "TU DOIS AUGMENTER CES CHIFFRES.\nTHOSE SONT DES NUMÉROS DE ROOKIE !"

    # game/code/data/extended/extra_gallery.rpy:326
    old "6 endings\naren't enough.\nI need all 18 now!!!"
    new "6 fins\ne suffisent pas.\nI'ai besoin des 18 maintenant !!!"

    # game/code/data/extended/extra_gallery.rpy:327
    old "{b}CHANGE MY MIND{/b}"
    new "{b}CHANGER D'AVIS{/b}"

    # game/code/data/extended/extra_gallery.rpy:385
    old "I WANT TO KEEP\nFETISH LOCATOR"
    new "Je veux garder\nFetish Locator"

    # game/code/data/extended/extra_gallery.rpy:387
    old "TO MAKE SURE NO ONE\nWILL BE BLACKMAILED"
    new "Pour être sûr que personne\nsera victime de chantage."

    # game/code/data/extended/extra_gallery.rpy:389
    old "YOU'RE GOING TO USE\nIT FOR GOOD, RIGHT?"
    new "Tu vas l'utiliser\nIT pour le bien, n'est-ce pas?"

    # game/code/data/extended/extra_gallery.rpy:390
    old "RIGHT?"
    new "N'est-ce pas?"

    # game/code/data/extended/extra_gallery.rpy:433
    old "THE FIRST ENDING\nFROM 18 TOTAL ENDINGS"
    new "LA PREMIÈRE FIN\nPARMI 18 FINS AU TOTAL"

    # game/code/data/extended/extra_gallery.rpy:434
    old "IS THAT A REAL ENDING?"
    new "C'est une vraie fin?"

    # game/code/data/extended/extra_gallery.rpy:570
    old "TRADE OFFER"
    new "OFFRE COMMERCIALE"

    # game/code/data/extended/extra_gallery.rpy:572
    old "I receive:"
    new "Je reçois:"

    # game/code/data/extended/extra_gallery.rpy:573
    old "Cum in every pussy\nat the wedding"
    new "Jouis dans chaque\chatte au mariage"

    # game/code/data/extended/extra_gallery.rpy:574
    old "You receive:"
    new "Tu reçois:"

    # game/code/data/extended/extra_gallery.rpy:575
    old "The whole\nharem pregnant"
    new "Tout le\nharem est enceinte"

    # game/code/data/extended/extra_gallery.rpy:710
    old "ORNY"
    new "ORNY"

    # game/code/data/extended/extra_gallery.rpy:711
    old "H"
    new "H"

    # game/code/data/extended/extra_gallery.rpy:712
    old "UNGRY"
    new "DÉGRÉ"

    # game/code/data/extended/extra_gallery.rpy:713
    old "APPY"
    new "POSTULER"

translate french strings:

    # game/code/data/extended/extra_gallery.rpy:51
    old "Finish Ending 4 and Ending 7"
    new "Terminer la fin 4 et la fin 7"

    # game/code/data/extended/extra_gallery.rpy:60
    old "36. Don't threaten me with a good time"
    new "36. Ne me menace pas de passer un bon moment"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Finish all 6 Endings from DLC-2"
    new "Terminez les 6 fins du DLC-2"

    # game/code/data/extended/extra_gallery.rpy:60
    old "37. I saw, I came, I conquered"
    new "37. J'ai vu, je suis venu, j'ai vaincu"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Have all Characters in last orgy in Ending 9"
    new "Ayez tous les personnages lors de la dernière orgie de Ending 9"

    # game/code/data/extended/extra_gallery.rpy:60
    old "38. Golden Pass"
    new "38. Passe d'Or"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Have all Characters in Ending 15"
    new "Ayez tous les personnages de la fin 15"

    # game/code/data/extended/extra_gallery.rpy:60
    old "39. She belongs in a museum"
    new "39. Sa place est dans un musée"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Find golden Lyssa (Ending 3)"
    new "Trouver Lyssa dorée (Fin 3)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "40. Do the Pulp Fiction Thing"
    new "40. Faites le truc de Pulp Fiction"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Do the pulp fiction thing (Ending 18)"
    new "Faites le truc de la pulp fiction (Fin 18)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "41. Backdoor Cat Girl"
    new "41. Porte dérobée de Cat Girl"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Backdoor Cat Girl (Ending 16)"
    new "Porte dérobée de Cat Girl (Fin 16)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "42. Pee Bootcamp"
    new "42. Camp d'entraînement pipi"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Begin Olivia's Training"
    new "Commencez la formation d'Olivia"

    # game/code/data/extended/extra_gallery.rpy:60
    old "43. Choo-choo"
    new "43. Tchou-tchou"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Choo-choo (Ending 5)"
    new "Tchou-tchou (Fin 5)"

    # game/code/data/extended/extra_gallery.rpy:60
    old "44. Londyn Twice"
    new "44. Londyn deux fois"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Meet Londyn in Ending 9 and Ending 18"
    new "Rencontrez Londyn dans la fin 9 et la fin 18"

    # game/code/data/extended/extra_gallery.rpy:60
    old "45. I've seen all the possible outcomes"
    new "45. J'ai vu tous les résultats possibles"

    # game/code/data/extended/extra_gallery.rpy:60
    old "Finish all 18 Endings"
    new "Terminez les 18 fins"

    # game/code/data/extended/extra_gallery.rpy:802
    old "SPIT ON THAT THANG!"
    new "Crache dessus ce truc !"

    # game/code/data/extended/extra_gallery.rpy:823
    old "WHEN HE MANAGES TO GET EVERYONE TO COME TO YOUR FANCY NEW SPA"
    new "Quand il parvient à faire venir tout le monde dans votre nouveau spa chic"

    # game/code/data/extended/extra_gallery.rpy:845
    old "Fucking\na girl"
    new "Putain\de fille"

    # game/code/data/extended/extra_gallery.rpy:847
    old "Fucking\na catgirl"
    new "Putain\de catgirl"

    # game/code/data/extended/extra_gallery.rpy:861
    old "Hard to\nswallow\npills"
    new "Difficile d'avaler\ndes pilules"

    # game/code/data/extended/extra_gallery.rpy:863
    old "He will never\nrun out of pee"
    new "Il ne manquera\jamais de pipi"

    # game/code/data/extended/extra_gallery.rpy:901
    old "NEXT STOP"
    new "PROCHAIN ​​ARRÊT"

    # game/code/data/extended/extra_gallery.rpy:903
    old "PENETRATION STATION"
    new "GARE DE PÉNÉTRATION"

    # game/code/data/extended/extra_gallery.rpy:917
    old "They don't know that\nI appear in two endings"
    new "Ils ne savent pas que\nj'apparais dans deux fins"

